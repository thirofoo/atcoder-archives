/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc283/submissions/37523274
 * Submitted at: 2022-12-24 22:58:41
...
int main() {
    ...
    ll ans = LLONG_MAX;
    bool f1 = true;
    rep(i,w)f1 |= ok[h-1][0][i];
    if(dp[h-1][0] != -1 & f1)ans = min(ans,dp[h-1][0]);
    bool f2 = true;
    rep(i,w)f2 |= ok[h-1][1][i];
    if(dp[h-1][1] != -1 & f2)ans = min(ans,dp[h-1][1]);
