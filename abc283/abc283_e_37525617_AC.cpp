/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc283/submissions/37525617
 * Submitted at: 2022-12-24 23:58:36
...
int main() {
    ...
    vector dp(h,vector<vector<ll>>(2,vector<ll>(2,LLONG_MAX)));
    dp[0][0][0] = 0,dp[0][0][1] = 1;
    for(ll i=1;i<h;i++){
        // dp前列列挙part
        rep(b1,2){
            rep(b2,2){
                if(dp[i-1][b1][b2] == LLONG_MAX)continue;
                ...
                rep(a,2){
                    ...
                    if(f){
                        if(i == h-1){
                            // 最終列確認part
                            bool ff = true;
                            rep(j,w)ff &= check(i,j,b2,a,0);
                            if(ff)dp[i][b2][a] = min(dp[i-1][b1][b2]+a,dp[i][b2][a]);
                        }
                        else dp[i][b2][a] = min(dp[i-1][b1][b2]+a,dp[i][b2][a]);
                    }
                }
            }
        }
    ...
    ll ans = LLONG_MAX;
    rep(i,2)rep(j,2)ans = min(ans,dp[h-1][i][j]);
