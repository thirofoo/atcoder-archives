/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc196/submissions/37610582
 * Submitted at: 2022-12-29 14:15:10
...
ll dfs(ll state,ll now,ll a,ll b){
    ...
    if(a != 0){
        // 2*1横
        if(now%w != w-1 && !(state & (1LL << now+1))){
            ans += dfs(state | (1LL << now) | (1LL << now+1),now,a-1,b);
        }
        // 2*1縦
        if(now/h + 1 != h  && !(state & (1LL << now+w))){
