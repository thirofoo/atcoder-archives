/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/33965962
 * Submitted at: 2022-08-12 23:36:21
 * Problem URL: https://atcoder.jp/contests/ahc013/tasks/ahc013_a
 * Result: AC
 * Execution Time: 2965 ms
 */
...
inline double temp(int start) {
    ...
    double start_temp = 15,end_temp = 0.05;
...
inline double prob(int now,int next,int start) {
    ...
    if(next-now >= 100)return 1;
    return exp((double)(next - now - 100) / temp(start));
}
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        if(!outField(x+dx[dr],y+dy[dr]) && field[x+dx[dr]][y+dy[dr]] != '0'){
            ...
            if(k == 200){
                ...
                double p = prob(bestScore,nowScore,sTime);
                if(tmp < p){
