/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34077818
 * Submitted at: 2022-08-16 09:12:57
...
int n,k,K,moveSize = 0,bestScore = 0,cnt = 0,sTime,nowTime,nm,dr,eleBase = 10;
bool moveF;
//Graph：隣接行列で表現
...
void solve(){
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        //数手分余らせて次へ行く
        if(movement.size()+cand.size()+swapNum(nowTime,sTime)+1 > k){
            total.push(T2(bestScore,movement,connect));
            field = reField;
            movement.assign(0,{});
            bestScore = calcScore();
