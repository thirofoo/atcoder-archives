/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34059392
 * Submitted at: 2022-08-15 12:09:45
...
//k == 5の時の温度は良くわからん。
vector<double> validScore = {0, 0, 130,  100,  50,   30  };
vector<double> startTemp =  {0, 0, 10,   10,   10,   6  };
vector<double> endTemp =    {0, 0, 0.05, 0.05, 0.05, 0.05};
...
int n,k,K,moveSize = 0,bestScore = -INT_MAX,cnt = 0,sTime,nowTime,nm,dr;
bool moveF;
//Graph：隣接行列で表現
...
void solve(){
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        //数手分余らせて次へ行く
        if(movement.size()+cand.size()+swapNum(nowTime,sTime)+1 + 2*K > k){
            //最終仕上げをして送る
            beforeFinish();
            field = reField;
            movement.assign(0,{});
