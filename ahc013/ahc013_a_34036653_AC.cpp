/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34036653
 * Submitted at: 2022-08-14 15:03:59
 * Problem URL: https://atcoder.jp/contests/ahc013/tasks/ahc013_a
 * Result: AC
 * Execution Time: 2957 ms
 */
...
int n,k,moveSize = 0,bestScore = -INT_MAX;
bool moveF;
//Graph：隣接行列で表現
...
void solve(){
    int cnt = 0,sTime = utility::mytm.elapsed(),nowTime,ch = 0,rn1 = rand_int()%moveSize,rn2;
    vector<string> reField = field;

    //実行時間制限までギリギリ近傍シミュレーション
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        
        if(movement.size()+cand.size() > k-swapNum(sTime,nowTime)-1){
