/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/33967293
 * Submitted at: 2022-08-13 00:48:04
 * Problem URL: https://atcoder.jp/contests/ahc013/tasks/ahc013_a
 * Result: RE
 * Execution Time: 3053 ms
 */
...
void solve(){
    int cnt = 0,sTime = utility::mytm.elapsed(),nowTime;
    deque<T> query;
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        if(!outField(x+dx[dr],y+dy[dr]) && field[x+dx[dr]][y+dy[dr]] != '0'){
            ...
            rep(i,dis){
                if(dc == 3)break;
                vx += dx[ddr%4],vy += dy[ddr%4];
                int tox = x+vx,toy = y+vy;
                if(!outField(tox,toy) && field[tox][toy] != '0'){
                    query.push_back(T(tox-dx[ddr%4],toy-dy[ddr%4],tox,toy));
                    moveNum(tox-dx[ddr%4],toy-dy[ddr%4],tox,toy);
                    dc = 0;
                }
                else{
                    vx -= dx[ddr],vy -= dy[ddr];
