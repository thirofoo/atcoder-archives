/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34058411
 * Submitted at: 2022-08-15 11:08:06
...
void solve(){
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        //数手分余らせて次へ行く
        if(movement.size()+cand.size()+swapNum(nowTime,sTime)+1 + K + 2 > k){
            //最終仕上げをして送る
            beforeFinish();
            field = reField;
            movement.assign(0,{});
