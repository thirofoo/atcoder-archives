/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/33967724
 * Submitted at: 2022-08-13 01:18:35
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        if(!outField(x+dx[dr],y+dy[dr]) && field[x+dx[dr]][y+dy[dr]] != '0'){
            ...
            rep(i,dis){
                if(dc == 3)break;
                int dddr = (ddr+r[dc])%4;
                vx += dx[dddr],vy += dy[dddr];
                int tox = x+vx,toy = y+vy;
                if(!outField(tox,toy) && field[tox][toy] != '0'){
                    query.push_back(T(tox-dx[dddr],toy-dy[dddr],tox,toy));
                    moveNum(tox-dx[dddr],toy-dy[dddr],tox,toy);
                    dc = 0;
                }
                else{
                    vx -= dx[dddr],vy -= dy[dddr];
