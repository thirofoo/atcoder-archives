/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34057549
 * Submitted at: 2022-08-15 10:03:35
...
int n,k,moveSize = 0,bestScore = -INT_MAX,cnt = 0,sTime,nowTime;
bool moveF;
//Graph：隣接行列で表現
        ...
        if((field[i][j] == '0' && moveF) || (field[i][j] != '0' && !moveF)){
            rep(nr,4){
                ...
                if(!outField(nx,ny) && ( (field[nx][ny] != '0' && moveF) || (field[nx][ny] == '0' && !moveF) )){
                    ...
                    //score↑で更新
                    if(nowScore > bestScore && movement.size()+cand.size() <= k){
                        bestScore = nowScore;
                        connect = cand;
...
void solve(){
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        //5手分余らせて次へ行く
        if(movement.size()+cand.size()+swapNum(nowTime,sTime)+1 + 5 > k){
            //最終仕上げをして送る
            beforeFinish();
