/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34082921
 * Submitted at: 2022-08-16 15:04:49
...
void solve(){
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        //数手分余らせて次へ行く
        if(movement.size()+cand.size()+swapNum(nowTime,sTime)+1 + 5*K > k){
            //最終仕上げをして送る
            beforeFinish();
            beforeFinish();
            beforeFinish();
