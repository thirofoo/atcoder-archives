/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34079497
 * Submitted at: 2022-08-16 11:19:17
 * Problem URL: https://atcoder.jp/contests/ahc013/tasks/ahc013_a
 * Result: AC
 * Execution Time: 2995 ms
 */
...
int moveSize = 0,bestScore = 0,turn = 0,mSize = 0,eleBase = 10;
bool moveFlag,reFlag = false;
//Graph：隣接行列で表現
...
//swap回数を決める関数
// vector<int> validSwap = {0,0,1000,1000,1000,1000};
...
void beforeFinish(){
    rep(i,n)rep(j,n){
        if((field[i][j] == '0' && moveFlag) || (field[i][j] != '0' && !moveFlag)){
            rep(nr,4){
                ...
                if(!outField(nx,ny) && ( (field[nx][ny] != '0' && moveFlag) || (field[nx][ny] == '0' && !moveFlag) )){
                    ...
                    if(nowScore > bestScore && movement.size()+cand.size()+1 <= k){
                        bestScore = nowScore;
                        connect = cand;
                        if(moveFlag)movement.push_back(T(nx,ny,i,j));
...
void solve(){
    ...
    while((nowTime = utility::mytm.elapsed()) <= TIME_LIMIT){
        ...
        //数手分余らせて次へ行く
        if(mSize+cand.size()+swapNum(nowTime,sTime)+1 + 10 > k){
            //最終仕上げをして送る
            beforeFinish();
