/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc015/submissions/36097867
 * Submitted at: 2022-10-30 18:54:10
...
#define BEAM_DEPTH 10
#define BEAM_WIDTH 10
//-----------------以下から実装部分-----------------//

int su = 0,pa = 0,st = 0,total = 0,nowp,nx,ny,cnt = 0,tx,ty;
vector<vector<bool>> visited;
vector<vector<int>> field(10,vector<int>(10,0));
vector<int> f(100,0);
vector<char> d = {'L','R','F','B'};
priority_queue<T> choice1,choice2;
queue<P> todo;
// 右下左上の順番
...
void solve(){
    ...
    rep(i,100){
        ...
        rep(dee,BEAM_WIDTH){
            if(choice2.empty())break;
            choice1.push(choice2.top());
            choice2.pop();
        }
        while(choice2.empty())choice2.pop();
        while(!choice1.empty()){
            auto [s,f,dd] = choice1.top(); choice1.pop();
            rep(j,4){
                ...
                //     cerr << endl;
                // }
                choice2.push(T(calcScore(nf),nf,d[j]));
            }
        }
        ...
        rep(de,BEAM_DEPTH){
            ...
            while(!choice1.empty()){
                ...
                rep(j,4){
                    ...
                    //     cerr << endl;
                    // }
                    choice1.push(T(calcScore(nf),nf,dd));
                }
            }
        }

        auto [score,nf,dir] = choice2.top();
        field = nf;
        cout << dir << endl;
        cout << flush;

        while(!choice2.empty()){
            // auto [x,y,z] = choice1.top();
            // cerr << x << " : " << z << endl;
            choice2.pop();
        }
        while(!choice1.empty()){
