/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc320/submissions/45622508
 * Submitted at: 2023-09-16 21:48:30
...
int main() {
    ...
    auto dfs = [&](auto self, ll now, ll now_x, ll now_y) -> void {
        if( x[now] != 1e14 ) return;
        x[now] = now_x;
        y[now] = now_y;
        for(auto [to, dx, dy]:Graph[now]) {
            if( x[to] != 1e14 ) continue;
    ...
    rep(i,n) {
        if( x[i] == 1e14 ) cout << "undecidable" << endl;
