/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/zone2021/submissions/54411439
 * Submitted at: 2024-06-09 23:14:14
...
int main() {
	...
	rep(i, n) reps(j, i+1, n) {
		...
		rep(k, 5) {
			...
			bit |= (1LL << idx);
			auto itr = power_min[bit].end(); itr--;
			if(itr != power_min[bit].begin() && ((*itr).second == i) || ((*itr).second == j)) itr--;
			if(itr != power_min[bit].begin() && ((*itr).second == i) || ((*itr).second == j)) itr--;
			if((*itr).first >= pv) ans = max(ans, min(nv, (*itr).first));
		}
	}
	cout << ans << endl;
