/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc335/submissions/49111668
 * Submitted at: 2024-01-06 22:12:50
...
int main() {
    ...
    rep(i,n) {
        ...
        // d が異なる境界でその数字帯の dp を更新
        if( i == n-1 || d != p[i+1].first ) {
            while( g_idx < g.size() && a[g[g_idx][0]] <= d ) {
                ll max_ele = 0;
                for(auto now:g[g_idx]) {
                    max_ele = max(max_ele, dp[now]);
                }
                for(auto now:g[g_idx]) dp[now] = max_ele;
                g_idx++;
            }
        }
    }
