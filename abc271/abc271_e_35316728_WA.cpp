/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc271/submissions/35316728
 * Submitted at: 2022-10-01 22:37:25
...
int main() {
    ...
    todo.push(T(0,0,-1));
    while(!todo.empty()){
        auto [cost,now,idx] = todo.top(); todo.pop();
        if(cost >= c[now])continue;
        c[now] = cost;
        for(auto [to,nc]:Graph[now]){
            auto itr = lower_bound(t[T(now,to,nc)].begin(),t[T(now,to,nc)].end(),idx);
