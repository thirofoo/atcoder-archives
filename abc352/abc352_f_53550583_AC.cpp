/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc352/submissions/53550583
 * Submitted at: 2024-05-17 19:27:25
    ...
    rep(i, g.size()) {
        ...
        rep(j, g.size()) {
            if( j == i ) {
                ...
                rep(k, (1LL << n)) {
                    if( __builtin_popcountll(k) != total_bit ) continue;
                    rep(l, n) {
                        ll n_bit = (g_bit[j] << l);
                ...
                total_bit += __builtin_popcountll(g_bit[j]);
            }
        }
        ll cand_bit = -1;
        bool is_unique = true;
