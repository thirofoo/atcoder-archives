/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc012/submissions/32981659
 * Submitted at: 2022-07-04 18:06:01
 * Problem URL: https://atcoder.jp/contests/ahc012/tasks/ahc012_a
 * Result: AC
 * Execution Time: 2963 ms
 */
...
int CalcScore(){
    ...
    rep(i,10)ans += min(a[i],p_cnt[i]) - (a[i] >= p_cnt[i] ? a[i]-p_cnt[i] : 0);
...
int main() {
    ...
    //         // cout << "no : " << cnt << " " << best_score << " " << now_score << endl << flush;
    //     }
    // }
    // cout << endl;
    int s_time = utility::mytm.elapsed();
    ...
    while(utility::mytm.elapsed() <= 2950){
        ...
        // cout << cnt << " " << flush;
        //どの線を選ぶか
        int l = rand_int()%(100-k),add = (int)(rand_int()%500)-250;
        ...
        if(best_score < now_score){
            ...
            double p = prob(best_score,now_score,s_time);
            if(tmp < p){
