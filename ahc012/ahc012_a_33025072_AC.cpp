/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc012/submissions/33025072
 * Submitted at: 2022-07-07 00:30:32
...
inline double temp(int start) {
    ...
    double start_temp = 50,end_temp = 1;
...
void CalcDiff(){
    l = rand_int()%(R_NUM-2)+1,add = (int)(rand_int()%60)-30;
    auto [X,y1,XX,y2] = line2[l];
    auto [Xx,y3,XXx,y4] = line2[l-1];
    auto [xX,y5,xXX,y6] = line2[l+1];
    if(y2+add <= y3 || y5 <= y1+add)return;
...
void AddLine(){
    int start = -10000;
    rep(i,C_NUM+1)line1.push_back(T(start+b*i,-1e7,start+b*i+1,1e7));
    rep(i,R_NUM+1)line2.push_back(T(-1e7,start+h*i,1e7,start+h*i+1));
}

void output(){
    cout << R_NUM + 1 + C_NUM + 1 << endl;
...
int main(){
    ...
    //直線移動part
    while((cnt++)%1000 != 0 || utility::mytm.elapsed() <= TIME_LIMIT){
        CalcDiff();
    }
