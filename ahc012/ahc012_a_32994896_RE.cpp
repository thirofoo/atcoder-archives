/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc012/submissions/32994896
 * Submitted at: 2022-07-05 13:35:26
...
//各区画内にある苺の数
vector<vector<int>> straw3(R_NUM,vector<int>(C_NUM,0));
...
void CalcDiff(int l,int add){
    ...
    for(int y=y1;y<=y1+add;y++){
        for(auto x:straw2[y+10000]){
            near_x.push_back(x);
            int c1 = straw3[l+1][(x+10000)/(20000/C_NUM)];
            int c2 = straw3[l][(x+10000)/(20000/C_NUM)];
            //苺の区画移動
            straw3[l+1][(x+10000)/(20000/C_NUM)]--;
            straw3[l][(x+10000)/(20000/C_NUM)]++;
            //各区画における苺の数の更新
...
// int CalcDiff2(int l,int add){
//     auto [X,y1,XX,XXX] = line2[l];
//     vector<int> cand_cnt = now_cnt;
//     vector<int> near_x;
//     for(int y=y1+add;y<=y1;y++){
//         for(auto x:straw2[y+10000]){
//             near_x.push_back(x);
//             int c1 = straw3[l][(x+10000)/(20000/C_NUM)];
//             int c2 = straw3[l+1][(x+10000)/(20000/C_NUM)];
//             //苺の区画移動
//             straw3[l][(x+10000)/(20000/C_NUM)]--;
//             straw3[l+1][(x+10000)/(20000/C_NUM)]++;
//             //各区画における苺の数の更新
...
//     }
//     int cand_score = 0;
//     rep(i,10)cand_score += min(a[i],now_cnt[i]);
//     cand_score *= 1e6; cand_score /= total;
//     if(best_score < cand_score){
//         best_score = cand_score;
//         now_cnt = cand_cnt;
...
int main(){
    ...
    rep(i,n){
        ...
        straw2[y+10000].push_back(x);
        straw3[(y+10000)/(20000/R_NUM)][(x+10000)/(20000/C_NUM)]++;
    }
    ...
    int start = -10000;
    rep(i,C_NUM)line1.push_back(T(start+20000/C_NUM*(i+1),-1e7,start+20000/C_NUM*(i+1)+1,1e7));
    rep(i,R_NUM)line2.push_back(T(-1e7,start+20000/R_NUM*(i+1),1e7,start+20000/R_NUM*(i+1)+1));
