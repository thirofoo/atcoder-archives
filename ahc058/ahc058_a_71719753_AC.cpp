/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc058/submissions/71719753
 * Submitted at: 2025-12-14 16:21:16
...
struct Solver {
      ...
      for (const auto &cur : beam) {
        ...
        if (!pre.empty()) {
          ...
          if (!cand2.empty()) {
            ...
            // たまにランダム追加（ただし comparator に乱数は入れない）
            if ((int)selected.size() < pm.KR + pm.KG && (int)cand2.size() >= 3) {
              int idx = (int)(rng.next_u64() % (uint64_t)min(6, (int)cand2.size()));
              auto [ii, jj] = make_pair(cand2[idx].i, cand2[idx].j);
              if (!used2[ii][jj]) selected.push_back({ii, jj});
            }
  ...
  void solve() {
    ...
    while (true) {
      ...
      if (iter % 4 == 0) { pm.KR = 6; pm.KG = 5; }
      if (iter % 4 == 1) { pm.KR = 8; pm.KG = 4; }
      if (iter % 4 == 2) { pm.KR = 7; pm.KG = 3; }
      if (iter % 4 == 3) { pm.KR = 5; pm.KG = 4; }

      pm.guarantee_early = (iter % 3 == 0 ? 2 : 1);
      pm.guarantee_late  = 1;
