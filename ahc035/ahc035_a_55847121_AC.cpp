/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc035/submissions/55847121
 * Submitted at: 2024-07-21 17:32:36
  ...
  void solve() {
    ...
    while(t--) {
      sort(status.begin(), status.end(), [&](const State& a, const State& b) {
        ...
        for(int i = 0; i < m; i++) {
          score1 += (a.status[i] == max_potential[i]) * 100 * t + a.status[i] * (10 - t);
          score2 += (b.status[i] == max_potential[i]) * 100 * t + b.status[i] * (10 - t);
        }
