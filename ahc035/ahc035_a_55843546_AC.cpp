/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc035/submissions/55843546
 * Submitted at: 2024-07-21 16:39:59
  ...
  void solve() {
    ...
    while(t--) {
      sort(status.begin(), status.end(), [&](const State& a, const State& b) {
        ...
        for(int i = 0; i < m; i++) {
          score1 += (a.status[i] == max_potential[i]) * 200 + a.status[i];
          score2 += (b.status[i] == max_potential[i]) * 200 + b.status[i];
        }
