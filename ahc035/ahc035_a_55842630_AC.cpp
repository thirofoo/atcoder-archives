/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc035/submissions/55842630
 * Submitted at: 2024-07-21 16:24:22
  ...
  void solve() {
    ...
    while(t--) {
      sort(status.begin(), status.end(), [&](const State& a, const State& b) {
        ...
        for(int i = 0; i < m; i++) {
          score1 += (a.status[i] == max_potential[i]) * 10000 + a.status[i];
          score2 += (b.status[i] == max_potential[i]) * 10000 + b.status[i];
        }
