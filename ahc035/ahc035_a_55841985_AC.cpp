/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc035/submissions/55841985
 * Submitted at: 2024-07-21 16:13:35
  ...
  void solve() {
    ...
    while(t--) {
      sort(status.begin(), status.end(), [&](const State& a, const State& b) {
        ...
        for(int i = 0; i < m; i++) {
          score1 += (a.status[i] == max_potential[i]);
          score2 += (b.status[i] == max_potential[i]);
        }
