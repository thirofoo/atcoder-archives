/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc035/submissions/55853178
 * Submitted at: 2024-07-21 18:52:30
  ...
  void solve() {
    ...
    while(t--) {
      sort(status.begin(), status.end(), [&](const State& a, const State& b) {
        ...
        for(int i = 0; i < m; i++) {
          score1 += (a.status[i] == max_potential[i]) * 5000 + a.status[i] * a.status[i];
          score2 += (b.status[i] == max_potential[i]) * 5000 + b.status[i] * b.status[i];
        }
