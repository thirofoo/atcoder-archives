/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc346/submissions/51590179
 * Submitted at: 2024-03-23 21:59:35
...
int main() {
    ...
    ll left = 1, right = t.size() * (n+1);
    while( right - left > 1 ) {
        ll mid = (left + right) / 2;
        // mid 個 S を繋げた文字列上で g(T,k) が部分列にあるか
        ll s_num = 1, now = 0;
        rep(i,t.size()) {
            // t[i] が k 個出現するまでに何回 S を使ってどこの idx にいるか
            ll size = idx[t[i]-'a'].size();
