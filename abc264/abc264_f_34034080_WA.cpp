/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc264/submissions/34034080
 * Submitted at: 2022-08-14 12:38:04
            ...
            rep(k,2){
                rep(l,2){
                    ...
                    rep(m,2){
                        ...
                        bool f;
                        if(m == 0){
                            f = ( (field[i][j]-'0')^k^l == (field[nx][ny]-'0')^k );
                            dp[nx][ny][k][f] = min(dp[nx][ny][k][f],dp[i][j][k][l]+(f ? 0 : c[ny]));
                        }
                        else{
                            f = ( (field[i][j]-'0')^k^l == (field[nx][ny]-'0')^l );
                            dp[nx][ny][k][f] = min(dp[nx][ny][k][f],dp[i][j][k][l]+(f ? 0 : c[ny]));
                        }
                    }
                }
