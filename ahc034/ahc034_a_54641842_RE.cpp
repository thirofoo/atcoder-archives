/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc034/submissions/54641842
 * Submitted at: 2024-06-16 18:40:53
...
struct State{
    ...
    inline vector<string> process(char dir) {
        ...
        char miss_dirr = (dir == next_dir1) ? next_dir2 : next_dir1;

        // その方向にルールベースで移動
        ll border;
        // if(dir == 'R') border = n-1-right;
        // else if(dir == 'D') border = n-1-down;
        // else if(dir == 'L') border = n-1-left;
        // else if(dir == 'U') border = n-1-up;
        ...
        while(border >= 0) {
            ...
            if(field[x][y] > 0) {
                ...
                if(abs(field[x][y]) <= have) {
                    ...
                    res_op.emplace_back(string(1, miss_dirr) );
                    auto [dx, dy] = dir_char[miss_dirr];
                    x += dx, y += dy;
                    score += 100 + have;
                    ...
                    res_op.emplace_back(string(1, reverse_dir(miss_dirr)));
                    x -= dx, y -= dy;
                    score += 100 + have;
    ...
    vector<string> beam_search(const vector<vector<int>> &field) {
        ...
        rep(turn, 2*n-1) {
            ...
            while(!beam.empty()) {
                ...
                if(next_beam.size() > BEAM_WIDTH) next_beam.pop();
                // cerr << "Beam size: " << beam.size() << '\n';
                // cerr << "Next Beam size: " << next_beam.size() << '\n';
            }
            swap(beam, next_beam);
        }
        cerr << "Beam Search End !!\n";
        // 一番良い State を取得
        while(beam.size() > 1) beam.pop();
        State best = beam.top();
        ...
        while(now_id != 0) {
            // cerr << "now_id: " << now_id << '\n';
            reverse(traces[now_id].ops.begin(), traces[now_id].ops.end());
            for(auto t : traces[now_id].ops) res.emplace_back(t);
            now_id = traces[now_id].parend_id;
        }
