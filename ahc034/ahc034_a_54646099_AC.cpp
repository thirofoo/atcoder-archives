/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc034/submissions/54646099
 * Submitted at: 2024-06-16 21:10:43
 * Problem URL: https://atcoder.jp/contests/ahc034/tasks/ahc034_a
 * Result: AC
 * Execution Time: 1157 ms
 */
...
struct State{
    ...
    inline vector<string> process(char dir) {
        ...
        if(tdx != 0 && up+down == n-1) {
            score = INF;
            return {};
        }
        if(tdy != 0 && left+right == n-1) {
            score = INF;
            return {};
        }
        ...
        auto [sdx, sdy] = dir_char[miss_dir];
        if(outField(this->x+sdx, this->y+sdy)) {
            score = INF;
            return {};
        }
