/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc034/submissions/54651997
 * Submitted at: 2024-06-17 01:37:45
...
struct State{
    ...
    State() : score(0) {}
    explicit State(vector<vector<short>> field, char next_dir1, char next_dir2) : field(field), next_dir1(next_dir1), next_dir2(next_dir2) {
        height = field.size();
        width = field[0].size();
        x = 0, y = 0;
        have = 0;
    ...
    inline vector<string> process(char dir) {
        ...
        if((dir == 'R' || dir == 'L') && (next_dir1 == 'U' || next_dir2 == 'U') && !field.empty()) {
            field.pop_back();
        }
        return res_op;
    }
...
struct Solver{
    ...
    vector<string> beam_search(const vector<vector<short>> field) {
        vector<Trace> traces;
        vector<string> res;
