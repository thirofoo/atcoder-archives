/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc297/submissions/40508047
 * Submitted at: 2023-04-10 14:54:44
...
int main() {
    ...
    // 主客転倒 & 包除原理
    // あるマスが長方形内に含まれる長方形の数
    // = (全長方形) - (左端・右端・上端・下端が不適なやつ) + (左上・左下・右上・右下が不適なやつ)
    ...
    for(ll i=1;i<=h;i++){
        for(ll j=1;j<=w;j++){
            ...
            if(h*(j-1) >= k)ans -= nCr(h*(j-1),k); // 左端
            if(h*(w-j) >= k)ans -= nCr(h*(w-j),k); // 右端
            if((i-1)*w >= k)ans -= nCr((i-1)*w,k); // 上端
            if((h-i)*w >= k)ans -= nCr((h-i)*w,k); // 下端
