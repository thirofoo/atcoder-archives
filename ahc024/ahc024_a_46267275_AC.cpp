/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc024/submissions/46267275
 * Submitted at: 2023-10-07 10:51:30
...
struct Solver{
    ...
    inline void move_color(int x, int y, int dir) {
        ...
        if( nx == 0 || ny == 0 || nx == r_num+1 || ny == c_num+1 || color[nx][ny] == color[x][y] ) return;
        ...
        if( !flag ) return color_rollback();
        if( !linking_one(nx,ny) ) return color_rollback();
    ...
    inline bool linking_one(int x, int y) {
        ...
        for(int d=0;d<DIR_NUM;d++) {
            ...
            if( !vis[nnx][nny] && vis_color[color[nnx][nny]] ) {
                res = false;
                break;
            }
