/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc024/submissions/45910963
 * Submitted at: 2023-09-24 16:45:29
...
struct Solver{
    ...
    void input(){
        ...
        rep(i,n)rep(j,n) {
            ...
            if( i == 0 || i == n-1 ) {
                region[c[i][j]].neighbor.emplace_back(0);
                region[c[i][j]].color_cnt[0]++;
            }
            if( j == 0 || j == n-1 ) {
                region[c[i][j]].neighbor.emplace_back(0);
                region[c[i][j]].color_cnt[0]++;
