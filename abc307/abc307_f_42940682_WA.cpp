/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc307/submissions/42940682
 * Submitted at: 2023-06-25 09:59:11
...
int main() {
    ...
    vector<ll> ans(n,1e9);
    priority_queue<P,vector<P>,greater<P>> edge, todo;
    rep(i,k){
        ans[a[i]] = 0;
    ...
    for(ll day=1;day<=d;day++){
        ...
        // 2. その人から本日感染する人をdijkstra
        while( !todo.empty() ){
            auto [power,now] = todo.top(); todo.pop();
            if( ans[now] < 1e9 )continue;
            ans[now] = day;
            for(auto [dis,to]:Graph[now]){
                if( ans[to] < 1e9 )continue;
                if( power < dis )edge.push(P(dis,to));
                else todo.push(P(power-dis,to));
            }
        }
    }
    rep(i,n)print( (ans[i] >= 1e9 ? -1 : ans[i]) );
