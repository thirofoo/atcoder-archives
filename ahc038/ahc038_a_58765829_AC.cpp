/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc038/submissions/58765829
 * Submitted at: 2024-10-13 18:39:17
...
struct Solver {
  ...
  Solver() {
    ...
    // 根幹となる Arm の作成
    if(V == 5) arm = Arm(N / 2, N / 2, 0, {1, 2, 4, 8});
    else if(V == 6) arm = Arm(N / 2, N / 2, 0, {1, 2, 4, 8, min(16, (N + 1) / 2)});
    else if(V == 7) arm = Arm(N / 2, N / 2, 0, {1, 1, 2, 4, 8, min(16, (N + 1) / 2)});
    else arm = Arm(N / 2, N / 2, 0, {1, 2, 4, 8, min(16, (N + 1) / 2)});
    return;
  }
