/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc038/submissions/58771081
 * Submitted at: 2024-10-13 21:19:51
...
struct Solver {
  ...
  Solver() {
    ...
    else if(V == 6) arm_length = {1, 2, 4, 8, min(16, (N + 1) / 2)};
    else if(V == 7) arm_length = {1, 1, 2, 4, 8, min(16, (N + 1) / 2)};
    else arm_length = {1, 2, 4, 8, min(16, (N + 1) / 2)};
  ...
  void output() {
    if(V <= 7) {
      cout << arm.arm_length.size() + 1 << endl;
      rep(i, arm.arm_length.size()) cout << i << " " << arm.arm_length[i] << endl;
      cout << arm.base_x << " " << arm.base_y << endl;
    } else {
      // 答え出力
      // vector<P> arms = {{0, 1}, {1, 1}, {2, 1}, {3, 2}, {4, 4}, {5, 8}, {6, 16}, {6, 16}, {7, 1}, {7, 1}, {7, 1}, {7, 1}, {7, 2}, {7, 2}, {7, 2}, {7, 2}};
      vector<P> arms = {
          {0,  1},
          {1,  2},
          {2,  4},
      ...
      for(int i = arm.arm_length.size() - 2; i >= 0; i--) {
        // 前と 1 : 2 になるように調整
        arms[i] = {arms[i].first, min(arms[i].second, (arms[i + 1].second + 1) / 2)};
      }
      arms[arm.arm_length.size()] = arms[arm.arm_length.size() - 1];
      rep(i, V - 1) cout << arms[i].first << " " << arms[i].second << endl;
      cout << arm.base_x << " " << arm.base_y << endl;

      // 最初の leaf 展開用の操作を追加
      int use_leaf       = V - 6;
      string pre_op      = "";
      // vector<string> tmp = {"..RRL.RRL", "...R...R."};
      // vector<string> tmp = {".........", "........."};
      vector<string> tmp = {".LLLLLLLL", ".LLLLLLLL"};
      ...
      rep(i, 2) {
        ...
        if(flag) continue;
        new_answer.emplace_back(op);
      }
      rep(i, answer.size()) {
        if(answer[i].size() == 2 * V) new_answer.emplace_back(answer[i]);
        else {
          string op = "";
