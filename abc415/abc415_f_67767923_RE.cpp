/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc415/submissions/67767923
 * Submitted at: 2025-07-20 02:39:02
 * Problem URL: https://atcoder.jp/contests/abc415/tasks/abc415_f
 * Result: RE
 * Execution Time: 1408 ms
 */
...
class IntervalSet {
  ...
  Interval find_next(ll i) const {
    ...
    if(current_interval.first == -1) return {-1, -1};
...
int main() {
  ...
  while(q--) {
    ...
    if(t == 1) {
      ...
      ans             = max(ans, sg_r.prod(tl3, r + 1));
      ans             = max(ans, sg_l.prod(l, tl2));
