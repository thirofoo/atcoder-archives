/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc415/submissions/67779355
 * Submitted at: 2025-07-20 16:51:28
...
int main() {
  ...
  vector<P> cand;
  rep(i, m) {
    if(cand.empty() || cand.back().first - cand.back().second != a[p[i]] - b[p[i]]) {
  ...
  rep(i, cand.size()) for(ll j = rest; j >= 0; j--) {
    ...
    if(j - v >= 0 && j >= cand[i].first) dp[j - v] = max(dp[j - v], dp[j] + cand[i].first);
