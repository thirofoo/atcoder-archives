/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc253/submissions/32042264
 * Submitted at: 2022-05-28 22:07:37
...
int main() {
    ...
    for(int i=1;i<n;i++){
        for(int j=1;j<m+1;j++){
            ...
            dp[i][j] += dp[i-1][max(0LL,j-k)] - dp[i-1][0] + mod;
            if(k == 0)dp[i][j] -= dp[i-1][j];
        }
        rep(j,m)dp[i][j+1] += dp[i][j];
    }

    cout << (dp[n-1][m].val() + mod)%mod << endl;
