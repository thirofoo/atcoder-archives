/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc344/submissions/51102955
 * Submitted at: 2024-03-10 01:07:13
...
int main() {
    ...
    vector dp(n,vector(n, vector(n, vector<P>(n, P(1e16,-1e8)))));
    dp[0][0][0][0] = P(0,0);
    rep(i,n) rep(j,n) rep(k,n) rep(l,n) {
        ...
        if( i != n-1 ) {
            ...
            ll charge = max((d[i][j]-money-1) / p[mx][my] + 1, 0ll);
            ll n_cnt = cnt + charge + 1;
            ll n_money = money + charge * p[mx][my] - d[i][j];
        ...
        if( j != n-1 ) {
            ...
            ll charge = max((r[i][j]-money-1) / p[mx][my] + 1, 0ll);
            ll n_cnt = cnt + charge + 1;
            ll n_money = money + charge * p[mx][my] - r[i][j];
    ...
    //         ll mx = -1, my = -1, maxi = -1;
    //         rep(k,n) rep(l,n) {
    //             if( maxi == -1 || dp[i][j][k][l].first < maxi ) {
    //                 mx = k, my = l;
    //                 maxi = dp[i][j][k][l].first;
    //             }
    //         }
    //         cerr << "P(" << dp[i][j][mx][my].first << "," << dp[i][j][mx][my].second << ") ";
    //     }
    //     cerr << endl;
    // }
