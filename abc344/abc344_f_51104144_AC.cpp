/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc344/submissions/51104144
 * Submitted at: 2024-03-10 01:55:17
...
int main() {
    ...
    rep(i,n) rep(j,n) rep(k,i+1) rep(l,j+1) {
        ...
        if( i != n-1 ) {
            ...
            if( n_cnt < dp[i+1][j][mx][my].first || (n_cnt == dp[i+1][j][mx][my].first && n_money > dp[i+1][j][mx][my].second) ) {
        ...
        if( j != n-1 ) {
            ...
            if( n_cnt < dp[i][j+1][mx][my].first || (n_cnt == dp[i][j+1][mx][my].first && n_money > dp[i][j+1][mx][my].second) ) {
