/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc344/submissions/51103078
 * Submitted at: 2024-03-10 01:11:13
...
int main() {
    ...
    rep(i,n) rep(j,n) rep(k,n) rep(l,n) {
        ...
        if( i != n-1 ) {
            ...
            ll charge = ( d[i][j] > money ? (d[i][j]-money-1) / p[mx][my] + 1 : 0 );
        ...
        if( j != n-1 ) {
            ...
            ll charge = ( r[i][j] > money ? (r[i][j]-money-1) / p[mx][my] + 1 : 0 );
            ll n_cnt = cnt + charge + 1;
