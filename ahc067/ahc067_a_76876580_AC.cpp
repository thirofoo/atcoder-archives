/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc067/submissions/76876580
 * Submitted at: 2026-06-21 22:09:11
...
void collectCandidatesFromTree(
    ...
    int nmax = min(9, (int)branches.size() + 1);
    ...
    auto build = [&](int n, const vector<int>& chosen) -> pair<vector<Door>, vector<Sw>> {
        ...
        // Seal all non-tree edges with permanent door19.
        for (auto e : nonTree) {
            if ((int)doors.size() >= M) break;
            doors.push_back(edgeDoor(e.first, e.second, 19));
        }
...
int main() {
    ...
    sort(allCands.begin(), allCands.end(), [](const Cand& a, const Cand& b) {
        if (a.est != b.est) return a.est > b.est;
        return a.nbits > b.nbits;
    });

    // Dedup already done, now exact-evaluate only top candidates.
    const int EXACT_LIMIT = min(300, (int)allCands.size());
    ...
    for (int i = 0; i < EXACT_LIMIT; i++) {
        int T = evalT(allCands[i].doors, allCands[i].sws);

        allCands[i].exact = T;

        if (T > bestT) {
            bestT = T;
            bestDoors = allCands[i].doors;
            bestSws = allCands[i].sws;
            bestN = allCands[i].nbits;
            bestTree = allCands[i].tree_id;
        }
    }
