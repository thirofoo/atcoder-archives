/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc137/submissions/36753902
 * Submitted at: 2022-11-24 18:16:47
...
int main() {
    ...
    rep(i,n-1){
        rep(j,m){
            auto [from,to,c] = edge[j];
            if(!(reachable_1[from] && reachable_n[from]))continue;
            if(!(reachable_1[to] && reachable_n[to]))continue;
            if(cost[from] == -LLONG_MAX)continue;
