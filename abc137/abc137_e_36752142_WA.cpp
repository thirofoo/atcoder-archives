/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc137/submissions/36752142
 * Submitted at: 2022-11-24 16:26:31
...
int main() {
    ...
    rep(i,n){
        rep(j,m){
            auto [from,to,c] = edge[j];
            if(cost[from] == -LLONG_MAX)continue;
            if(cost[to] < max(cost[from]+c-(n-1-i+1)*p,0LL)){
                cost[to] = max(cost[from]+c-(n-1-i+1)*p,0LL);
                ng[to] = true;
            }
        }
    }
