/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc327/submissions/47255894
 * Submitted at: 2023-11-04 21:56:38
...
int main() {
    ...
    for(int i=1; i<d; i++) {
        for(auto fp:fruit[i]) {
            lsg.apply(max(0ll,fp-w+1), min(300000ll,fp+w), 1ll);
        }
    }
    ll ans = lsg.all_prod();
    for(int i=d; i<300000; i++) {
        for(auto fp:fruit[i-d+1]) lsg.apply(max(0ll,fp-w+1), min(300000ll,fp+w), -1ll);
        for(auto fp:fruit[i]) lsg.apply(max(0ll,fp-w+1), min(300000ll,fp+w), 1ll);
