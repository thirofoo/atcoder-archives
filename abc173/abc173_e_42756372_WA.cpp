/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc173/submissions/42756372
 * Submitted at: 2023-06-20 10:34:02
...
int main() {
    ...
    bool minus = false;
    mint ans = 1;
    ll used_minus = 1e9+1,used_plus = 1e9+1,unused_minus = -1e9+1,unused_plus = -1e9+1;
    rep(i,n){
        if(i < k){
            ans *= abst[i].first;
            if(!abst[i].second){
    ...
    if( unused_minus*used_minus >= used_plus*unused_plus ){
        print( (ans * unused_minus / used_plus).val() );
    }
    else{
        print( (ans * unused_plus  / used_minus).val() );
    }
