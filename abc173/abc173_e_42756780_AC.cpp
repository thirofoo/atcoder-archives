/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc173/submissions/42756780
 * Submitted at: 2023-06-20 11:07:14
...
int main() {
    ...
    rep(i,n){
        if(i < k){
            ...
            if(!abst[i].second){
                ...
                used_plus = min(used_plus,abst[i].first);
            }
        }
        else{
    ...
    if( unused_minus != -1 && used_plus != 1e9+1 && unused_plus != -1 && used_minus != 1e9+1 ){
        if(unused_minus*used_minus >= used_plus*unused_plus)ans = ans * unused_minus / used_plus;
        else ans = ans * unused_plus  / used_minus;
    }
    else if ( unused_minus != -1 && used_plus != 1e9+1 )ans = ans * unused_minus / used_plus;
    else if( unused_plus != -1 && used_minus != 1e9+1 )ans = ans * unused_plus  / used_minus;
    print(ans.val());
