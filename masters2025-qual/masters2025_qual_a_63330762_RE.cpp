/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/masters2025-qual/submissions/63330762
 * Submitted at: 2025-03-02 16:32:51
  ...
  void solve() {
		...
		while(carried_cnt < 2 * N) {
			...
			rep(d, DIR_NUM) {
				...
				while(true) {
					Point next = now + delta[d];
					if(outField(next) || field[next.x][next.y] != '.') break;
