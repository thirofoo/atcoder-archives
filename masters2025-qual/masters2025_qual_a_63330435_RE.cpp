/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/masters2025-qual/submissions/63330435
 * Submitted at: 2025-03-02 16:25:03
...
struct Solver {
  ...
  void solve() {
		...
		// 穴の上下左右の岩も対象
		vector<vector<bool>> hole_surround(N, vector<bool>(N, false));
		rep(d, DIR_NUM) {
			Point now = hole;
			while(true) {
				Point next = now + delta[d];
				if(outField(next)) break;
				hole_surround[now.x][now.y] = true;
				if(field[next.x][next.y] == '@') {
