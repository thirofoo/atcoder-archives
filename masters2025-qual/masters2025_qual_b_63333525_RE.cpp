/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/masters2025-qual/submissions/63333525
 * Submitted at: 2025-03-02 17:34:39
 * Problem URL: https://atcoder.jp/contests/masters2025-qual/tasks/masters2025_qual_b
 * Result: RE
...
struct Solver {
  ...
  void solve() {
    ...
    Point hole, now_start;
    vector<char> big_ch = {'A', 'B', 'C'};
    vector<char> small_ch = {'a', 'b', 'c'};
    ...
    rep(ch, M) {
      ...
      rep(i, N) rep(j, N) {
        if(field[i][j] != big_ch[ch]) continue;
        hole = Point(i, j);
        break;
      }
      ...
      rep(i, N) rep(j, N) {
        if(field[i][j] != small_ch[ch]) continue;
      ...
      while(carried_cnt < N) {
        ...
        while(!que.empty()) {
          ...
          que.pop();
          if(field[cur.x][cur.y] != '.' && (field[cur.x][cur.y] == small_ch[ch] || use_rock_flag[cur.x][cur.y])) {
            goal = cur;
            break;
          }
          rep(i, DIR_NUM) {
        ...
        while(!que.empty()) {
          ...
          rep(i, DIR_NUM) {
            Point next = cur + delta[i];
            if(outField(next) || dist[next.x][next.y] != INF || (field[next.x][next.y] != '.' && field[next.x][next.y] != big_ch[ch])) continue;
            dist[next.x][next.y] = d + 1;
            prev_p[next.x][next.y] = cur;
            que.push({next, d + 1});
          }
        ...
        // field から鉱石 or 岩を消す
        carried_cnt += (field[goal.x][goal.y] == small_ch[ch] ? 1 : 0);
        field[goal.x][goal.y] = '.';
      }
    }
