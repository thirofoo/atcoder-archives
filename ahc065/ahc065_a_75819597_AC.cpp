/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc065/submissions/75819597
 * Submitted at: 2026-05-16 15:45:39
...
struct Solver {
  ...
  static constexpr int BEAM_WIDTH = 20;
  static constexpr int EXTRA_TRANSITIONS = 3;
  ...
  long long evaluate_state(const BeamState &st, int start_box) const {
    ...
    value += (long long)st.path.size() * OP_PENALTY;
    return value;
  }
  ...
  vector<pair<int, int>> transition_ops_from_state(const BeamState &st, int target) const {
    ...
    set<pair<int, int>> used(ops_shortest.begin(), ops_shortest.end());
    ...
    sort(extras.begin(), extras.end(), [](const auto &a, const auto &b) {
