/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc021/submissions/42958604
 * Submitted at: 2023-06-25 18:59:35
 * Problem URL: https://atcoder.jp/contests/ahc021/tasks/ahc021_a
 * Result: AC
 * Execution Time: 1835 ms
 */
    ...
    void solve(){
        ...
        rep(i,n*(n+1)/2){
            ...
            while(!todo.empty()){
                ...
                if( now_x == lim_r && ball[now_x][now_y] < i )continue;

                // cerr << now_x << " " << now_y << endl;
                // cerr << cost << " " << visited[now_x][now_y] << endl;
                // cerr << flush;
                ...
                rep(j,4){
                    ...
                    todo.push(T2(cost + n*(n+1)/2 - ball[px1][py1] + rand_int()%30 ,px1,py1,now_x,now_y));
                }
            }

            int cand_x = lim_r, cand_y = 0, min_cost = 1e9;
            rep(j,lim_r+1){
                if( ball[cand_x][j] < i )continue;
                if(visited[cand_x][j] < min_cost){
                    min_cost = visited[cand_x][j];
                    cand_y = j;
                }
            }
...
struct Solver{
    ...
    void output(){
        sort(fields.begin(),fields.end(),[](State &f1, State &f2){
            return f1.operation.size() < f2.operation.size();
        });
        rep(i,fields.size()){
            cerr << fields[i].operation.size() << " ";
        }
        cerr << endl;

        cout << fields[0].operation.size() << endl;
        rep(i,fields[0].operation.size()){
            auto [x,y,z,w] = fields[0].operation[i];
            cout << x << " " << y << " " << z << " " << w << endl;
        }
    }
