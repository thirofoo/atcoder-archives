/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc021/submissions/42956789
 * Submitted at: 2023-06-25 18:36:36
        ...
        rep(i,n*(n+1)/2){
            ...
            while(!todo.empty()){
                ...
                rep(j,4){
                    ...
                    // int to_ball = ball[]
                    int next_cost = cost + n*(n+1)/2 - ball[px1][py1];
                    next_cost += (ball[px1][py1] < i ? 1000 : 0);

                    if(visited[px1][py1] <= next_cost)continue;
                    // next_cost += (j >= 2 ? 1000 : 0);

                    todo.push(T2(next_cost ,px1,py1,now_x,now_y));
                }
            }
