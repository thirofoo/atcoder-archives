/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc021/submissions/42952454
 * Submitted at: 2023-06-25 17:18:11
    ...
    void init(){
        ...
        rep(i,n*(n+1)/2){
            ...
            if(x != lim_r){
                px1 = -1, py1 = -1;
                rep(j,x){
                    if( (lim_r+1)*(lim_r+2)/2 <= ball[lim_r][j] ){
                        px1 = x-1;
                        py1 = j;
                        break;
                    }
                ...
                if( py1 <= y ){
                    int ty = y-py1-1;
                    rep(j,ty){
                        px2 = x + dx[2]; py2 = y + dy[2];
                        operation.push_back(T(x,y,px2,py2));
                        swap(place[ball[x][y]],place[ball[px2][py2]]);
                        swap(ball[x][y],ball[px2][py2]);
                        x = px2; y = py2;
                    }
                    px2 = x + dx[0]; py2 = y + dy[0];
                    parent1 = ball[px2][py2];
                    operation.push_back(T(x,y,px2,py2));
                    swap(place[ball[x][y]],place[ball[px2][py2]]);
                    swap(ball[x][y],ball[px2][py2]);
                    x = px2; y = py2;
