/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc062/submissions/74068006
 * Submitted at: 2026-03-14 18:44:41
 * Problem URL: https://atcoder.jp/contests/ahc062/tasks/ahc062_a
 * Result: AC
...
struct Solver {
  ...
  bool try_or_opt(double temp) {
    // Pick random non-endpoint vertex
    const int k = 1 + rand_int() % (V - 2);
    const int v = cur_order[k];
    const int prev = cur_order[k - 1];
    const int next = cur_order[k + 1];

    // Check removal: prev must be adjacent to next
    if (!is_adjacent(prev, next))
      return false;

    // Pick a random grid neighbor of v
    const int nb_cnt = (int)neighbors[v].size();
    const int nb = neighbors[v][rand_int() % nb_cnt];
    const int j = vtx_pos[nb];

    // Skip if nb is already adjacent to v in the path
    if (j == k - 1 || j == k || j == k + 1)
      return false;

    // Try to insert v adjacent to nb:
    // Option A: insert v AFTER nb (between path[j] and path[j+1])
    // Option B: insert v BEFORE nb (between path[j-1] and path[j])
    int insert_pos = -1; // position in the CURRENT array where v will go
    if (j + 1 < V && j != k - 1 && is_adjacent(v, cur_order[j + 1])) {
      insert_pos = j; // insert after j
    } else if (j - 1 >= 0 && j != k + 1 && is_adjacent(v, cur_order[j - 1])) {
      insert_pos = j - 1; // insert after j-1 (= before j)
    } else {
      return false;
    }

    // Compute delta:
    // We remove v from position k and insert it.
    // After removal, the positions of vertices shift.
    // Let's think in terms of the original positions.
    long long delta;
    int new_pos_of_v; // final position of v after the operation

    if (insert_pos > k) {
      // Moving v to the right.
      // After removal from k, vertices k+1..insert_pos shift left by 1.
      // v goes to position insert_pos (which was insert_pos before removal).
      // delta for v: (insert_pos - k) * A[v]
      // delta for shifted vertices: -sum(A[cur_order[t]]) for t in [k+1, insert_pos]
      long long range_sum = 0;
      for (int t = k + 1; t <= insert_pos; ++t)
        range_sum += A[cur_order[t]];
      delta = (long long)(insert_pos - k) * A[v] - range_sum;
      new_pos_of_v = insert_pos;
    } else {
      // Moving v to the left. insert_pos < k.
      // After removal from k, we insert at insert_pos + 1 (in the shifted array).
      // Vertices insert_pos+1..k-1 shift right by 1.
      // v goes from k to insert_pos + 1.
      // delta for v: (insert_pos + 1 - k) * A[v]
      // delta for shifted vertices: +sum(A[cur_order[t]]) for t in [insert_pos+1, k-1]
      long long range_sum = 0;
      for (int t = insert_pos + 1; t <= k - 1; ++t)
        range_sum += A[cur_order[t]];
      delta = (long long)(insert_pos + 1 - k) * A[v] + range_sum;
      new_pos_of_v = insert_pos + 1;
    }
  ...
  void solve() {
    ...
    while (utility::timer.elapsed_ms() < TIME_LIMIT_MS) {
      ...
      // Mix or-opt with 2x2 moves: ~33% or-opt, ~67% 2x2
      if ((rand_int() & 3) == 0) {
        try_or_opt(temp);
      } else {
