/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc017/submissions/38640929
 * Submitted at: 2023-02-05 16:21:51
 * Problem URL: https://atcoder.jp/contests/ahc017/tasks/ahc017_a
 * Result: AC
 * Execution Time: 5944 ms
 */
...
struct Solver{
    ...
    int cnt, turn, start_time, now_time, e1, e2;
    ...
    vector<int> ans, op, start_v, day_cnt, edge_u, edge_v, cost;
    ...
    Solver(){
        ...
        rep(i,m){
            ...
            edge_idx[v][u] = i;
            edge_idx[u][v] = i;
            edge_u.emplace_back(u);
            edge_v.emplace_back(v);
        }
        rep(i,n){
            x,y; cin >> x >> y;
            vertex.emplace_back(T(x,y,i));
        }
        ...
        int sv6,now6 = 1e9;
        rep(i,n){
            auto [x1,y1,idx1] = vertex[i];
            if(i == 0)sv1 = idx1;
            if(i == n-1)sv2 = idx1;
            if(abs(x1-250)+abs(y1-933) < now3){
                now3 = abs(x1-250)+abs(y1-933);
                sv3 = idx1;
            }
            if(abs(x1-750)+abs(y1-933) < now4){
                now4 = abs(x1-750)+abs(y1-933);
                sv4 = idx1;
            }
            if(abs(x1-250)+abs(y1-67) < now5){
                now5 = abs(x1-250)+abs(y1-67);
                sv5 = idx1;
            }
            if(abs(x1-750)+abs(y1-67) < now6){
                now6 = abs(x1-750)+abs(y1-67);
                sv6 = idx1;
            }
