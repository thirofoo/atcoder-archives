/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc386/submissions/61235388
 * Submitted at: 2024-12-29 23:05:41
...
int main() {
  ...
  rep(i, s.size()) rep(d, 2 * k + 1) {
    ll j = i + d - k;
    // 文字変更遷移
    if(0 <= j && j < t.size()) dp[i + 1][d] = min(dp[i + 1][d], dp[i][d] + (s[i] == t[j] ? 0 : 1));
    // t に文字挿入遷移
