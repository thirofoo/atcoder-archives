/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/maximum-cup-2013/submissions/45799753
 * Submitted at: 2023-09-23 00:40:51
...
uint dp[1 << 15][15];
uint lj[15], lk[15];
uint res = 1e9, ji, ki, i, j, k, i1, i2, n, N, K;
...
int main() {
    ...
    dp[1][0] = 0;
    for (i=1; i<N; i++) {
        if( n == 15 && i & (1 << 14) && i & (1 << 13) && i & (1 << 12) ) continue;
        ji = 0, ki = 0;
        for (j=0; j<n; j++) {
            if( i & 1 << j ) lj[ji++] = j;
            else lk[ki++] = j;
