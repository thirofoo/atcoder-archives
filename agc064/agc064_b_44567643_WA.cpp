/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/agc064/submissions/44567643
 * Submitted at: 2023-08-13 23:18:40
...
int main() {
    ...
    rep(i,m) {
        ...
        else rest.push_back(edge[i]);
    }

    // 同じ色 && 端点が共にどの色を欲する場合は先に付け得
    while( !rest.empty() ) {
        auto [a,b,c,i] = rest.front(); rest.pop_front();
        if( uf.same(a,b) ) {
            tmp.push_back(T(a,b,c,i));
            continue;
        }
