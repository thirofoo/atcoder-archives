/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/agc064/submissions/44566343
 * Submitted at: 2023-08-13 22:50:38
        ...
        rep(i,m) {
            ...
            if( (s[a] == c && ( (c == 'R' && r_cnt[a] == 1 ) || (c == 'B' && b_cnt[a] == 1 ) )) ) {
                ...
                if( c == 'R' ) {
                    r_cnt[a]--;
                    r_cnt[b]--;
                    if( r_cnt[b] == 1 && !ok[b] ) f = true;
                }
                else {
                    b_cnt[a]--;
                    b_cnt[b]--;
                    if( b_cnt[b] == 1 && !ok[b] ) f = true;
                }
                uf.merge(a,b);
                ans.push_back(i+1);
            }
                ...
                if( c == 'R' ) {
                    r_cnt[a]--;
                    r_cnt[b]--;
                    if( r_cnt[a] == 1 && !ok[a] ) f = true;
                }
                else {
                    b_cnt[a]--;
                    b_cnt[b]--;
                    if( b_cnt[a] == 1 && !ok[a] ) f = true;
                }
                uf.merge(a,b);
                ans.push_back(i+1);
            }
