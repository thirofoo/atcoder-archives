/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc343/submissions/50865926
 * Submitted at: 2024-03-03 15:04:04
 * Problem URL: https://atcoder.jp/contests/abc343/tasks/abc343_g
 * Result: AC
 * Execution Time: 738 ms
 */
...
int main() {
    ...
    rep(i,n) rep(j,n) {
        ...
        // 完全包含かどうかも確認
        if( s[i].find(s[j]) != string::npos ) duplicate[i][j] = s[j].size();
    }
    ...
    rep(i, (1ll << n)-1) rep(j,n) {
        ...
        rep(k,n) {
            if( i & (1ll << k) ) continue;
            ll ni = i | (1ll << k);
            ll nk = (duplicate[j][k] == s[k].size() ? j : k); // 完全包含時は nk は前のまま
            dp[ni][nk] = min(dp[ni][nk], dp[i][j] + (ll)s[k].size() - duplicate[j][k]);
        }
    }
    ll ans = 1e16;
