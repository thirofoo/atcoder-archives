/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc343/submissions/50863305
 * Submitted at: 2024-03-03 13:35:39
...
int main() {
    ...
    rep(i,n) rep(j,n) {
        ...
        // 完全包含かどうかも確認
        rep(k, abs((ll)si-(ll)sj)+1) {
            if( si <= sj && rh[j].get(k,k+si) == rh[i].get(0,si) ) {
                duplicate[i+1][j+1] = si;
                break;
            }
            if( si > sj && rh[i].get(k,k+sj) == rh[j].get(0,sj) ) {
                duplicate[i+1][j+1] = sj;
                break;
            }
        }
    }
