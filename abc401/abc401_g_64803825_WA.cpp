/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc401/submissions/64803825
 * Submitted at: 2025-04-13 00:56:00
...
int main() {
  ...
  while(time++ <= 100) {
    ...
    rep(i, n) rep(j, n) {
      auto [sx, sy] = s[i];
      auto [gx, gy] = g[j];
      Bint dist     = (Bint) (sx - gx) * (sx - gx) + (Bint) (sy - gy) * (sy - gy);
      if(dist <= (Bint) mid * (Bint) mid) graph.add_edge(i + 1, n + 1 + j, 1);
    }
