/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc401/submissions/64787371
 * Submitted at: 2025-04-12 22:27:21
...
int main() {
  ...
  auto dfs_dp = [&](const vector<vector<ll>> &g) -> vector<ll> {
    ...
    auto dfs1 = [&](auto dfs1, ll v, ll p) -> void {
      dp[v] = 0;
      for(ll u: g[v]) {
        if(u == p) continue;
        dfs1(dfs1, u, v);
    ...
    auto dfs2 = [&](auto dfs2, ll v, ll p, ll up) -> void {
      ans[v]  = max(dp[v], up);
      ll best = -1, second = -1;
      int bestCount = 0;
      for(ll u: g[v]) {
  ...
  ll ans = 0;
  for(ll x: dp1) ans += x * n2;
  for(ll x: dp2) ans += x * n1;
  ans += n1 * n2;
  cout << ans << "\n";
