/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc061/submissions/73382839
 * Submitted at: 2026-02-17 16:52:52
...
struct Params {
  ...
  double eval_disconnect_w;
};

static constexpr Params PARAMS_BY_M[9] = {
  {20, 9, 15, 0.15, 0.10, 0.50, 0.65, 0.10, 0.7, 0.15, 500.0},
  {20, 9, 15, 0.15, 0.10, 0.50, 0.65, 0.10, 0.7, 0.15, 500.0},
  {20, 9, 15, 0.15, 0.10, 0.50, 0.65, 0.10, 0.7, 0.15, 500.0},
  {20, 9, 15, 0.15, 0.10, 0.50, 0.65, 0.10, 0.7, 0.15, 500.0},
  {20, 9, 15, 0.15, 0.10, 0.50, 0.65, 0.10, 0.7, 0.15, 500.0},
...
struct Solver {
  ...
  // Containment-aware rollout move selection for M=2, U>=2
  int choose_my_rollout_move_containment(const State& st, double r_eps, double r_sel) const {
    MoveList my_moves = enumerate_moves(st, 0);
    if(my_moves.n <= 1) return my_moves.a[0];

    // 15% epsilon: random exploration for MCTS diversity
    if(r_eps < 0.15) {
      return my_moves.a[pick_index(r_sel, my_moves.n)];
    }

    MoveList enemy_moves = enumerate_moves(st, 1);
    array<bool, 100> enemy_reach{};
    for(int i = 0; i < enemy_moves.n; i++) {
      enemy_reach[enemy_moves.a[i]] = true;
    }

    int best = -1;
    double best_v = -1;

    // P1: enemy lv1 cells in both reaches (capture = cuts enemy territory)
    for(int i = 0; i < my_moves.n; i++) {
      int cell = my_moves.a[i];
      if(st.owner[cell] == 1 && st.level[cell] == 1 && enemy_reach[cell]) {
        if(V[cell] > best_v) { best_v = (double) V[cell]; best = cell; }
      }
    }
    if(best >= 0) return best;

    // P2: our lv<2 cells in enemy's reach (reinforce = defense + block)
    best = -1; best_v = -1;
    for(int i = 0; i < my_moves.n; i++) {
      int cell = my_moves.a[i];
      if(st.owner[cell] == 0 && st.level[cell] < 2 && enemy_reach[cell]) {
        if(V[cell] > best_v) { best_v = (double) V[cell]; best = cell; }
      }
    }
    if(best >= 0) return best;

    // P3: unowned cells in enemy's reach (block expansion)
    best = -1; best_v = -1;
    for(int i = 0; i < my_moves.n; i++) {
      int cell = my_moves.a[i];
      if(st.owner[cell] == -1 && enemy_reach[cell]) {
        if(V[cell] > best_v) { best_v = (double) V[cell]; best = cell; }
      }
    }
    if(best >= 0) return best;

    // P4: any enemy lv1 cell we can reach
    best = -1; best_v = -1;
    for(int i = 0; i < my_moves.n; i++) {
      int cell = my_moves.a[i];
      if(st.owner[cell] == 1 && st.level[cell] == 1) {
        if(V[cell] > best_v) { best_v = (double) V[cell]; best = cell; }
      }
    }
    if(best >= 0) return best;

    // P5: move closer to enemy position
    best = -1; best_v = -1;
    int best_dist = 1000;
    int ep = st.pos[1];
    int epr = ep / N, epc = ep % N;
    for(int i = 0; i < my_moves.n; i++) {
      int cell = my_moves.a[i];
      if(st.owner[cell] == 0 && st.level[cell] >= U) continue;
      int cr = cell / N, cc = cell % N;
      int dist = abs(cr - epr) + abs(cc - epc);
      if(dist < best_dist || (dist == best_dist && V[cell] > best_v)) {
        best_dist = dist; best_v = (double) V[cell]; best = cell;
      }
    }
    if(best >= 0) return best;
  ...
  double rollout_once(const State& root, int first_move, int horizon, uint64_t sample_id) const {
    ...
    constexpr int kResampleInterval = 4;
    for(int d = 0; d < horizon && st.t < T; d++) {
      // Re-sample enemy params periodically to avoid single-model bias
      if(d > 0 && (d % kResampleInterval) == 0) {
        for(int p = 1; p < M; p++) {
          sampled_enemy[p] = sample_enemy_param_rollout(p, rng.rand_double());
        }
      }
      ...
      if(d == 0) {
        ...
        target[0] = choose_my_rollout_move_containment(st, rng.rand_double(), rng.rand_double());
      } else {
        target[0] = choose_my_rollout_move(st, rng.rand_double(), rng.rand_double());
      }
      for(int p = 1; p < M; p++) {
  ...
  pair<int, int> monte_carlo_rollout_decision(const State& root, double turn_end) const {
    ...
    while(utility::mytm.elapsed() <= turn_end) {
      ...
      for(int i = 0; i < K; i++) {
        ...
        double explore = C * ucb_scale * sqrt(log_total / cnt[i]);
        explore = max(explore, ucb_scale * 0.03); // anti-convergence floor
        double ucb = mean_i + explore;
        if(ucb > best_ucb) {
          best_ucb = ucb;
          sel = i;
        }
