/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc061/submissions/73522875
 * Submitted at: 2026-02-22 11:41:36
 * Problem URL: https://atcoder.jp/contests/ahc061/tasks/ahc061_a
 * Result: AC
 * Execution Time: 1899 ms
 */
...
struct Solver {
  ...
  static constexpr int kEnemyUpdateMinMoves = 8;
  ...
  void init_enemy_estimator() {
    ...
    for(double wa : grid) {
      for(double wb : grid) {
        for(double wc : grid) {
          for(double wd : grid) {
            // Ratio parameterization: normalize by wa so inference is scale-invariant.
            // wa is fixed to 1.0 and others are treated as ratios to wa.
            double inv_wa = 1.0 / wa;
            enemy_candidates.push_back({1.0, wb * inv_wa, wc * inv_wa, wd * inv_wa});
          }
        }
      }
    }
    ...
    constexpr double mu_w = 1.00;
    constexpr double mu_e = 0.30;
    constexpr double inv_2sig2_w = 1.0 / (2.0 * 0.70 * 0.70);
  ...
  void update_enemy_params(const State& prev, const array<int, 8>& selected) {
    ...
    for(int p = 1; p < M; p++) {
      ...
      if(moves.n <= kEnemyUpdateMinMoves) continue;
  ...
  inline double score_like_objective(const State& st) const {
    ...
    return 1e5 * log2(1.0 + s0d / sad);
  }

  double evaluate(const State& st) const {
    return score_like_objective(st);
  }
