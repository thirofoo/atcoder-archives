/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc061/submissions/73399660
 * Submitted at: 2026-02-18 15:04:27
...
static constexpr Params PARAMS_BY_M[9] = {
  {20, 20, 20, 1000, 15, 0.15, -1.0, 0.50, 0.65, -1.0, -1.0, -1.0, -1.0},
  {20, 20, 20, 1000, 15, 0.15, -1.0, 0.50, 0.65, -1.0, -1.0, -1.0, -1.0},
  {20, 20, 20, 1000, 15, 0.15, -1.0, 0.50, 0.65, -1.0, -1.0, -1.0, -1.0},
  {20, 20, 20, 1000, 15, 0.15, -1.0, 0.50, 0.65, -1.0, -1.0, -1.0, -1.0},
  {20, 20, 20, 1000, 15, 0.15, -1.0, 0.50, 0.65, -1.0, -1.0, -1.0, -1.0},
...
struct Solver {
  ...
  pair<int, int> beam_search_decision(const State& root, double turn_end) const {
    ...
    nxt.reserve(max(beam_width, beam_width * beam_width));
    ...
    for(int mv : root_moves) {
      ...
      BeamNode node;
      node.st = root;
      apply_predicted_turn_inplace(node.st, mv);
      node.first_move = mv;
      node.value = evaluate(node.st);
      cur.push_back(move(node));
      if(cur.back().value > best_value || (cur.back().value == best_value && mv < best_move)) {
        best_value = cur.back().value;
        best_move = mv;
      }
    }
    if(cur.empty()) {
    ...
    keep_top_beam(cur, beam_width);

    for(int depth = 2; depth <= max_depth; depth++) {
      if(utility::mytm.elapsed() > turn_end) break;
      nxt.clear();

      for(const BeamNode& parent : cur) {
        if(utility::mytm.elapsed() > turn_end) break;
        vector<int> moves;
        ...
        for(int mv : moves) {
          ...
          child.first_move = parent.first_move;
          ...
          nxt.push_back(move(child));
        }
      }

      if(nxt.empty()) break;
      keep_top_beam(nxt, beam_width);
      cur.swap(nxt);
    }
