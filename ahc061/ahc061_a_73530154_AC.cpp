/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc061/submissions/73530154
 * Submitted at: 2026-02-22 18:14:53
...
struct Solver {
  ...
  void cap_moves_by_heuristic(const State& st, vector<int>& moves, int cap) const {
    ...
    bool use_m2_top_my_bonus = (M == 2);
    if(use_m2_top_my_bonus) mark_enemy_top_reachable_my_cells_m2(st, m2_enemy_top_my_cells);
    array<pair<double, int>, 100> scored;
    int scored_size = 0;
    for(int cell : moves) {
      double h = move_heuristic(st, cell);
      if(use_m2_top_my_bonus && m2_enemy_top_my_cells[cell]) h *= 2.0;
