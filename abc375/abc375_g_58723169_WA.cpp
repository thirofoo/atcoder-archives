/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc375/submissions/58723169
 * Submitted at: 2024-10-12 22:33:22
...
int main() {
  ...
  // 初期コストとして最短経路で使われている辺を使う
  rep(i, m) {
    ll e = perm[i];
    if(used.find(e) != used.end()) uf.merge(a[e], b[e], c[e]);
  }
  dsu uf2(n);
  rep(i, m) {
    ll e = perm[i];
    ...
    if(uf.same(a[e], b[e])) {
      if(uf.diff(a[e], b[e]) == max(uf.weight(a[e]), uf.weight(b[e])) + c[e]) {
        uf.merge(a[e], b[e], c[e]);
        uf2.merge(a[e], b[e]);
      }
    } else {
      uf.merge(a[e], b[e], c[e]);
