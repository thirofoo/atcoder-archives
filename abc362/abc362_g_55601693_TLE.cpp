/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc362/submissions/55601693
 * Submitted at: 2024-07-14 15:44:42
 * Problem URL: https://atcoder.jp/contests/abc362/tasks/abc362_g
 * Result: TLE
 * Execution Time: 3317 ms
 */
...
int main() {
  ...
  ll q;
  cin >> q;
  vector<ll> sa = constract_SA(s);
  rep(i, s.size()) cerr << sa[i] << " ";
  cerr << endl;

  auto check = [&](ll idx, string &t) {
    string substr = s.substr(sa[idx], min(sa[idx] + t.size(), (ull) s.size()));
    return (substr < t);
  };
