/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc362/submissions/55574324
 * Submitted at: 2024-07-13 22:38:49
...
int main() {
    ...
    unordered_map<ull, ll> ans;
    rep(i, q) {
        cin >> t[i];
        rh2[i] = RollingHash(t[i]);
        exist[t[i].size()]++;
        set.insert(rh2[i].get(0, t[i].size()));
    }
    for(int i = 1; i <= s.size(); i++) {
        if(exist[i] == 0) continue;
        rep(j, s.size() - i + 1) {
            if(!set.count(rh.get(j, j + i))) continue;
            ans[rh.get(j, j + i)]++;
        }
    }
