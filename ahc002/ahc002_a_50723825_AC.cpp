/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc002/submissions/50723825
 * Submitted at: 2024-02-29 20:07:55
...
map<char, P> char_to_delta = {{'R', { 0, 1}}, {'D', { 1, 0}}, {'L', { 0,-1}}, {'U', {-1, 0}}};
...
struct Solver{
    ...
    void solve() {
        // 1. dfs で貪欲初期解生成
        // ※ (TIME_LIMIT / 5) ms までで初期解生成
        utility::mytm.CodeStart();
        ans_score = 0;
        for(auto &&[i, j] : group[tile[sx][sy]]) used[i][j] = true;
        firstDFS(sx, sy, 0, TIME_LIMIT / 5);

        if( ans_point.size() <= 2 ) {
            cerr << "Warn: 初期解小さすぎ" << endl;
            return;
        }
        ...
        // 2. 部分破壊 & 再構築 の山登り
        // ※ 一旦計算量爆発しないように矩形範囲内で再構築
        while( utility::mytm.elapsed() < TIME_LIMIT ) {
            // 近傍 1. (l,r] の経路破壊 ⇒ dfs で再構築 ※ 経路長は 5 ~ 10
            int l = rand_int() % ((int)ans_point.size()-1) + 1; // Start 以外
            int r = min(l + rand_int()%5 + 5, (unsigned int)ans_point.size()-1); // End 以外

            int now_score = calcScore(l,r);
            int next_score = query1(l,r);
    ...
    inline void firstDFS(int x, int y, int score, const int &end_time) {
        ...
        back_cnt = now_point.size() / 300; // 人力パラメータ調整
        return;
    }
