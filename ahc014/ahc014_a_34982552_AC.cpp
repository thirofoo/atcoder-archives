/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/34982552
 * Submitted at: 2022-09-19 02:51:30
 * Problem URL: https://atcoder.jp/contests/ahc014/tasks/ahc014_a
 * Result: AC
 * Execution Time: 4942 ms
 */
...
typedef tuple<int, int, T, int, int> T3;
...
inline double temp(int start) {
    ...
    double start_temp = 20,end_temp = 5;
...
int n,m,turn = 0,x2,y2,x3,y3,x4,y4,lx,ly,nd,rd,nrd,start,cnt,total;
double r;
bool f;
...
// 長方形を追加する関数
void createRect(int time){
    int x1,y1; cnt = 0;
    while(time){
        if(cnt >= n*n*3)break;
        x1 = randInt()%n,y1 = randInt()%n;
        if(vertex[x1][y1]){
            cnt++;
            continue;
        }
        ...
        rep(i,8){
            ...
            todo.push(T3((x1-(n-1)/2)*(x1-(n-1)/2)+(y1-(n-1)/2)*(y1-(n-1)/2)+1,i,T(x1,y1,x2,y2,x4,y4,x3,y3),x1,y1));
            f = true;
        }
        if(f)time--;
    }
    ...
    auto [s,dir,p,sx,sy] = todo.top(); todo.pop();
