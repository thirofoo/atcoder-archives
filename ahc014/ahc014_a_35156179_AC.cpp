/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35156179
 * Submitted at: 2022-09-25 16:06:21
 * Problem URL: https://atcoder.jp/contests/ahc014/tasks/ahc014_a
 * Result: AC
 * Execution Time: 4941 ms
 */
...
inline double temp(int start) {
    double start_temp = 10,end_temp = 100;
...
set<P> erasable;
vector<T2> cand;
vector<vector<int>> vertex,first,dig;
...
// 長方形を追加する関数
inline bool createRect(){
    rep(i,8){
        // 候補を格納するpart
        if(!decideVertex(tx,ty,i))continue;
        todo.push(T3(score(x4,y4),T(tx,ty,x2,y2,x4,y4,x3,y3,i)));
    }
...
inline void query1(){
    ...
    while(rnd1--){
        ...
        if(!vertex[tx][ty])rnd2 += createRect();
    }
    if(!rnd2)return;
    nowScore = info.first;
...
inline void query3(){
    ...
    if(nowScore >= preScore - que.size()*(n*n/16)*(TIME_LIMIT-now)/TIME_LIMIT + rnd2*(n*n/16)*(TIME_LIMIT-now)/TIME_LIMIT)return;
    r = prob(preScore - que.size()*(n*n/16)*(TIME_LIMIT-now)/TIME_LIMIT + rnd2*(n*n/16)*(TIME_LIMIT-now)/TIME_LIMIT,nowScore,start);
    if(r <= randDouble()){
