/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35004708
 * Submitted at: 2022-09-20 08:52:25
 * Problem URL: https://atcoder.jp/contests/ahc014/tasks/ahc014_a
 * Result: AC
 * Execution Time: 4905 ms
 */
...
int n,m,turn = 0,x2,y2,x3,y3,x4,y4,lx,ly,nd,rd,nrd,start,now,total,tx2,ty2,tx3,ty3,tmp;
bool f = true;
double r;
...
inline bool checkV(int x,int y,int d){
    lx = 0,ly = 0;
    while(!outField(x+lx,y+ly) && !vertex[x+lx][y+ly]){
        if(Graph[x][y][d])break;
        lx += dx[d]; ly += dy[d];
    }
    if(outField(x+lx,y+ly) || !vertex[x+lx][y+ly])return false;
    return true;
}
...
bool createRect(int x1,int y1){
    rep(i,8){
        ...
        if(!checkV(x1,y1,i))continue;
        x2 = x1+lx, y2 = y1+ly;

        nd = nextDir(i);
        if(!checkV(x1,y1,nd))continue;
        x3 = x1+lx, y3 = y1+ly;
    ...
    rep(i,8){
        ...
        while(!outField(nx1+ddx,ny1+ddy) && !vertex[nx1+ddx][ny1+ddy]){
            if(abs(nx1-(n-1)/2)+abs(ny1-(n-1)/2) > abs(nx1+ddx-(n-1)/2)+abs(ny1+ddx-(n-1)/2)){
                ddx += dx[i]; ddy += dy[i];
                continue;
            }
