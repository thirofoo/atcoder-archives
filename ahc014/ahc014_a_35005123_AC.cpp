/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35005123
 * Submitted at: 2022-09-20 09:51:51
...
inline double temp(int start) {
    ...
    double start_temp = 50,end_temp = 200;
...
int n,m,turn = 0,x2,y2,x3,y3,x4,y4,lx,ly,nd,rd,nrd,start,now,total,tmp,tx,ty,tx2,ty2,tx3,ty3;
bool f = true;
double r;
...
void solve(){
    ...
    while(utility::mytm.elapsed() <= TIME_LIMIT){
        ...
        if(vertex[tx][ty])continue;

        if(!createRect(tx,ty))cnt++;
        else cnt = 0;
    }
