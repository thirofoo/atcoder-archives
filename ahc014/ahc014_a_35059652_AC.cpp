/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35059652
 * Submitted at: 2022-09-23 14:45:16
 * Problem URL: https://atcoder.jp/contests/ahc014/tasks/ahc014_a
 * Result: AC
 * Execution Time: 4926 ms
 */
...
set<P> erasable;
vector<T2> cand;
vector<vector<int>> vertex,first;
...
void eraseRect(){
    ...
    if(tmp1 == 2 && !first[nx2][ny2])erasable.insert(P(nx2,ny2));
    if(tmp2 == 2 && !first[nx3][ny3])erasable.insert(P(nx3,ny3));
    if(tmp3 == 2 && !first[nx4][ny4])erasable.insert(P(nx4,ny4));
}
...
// n個長方形を削除する関数
void query2(){
    if(info.second.empty())return;

    rnd1 = randInt()%(info.second.size()/20+2);
    preScore = info.first;
