/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/34988358
 * Submitted at: 2022-09-19 13:51:10
...
typedef pair<int, T> T3;
typedef tuple<int, int, int> T4;
...
inline double temp(int start) {
    ...
    double start_temp = 20,end_temp = 1000;
...
int n,m,turn = 0,x2,y2,x3,y3,x4,y4,lx,ly,nd,rd,nrd,start,cnt = 0,total,x,y,tmp;
double r;
bool f;
...
void input(){
    ...
    rep(i,n)rep(j,n)outside.push_back(T4(abs(i-(n-1)/2)+abs(j-(n-1)/2),i,j));
    sort(outside.begin(),outside.end());
    reverse(outside.begin(),outside.end());
...
void output(){
    ...
    for(auto [x1,y1,x2,y2,x3,y3,x4,y4,dir]:cand[0].second){
...
inline void eraseGraph(int x,int y,int nx,int ny,int d){
    while(x != nx || y != ny){
        Graph[x][y][d] = false;
        x += dx[d]; y += dy[d];
    }
...
// 長方形を消す関数
void eraseRect(){
    auto [nx1,ny1,nx2,ny2,nx4,ny4,nx3,ny3,dir] = cand[turn].second.back();
    cand[turn].second.pop_back();
    vertex[nx1][ny1] = false;
    cand[turn].first -= (nx1-(n-1)/2)*(nx1-(n-1)/2)+(ny1-(n-1)/2)*(ny1-(n-1)/2)+1;
    nd = nextDir(dir);
    rd = reverseDir(dir);
    nrd = reverseDir(nd);
    eraseGraph(nx1,ny1,nx2,ny2,dir);
    eraseGraph(nx1,ny1,nx3,ny3,nd);
    eraseGraph(nx2,ny2,nx1,ny1,rd);
    eraseGraph(nx2,ny2,nx4,ny4,nd);
    eraseGraph(nx3,ny3,nx1,ny1,nrd);
    eraseGraph(nx3,ny3,nx4,ny4,dir);
    eraseGraph(nx4,ny4,nx2,ny2,nrd);
    eraseGraph(nx4,ny4,nx3,ny3,rd);
}
...
bool createRect(int x1,int y1){
    rep(i,8){
        ...
        todo.push(T3((x1-(n-1)/2)*(x1-(n-1)/2)+(y1-(n-1)/2)*(y1-(n-1)/2)+1-abs(x1-x4)*abs(x1-x4)-abs(y1-y4)*abs(y1-y4)-abs(x3-x2)*abs(x3-x2)-abs(y3-y2)*abs(y3-y2),T(x1,y1,x2,y2,x4,y4,x3,y3,i)));
    }
    ...
    while(true){
        ...
        auto [ns,np] = todo.top();
        r = prob(n*n/32+1,ns,start);
        if(r < 1 && r < randDouble())todo.pop();
        else break;
    }
    auto [s,p] = todo.top();
    cand[turn].first += s;
