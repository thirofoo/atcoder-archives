/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35057360
 * Submitted at: 2022-09-23 11:45:55
 * Problem URL: https://atcoder.jp/contests/ahc014/tasks/ahc014_a
 * Result: RE
 * Execution Time: 4929 ms
 */
...
void query3(){
    ...
    rep(j,info.second.size()){
        ...
        vnum = vertex[nx1][ny1];
        vertex[nx1][ny1] = 0;
        todo2.push(T4(abs(nx1-nx4)+abs(ny1-ny4)+abs(nx2-nx3)+abs(ny2-ny3),vnum,T(nx1,ny1,nx2,ny2,nx4,ny4,nx3,ny3,d)));
        ...
        rep(i,8){
            ...
            todo2.push(T4(abs(nx1-x4)+abs(ny1-y4)+abs(x2-x3)+abs(y2-y3),max({vertex[x2][y2],vertex[x3][y3],vertex[x4][y4]})+1,T(nx1,ny1,x2,y2,x4,y4,x3,y3,i)));
        }
        if(tmp1 == 2 && !first.count(P(nx2,ny2)))erasable.insert(P(nx2,ny2));
        if(tmp2 == 2 && !first.count(P(nx3,ny3)))erasable.insert(P(nx3,ny3));
        if(tmp3 == 2 && !first.count(P(nx4,ny4)))erasable.insert(P(nx4,ny4));

        auto [s,up,p] = todo2.top();
        info.second[j] = p;
        updateInfo(p);
        vertex[nx1][ny1] = up;
        turn++;
