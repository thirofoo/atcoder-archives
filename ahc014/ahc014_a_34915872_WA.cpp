/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/34915872
 * Submitted at: 2022-09-17 19:24:27
...
bool createRect(ll x1,ll y1){
    ...
    rep(i,8){
        ...
        lx = 0,ly = 0;
        while(!outField(x1+lx,y1+ly) && !vertex[x1+lx][y1+ly]){
            if(Graph[x1][y1][nextDir(i)])break;
            lx += dx[nextDir(i)]; ly += dy[nextDir(i)];
        }
        if(outField(x1+lx,y1+ly) || !vertex[x1+lx][y1+ly])continue;
        x3 = x1+lx, y3 = y1+ly;
        // cout << x3 << " " << y3 << " " << flush;
        ...
        while(!outField(tx2,ty2) && !sameLine(tx2,ty2,tx3,ty3,i)){
            if(Graph[tx2][ty2][nextDir(i)])break;
            tx2 += dx[nextDir(i)];
            ty2 += dy[nextDir(i)];
            if(outField(tx2,ty2) || vertex[tx2][ty2])break;
        }
        if(outField(tx2,ty2) || !sameLine(tx2,ty2,tx3,ty3,i))continue;
        // cout << tx2 << " " << ty2 << " " << flush;
    ...
    // cerr << cand[turn].first << " " << (cand[turn].first+s)/(cand[turn].second.size()+1) << endl;
    cand[turn].first += s;
    ...
    tx = nx2,ty = ny2;
    while(tx != nx1 || ty != ny1){
        Graph[tx][ty][reverseDir(dir)] = true;
        tx += dx[reverseDir(dir)];
        ty += dy[reverseDir(dir)];
    }
    tx = nx2,ty = ny2;
    while(tx != nx4 || ty != ny4){
        Graph[tx][ty][nextDir(dir)] = true;
    ...
    tx = nx3,ty = ny3;
    while(tx != nx1 || ty != ny1){
        Graph[tx][ty][reverseDir(nextDir(dir))] = true;
        tx += dx[reverseDir(nextDir(dir))];
        ty += dy[reverseDir(nextDir(dir))];
    }
    tx = nx3,ty = ny3;
    while(tx != nx4 || ty != ny4){
        Graph[tx][ty][dir] = true;
