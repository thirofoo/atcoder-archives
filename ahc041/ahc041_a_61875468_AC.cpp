/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc041/submissions/61875468
 * Submitted at: 2025-01-19 21:37:34
...
int main() {
  ...
  rep(i, H + 1) {
    ...
    rep(j, cand.size()) {
      ...
      if(need_fix) {
        fixed[cand[j]] = i;
        if(i != H) {
          for(auto to: Graph[cand[j]]) {
            if(used[to]) continue;
            kari[to] = max(kari[to], i + 1);
          }
        }
        used[cand[j]] = true;
  ...
  rep(i, N) if(!used[i]) fixed[i] = kari[i];
