/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/masters2026-qual/submissions/73752625
 * Submitted at: 2026-03-01 18:59:34
...
// Fast simulate for SA: no added walls, uses bitmask coverage, early exit on full coverage
int simulate_fast(const int trans[][4], int m, int si, int sj, int sd,
                  uint32_t cov_bits[N]) {
    int total = N * N * 4 * m;
    sim_ts++;
    int ts = sim_ts;
    int ci = si, cj = sj, cd = sd, cs = 0;
    memset(cov_bits, 0, sizeof(uint32_t) * N);
    int cnt = 0;
    int cycle_start = -1;
    // Track route compactly
    static uint16_t route[N * N * 4 * 61 + 1]; // packed (i*20+j)
    int rlen = 0;
    for (int t = 0; t <= total; t++) {
        int key = ((ci * N + cj) * 4 + cd) * m + cs;
        if (sim_vis[key] == ts) {
            // Found cycle - recount coverage from cycle start only
            int ci2 = si, cj2 = sj, cd2 = sd, cs2 = 0;
            for (int k = 0; k < t; k++) {
                int k2 = ((ci2 * N + cj2) * 4 + cd2) * m + cs2;
        ...
        sim_vis[key] = ts;
        route[rlen++] = (uint16_t)(ci * N + cj);
        if (!(cov_bits[ci] & (1u << cj))) {
            cov_bits[ci] |= (1u << cj);
            cnt++;
            if (cnt == N * N) {
                // All cells covered! Fill bool-style for compatibility
                return cnt;
            }
        }
...
// Convert bitmask coverage to bool array for segment functions
inline void bits_to_bool(const uint32_t cov_bits[N], bool cov[N][N]) {
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++)
            cov[i][j] = (cov_bits[i] >> j) & 1;
}

bool build_rect_single_robot_candidate(RobotOut& out, long long& cost_like_current) {
    struct Rule {
        char a0;
        int b0;
        char a1;
    ...
    auto can_move = [&](int r, int c, int dir) {
    ...
    auto neighbors = [&](int r, int c) {
        vector<pair<int, int>> outn;
        if (can_move(r, c, 0)) outn.push_back({0, id(r - 1, c)});
        if (can_move(r, c, 1)) outn.push_back({1, id(r, c + 1)});
        if (can_move(r, c, 2)) outn.push_back({2, id(r + 1, c)});
        if (can_move(r, c, 3)) outn.push_back({3, id(r, c - 1)});
        return outn;
    };
    ...
    auto bfs_path = [&](int src, int dst) {
        ...
        if (prev[dst] == -1) return vector<int>{};
        vector<int> path;
    ...
    auto bfs_path_restricted = [&](int src, int dst, const vector<char>& allowed) {
        if (src == dst) return vector<int>{src};
        if (!allowed[src] || !allowed[dst]) return vector<int>{};
        vector<int> prev(N * N, -1);
        queue<int> q;
        q.push(src);
        prev[src] = src;
    ...
    auto rect_cells_unassigned = [&](const RectBox& bx, const vector<char>& assigned) {
        for (int r = bx.r0; r <= bx.r1; r++)
            for (int c = bx.c0; c <= bx.c1; c++)
                if (assigned[id(r, c)]) return false;
        return true;
    };
    auto rect_fully_open = [&](const RectBox& bx) {
        for (int r = bx.r0; r <= bx.r1; r++)
            for (int c = bx.c0; c < bx.c1; c++)
    ...
    auto build_local_rect_walk = [&](const RectBox& bx) {
        vector<int> order;
        for (int r = bx.r0; r <= bx.r1; r++) {
            int dr = r - bx.r0;
            if ((dr & 1) == 0) {
    ...
    auto build_rect_route_walk = [&](bool& ok) {
        ok = true;
    ...
    auto build_rules_from_walk = [&](const vector<int>& walk) {
        vector<int> move_dirs;
        for (int i = 0; i + 1 < (int)walk.size(); i++) move_dirs.push_back(step_dir(walk[i], walk[i + 1]));
        if (move_dirs.empty()) return pair<vector<Rule>, int>{{{'R', 0, 'R', 0}}, 0};
        ...
        return pair<vector<Rule>, int>{rules, move_dirs[0]};
    };

    auto minimize_rules = [&](const vector<Rule>& rules, int start_state) {
        int n = (int)rules.size();
        if (n == 0) return pair<vector<Rule>, int>{{}, 0};
        vector<int> cls(n, 0);
        {
            map<pair<char, char>, int> mp;
            int nxt = 0;
            for (int i = 0; i < n; i++) {
                auto key = make_pair(rules[i].a0, rules[i].a1);
        ...
        vector<Rule> out_rules(q.size());
        for (int i = 0; i < (int)q.size(); i++) {
            int old = q[i];
            out_rules[i] = {comp[old].a0, remap[comp[old].b0], comp[old].a1, remap[comp[old].b1]};
        }
        return pair<vector<Rule>, int>{out_rules, 0};
    };

    auto simulate_covered = [&](const vector<Rule>& rules, int start_dir) {
    ...
    bool ok = false;
    auto walk = build_rect_route_walk(ok);
    if (!ok || walk.empty()) return false;

    auto [rules, start_dir] = build_rules_from_walk(walk);
    auto [min_rules, min_start] = minimize_rules(rules, 0);
    (void)min_start;
    if (simulate_covered(min_rules, start_dir) != N * N) return false;
    ...
    cost_like_current = (long long)AM * out.m;
    return true;
}
...
int main() {
    ...
    // ============================================================
    // Phase 2: Variable-state automaton SA for Problem C (no walls)
    // AM=1 means states are cheap. AK=1000 means robots are expensive.
    // Strategy: grow automaton up to ~50 states to eliminate fallback robots.
    // ============================================================
    {
        int zigzag_base[4][4] = {
            {1, 1, 1, 1},
        ...
        const int MAX_STATES = 60;
        int aut[MAX_STATES][4];
        for (int restart = 0; restart < 500 && elapsed_ms() < 1950; restart++) {
            int num_states;
            bool zigzag_ext = (restart < 200);  // First 200: zigzag extension mode
            int protected_states = 0;
            // Seed initialization
            if (zigzag_ext) {
                // Zigzag extension: fix S0-S3, add random extension states
                num_states = 4 + rng() % 4;  // 4~7 states to start
                int (*base)[4] = (restart % 2 == 0) ? zigzag_base : zigzag_mirror;
                for (int s = 0; s < 4; s++)
                    memcpy(aut[s], base[s], sizeof(int) * 4);
                protected_states = 4;
                // Initialize extension states
                for (int s = 4; s < num_states; s++) {
                    aut[s][0] = rng() % 3;
                    aut[s][1] = rng() % num_states;
                    aut[s][2] = 1 + rng() % 2;
                    aut[s][3] = rng() % num_states;
                }
                // Redirect some wall-side transitions of S0-S3 to extension states
                for (int i = 0; i < 2 && num_states > 4; i++) {
                    int s = rng() % 4;
                    aut[s][3] = 4 + rng() % (num_states - 4);  // wall-side next_state
                }
            } else {
                // Free SA: random seed
                num_states = 2 + rng() % 8;
            ...
            bool cov[N][N];
            int cover = simulate(aut, num_states, si, sj, sd, cov);
            long long fb = (cover < N * N) ? segment_uncovered_cost(cov) : 0;
            long long total_m = (long long)AM * num_states + fb;
            int best_aut[MAX_STATES][4];
            memcpy(best_aut, aut, sizeof(int) * 4 * num_states);
            int best_ns = num_states;
            int bsi = si, bsj = sj, bsd = sd;
            long long best_total = total_m;
            double temp = 10.0;
            for (int it = 0; it < 10000 && elapsed_ms() < 1950; it++) {
                int old_aut[MAX_STATES][4];
                memcpy(old_aut, aut, sizeof(int) * 4 * num_states);
                int old_ns = num_states;
                int osi = si, osj = sj, osd = sd;
                int mv = rng() % 24;
                if (zigzag_ext) {
                    // Zigzag extension mutations: protect S0..S3 actions
                    if (mv < 6 && num_states > protected_states) {
                        // Mutate an extension state (S4+)
                        int s = protected_states + rng() % (num_states - protected_states);
                        int field = rng() % 4;
                        if (field == 0) aut[s][0] = rng() % 3;
                        else if (field == 1) aut[s][1] = rng() % num_states;
                        else if (field == 2) aut[s][2] = 1 + rng() % 2;
                        else aut[s][3] = rng() % num_states;
                    } else if (mv < 9) {
                        // Redirect a wall-side transition of a core state to an ext state
                        if (num_states > protected_states) {
                            int s = rng() % protected_states;
                            aut[s][3] = protected_states + rng() % (num_states - protected_states);
                        }
                    } else if (mv < 12) {
                        // Change start position
                        si = rng() % N; sj = rng() % N; sd = rng() % 4;
                    } else if (mv < 15 && num_states < MAX_STATES) {
                        // Add extension state
                        int src = rng() % num_states;
                        memcpy(aut[num_states], aut[src], sizeof(int) * 4);
                        int field = rng() % 4;
                        if (field == 0) aut[num_states][0] = rng() % 3;
                        else if (field == 1) aut[num_states][1] = rng() % (num_states + 1);
                        else if (field == 2) aut[num_states][2] = 1 + rng() % 2;
                        else aut[num_states][3] = rng() % (num_states + 1);
                        // Link from a core or ext state
                        int from = rng() % num_states;
                        if (from < protected_states)
                            aut[from][3] = num_states;  // wall-side only for core
                        else {
                            if (rng() % 2) aut[from][1] = num_states;
                            else aut[from][3] = num_states;
                        }
                        num_states++;
                    } else if (mv < 18 && num_states > protected_states + 1) {
                        // Remove an extension state
                        int removed = num_states - 1;
                        int target = (removed > protected_states) ?
                            protected_states + rng() % (removed - protected_states) :
                            rng() % protected_states;
                        for (int s = 0; s < removed; s++) {
                            if (aut[s][1] == removed) aut[s][1] = target;
                            if (aut[s][3] == removed) aut[s][3] = target;
                        }
                        num_states--;
                    } else if (mv < 21 && num_states > protected_states) {
                        // Reinitialize an extension state
                        int s = protected_states + rng() % (num_states - protected_states);
                        aut[s][0] = rng() % 3;
                        aut[s][1] = rng() % num_states;
                        aut[s][2] = 1 + rng() % 2;
                        ...
                        // Redirect transitions among ext states
                        if (num_states > protected_states) {
                            int x = rng() % num_states, y = rng() % num_states;
                            for (int s = protected_states; s < num_states; s++) {
                                if (aut[s][1] == x) aut[s][1] = y;
                                if (aut[s][3] == x) aut[s][3] = y;
                            }
                        }
                    }
                } else {
                    // Free SA mutations (same as before)
                    if (mv < 6) {
                        int s = rng() % num_states, w = rng() % 2;
                        if (w == 0) {
                            if (rng() % 2) aut[s][0] = rng() % 3;
                            else aut[s][1] = rng() % num_states;
                        } else {
                            if (rng() % 2) aut[s][2] = 1 + rng() % 2;
                            else aut[s][3] = rng() % num_states;
                        }
                    } else if (mv < 8) {
                        ...
                        si = rng() % N; sj = rng() % N; sd = rng() % 4;
                    } else if (mv < 13 && num_states >= 2) {
                        int s1 = rng() % num_states, s2 = rng() % num_states;
                        int tmp[4];
                        memcpy(tmp, aut[s1], sizeof(tmp));
                        memcpy(aut[s1], aut[s2], sizeof(tmp));
                        memcpy(aut[s2], tmp, sizeof(tmp));
                        for (int s = 0; s < num_states; s++)
                            for (int idx : {1, 3}) {
                                if (aut[s][idx] == s1) aut[s][idx] = s2;
                                else if (aut[s][idx] == s2) aut[s][idx] = s1;
                            }
                    } else if (mv < 15) {
                        int x = rng() % num_states, y = rng() % num_states;
                        for (int s = 0; s < num_states; s++) {
                    ...
                    } else if (mv < 20 && num_states < MAX_STATES) {
                        int src = rng() % num_states;
                        memcpy(aut[num_states], aut[src], sizeof(int) * 4);
                        int field = rng() % 4;
                        if (field == 0) aut[num_states][0] = rng() % 3;
                        ...
                        int redirect_from = rng() % num_states;
                        if (rng() % 2) aut[redirect_from][1] = num_states;
                        else aut[redirect_from][3] = num_states;
                        num_states++;
                    } else if (mv < 22 && num_states > 2) {
                        int removed = num_states - 1;
                        int target = rng() % removed;
                        for (int s = 0; s < removed; s++) {
                            if (aut[s][1] == removed) aut[s][1] = target;
                            if (aut[s][3] == removed) aut[s][3] = target;
                        }
                ...
                bool nc[N][N];
                int ncover = simulate(aut, num_states, si, sj, sd, nc);
                long long nfb = (ncover < N * N) ? segment_uncovered_cost(nc) : 0;
                long long nt = (long long)AM * num_states + nfb;
                ...
                if (delta < 0 || (rng() % 10000 < 10000 * exp(-delta / temp))) {
                    ...
                    memcpy(aut, old_aut, sizeof(int) * 4 * old_ns);
                    num_states = old_ns;
            ...
            if (best_total < best_M) {
                best_M = best_total;
                bool fcov[N][N];
                simulate(best_aut, best_ns, bsi, bsj, bsd, fcov);
                best_robots.clear();
                RobotOut mr;
                mr.m = best_ns;
                mr.si = bsi; mr.sj = bsj; mr.sd = DIR_CHAR[bsd];
                mr.trans.resize(best_ns);
                for (int s = 0; s < best_ns; s++)
                    mr.trans[s] = {best_aut[s][0], best_aut[s][1], best_aut[s][2], best_aut[s][3]};
                best_robots = {mr};
                vector<RobotOut> fb;
    ...
    {
        RobotOut rect_one;
        long long rect_cost = (long long)4e18;
        if (build_rect_single_robot_candidate(rect_one, rect_cost) && rect_cost < best_M) {
            best_M = rect_cost;
            best_robots.clear();
            best_robots.push_back(rect_one);
        }
    }
    ...
    // No walls for Problem C (AW=1000)
    for (int i = 0; i < N; i++) {
