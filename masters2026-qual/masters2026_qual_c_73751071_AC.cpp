/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/masters2026-qual/submissions/73751071
 * Submitted at: 2026-03-01 18:32:27
...
void run_sa(vector<Robot> state, int& best_V, vector<RobotOut>& best_robots,
    ...
    while (true) {
        ...
        if (is_covered && op < 20 && (int)state.size() > 1) {
            ...
            // M±1
            int new_m = state[ri].m + ((rng() % 2) ? 1 : -1);
            new_m = max(1, min(MAX_M, new_m));
            if (new_m != state[ri].m) {
                int old_m = state[ri].m;
                state[ri].m = new_m;
                for (int s = old_m; s < new_m; s++) {
                    state[ri].trans[s][0] = rng() % 3;
                    state[ri].trans[s][1] = rng() % new_m;
                    state[ri].trans[s][2] = 1 + rng() % 2;
                    state[ri].trans[s][3] = rng() % new_m;
                }
                for (int s = 0; s < new_m; s++) {
                    state[ri].trans[s][1] %= new_m;
                    state[ri].trans[s][3] %= new_m;
...
pair<int, int> cell_rc(int idx) { return {idx / N, idx % N}; }

int can_move_grid(int r, int c, int dir) {
...
int step_dir_cell(int a, int b) {
    auto [r, c] = cell_rc(a);
    auto [nr, nc] = cell_rc(b);
...
vector<pair<int, int>> neighbors_cell(int r, int c) {
    vector<pair<int, int>> out;
    if (can_move_grid(r, c, 0)) out.push_back({0, cell_id(r - 1, c)});
    if (can_move_grid(r, c, 1)) out.push_back({1, cell_id(r, c + 1)});
    if (can_move_grid(r, c, 2)) out.push_back({2, cell_id(r + 1, c)});
    if (can_move_grid(r, c, 3)) out.push_back({3, cell_id(r, c - 1)});
    return out;
}

vector<int> bfs_path_cell(int src, int dst) {
    ...
    while (!q.empty()) {
        int cur = q.front();
        q.pop();
        auto [r, c] = cell_rc(cur);
        for (auto [dir, nxt] : neighbors_cell(r, c)) {
            (void)dir;
...
vector<int> bfs_path_restricted_cell(int src, int dst, const vector<char>& allowed) {
    if (src == dst) return {src};
...
bool rect_cells_unassigned(const RectBox& bx, const vector<char>& assigned) {
    for (int r = bx.r0; r <= bx.r1; r++) {
        for (int c = bx.c0; c <= bx.c1; c++) {
            if (assigned[cell_id(r, c)]) return false;
        }
    }
    return true;
}

bool rect_fully_open(const RectBox& bx) {
    for (int r = bx.r0; r <= bx.r1; r++) {
        for (int c = bx.c0; c < bx.c1; c++) {
...
vector<int> build_local_rect_walk(const RectBox& bx) {
    ...
    for (int r = bx.r0; r <= bx.r1; r++) {
        int dr = r - bx.r0;
        if ((dr & 1) == 0) {
            for (int c = bx.c0; c <= bx.c1; c++) order.push_back(cell_id(r, c));
        } else {
            for (int c = bx.c1; c >= bx.c0; c--) order.push_back(cell_id(r, c));
        }
    }
    ...
    for (int r = bx.r0; r <= bx.r1; r++) {
        for (int c = bx.c0; c <= bx.c1; c++) allowed[cell_id(r, c)] = 1;
    }

    auto back = bfs_path_restricted_cell(order.back(), order[0], allowed);
    if (back.empty()) back = bfs_path_cell(order.back(), order[0]);
    ...
    while (remaining > 0) {
        ...
        for (int r0 = 0; r0 < N; r0++) {
            for (int c0 = 0; c0 < N; c0++) {
                if (assigned[cell_id(r0, c0)]) continue;
        ...
        RectRegion reg;
        reg.entry = cell_id(best.r0, best.c0);
...
vector<int> build_rect_route_walk(bool& ok) {
    ...
    for (int step = 0; step < R; step++) {
        ...
        for (int i = 0; i < R; i++) {
            if (used[i]) continue;
            auto path = bfs_path_cell(cur, regs[i].entry);
    ...
    for (int idx : order) {
        auto path = bfs_path_cell(cur, regs[idx].entry);
        if (path.empty()) {
            ok = false;
            return {};
        }
    ...
    auto back = bfs_path_cell(cur, root);
    if (back.empty()) {
        ok = false;
        return {};
    }
...
pair<vector<OneRule>, int> build_rules_from_walk_one(const vector<int>& walk) {
    vector<int> move_dirs;
    for (int i = 0; i + 1 < (int)walk.size(); i++) move_dirs.push_back(step_dir_cell(walk[i], walk[i + 1]));
    ...
    for (int si = 0; si < (int)segs.size(); si++) {
        ...
        auto [er, ec] = cell_rc(endpoint);
        bool wall_at_end = !can_move_grid(er, ec, dir);
...
pair<vector<OneRule>, int> minimize_rules_one(const vector<OneRule>& rules, int start_state) {
...
int simulate_covered_one(const vector<OneRule>& rules, int start_dir) {
...
OneRobotCandidate build_rect_single_robot_candidate() {
    ...
    auto [rules, start_dir] = build_rules_from_walk_one(walk);
    auto [min_rules, min_start] = minimize_rules_one(rules, 0);
    (void)min_start;

    if (simulate_covered_one(min_rules, start_dir) != N * N) return cand;
