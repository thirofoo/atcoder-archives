/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc323/submissions/46347450
 * Submitted at: 2023-10-07 22:32:20
...
int main() {
    ...
    if( xa == xb && xb == xc ) {
        if( (ya <= yb && yb <= yc) || (yc <= yb && yb <= ya) ) cout << abs(ya-yc) << endl;
        else cout << 2 + abs(ya-yb)+1 + abs(yb-yc) << endl;
        return 0;
    }
    if( ya == yb && yb == yc ) {
        if( (xa <= xb && xb <= xc) || (xc <= xb && xb <= xa) ) cout << abs(xa-xc) << endl;
