/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc147/submissions/50890334
 * Submitted at: 2024-03-04 09:28:51
...
int main() {
    ...
    rep(i,h) rep(j,w) {
        // 縦移動
        if( i != h-1 ) {
            diff = a[i+1][j]-b[i+1][j];
            dp[i+1][j] |= ( diff >= 0 ? (dp[i][j] << diff) : (dp[i][j] >> (-diff)) );

            diff = b[i+1][j]-a[i+1][j];
            dp[i+1][j] |= ( diff >= 0 ? (dp[i][j] << diff) : (dp[i][j] >> (-diff)) );
        }

        // 横移動
        if( j != w-1 ) {
            diff = a[i][j+1]-b[i][j+1];
            dp[i][j+1] |= ( diff >= 0 ? (dp[i][j] << diff) : (dp[i][j] >> (-diff)) );

            diff = b[i][j+1]-a[i][j+1];
            dp[i][j+1] |= ( diff >= 0 ? (dp[i][j] << diff) : (dp[i][j] >> (-diff)) );
        }
    }
    ll ans = 1e16;
