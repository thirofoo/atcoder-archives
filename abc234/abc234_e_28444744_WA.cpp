/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc234/submissions/28444744
 * Submitted at: 2022-01-10 12:07:27
...
int main() {
    ...
    if(x.size() <= 2)cout << x << endl;
        ...
        bool frag = true;
        rep(i,x.size()-1)if(x[i] < x[i+1])frag = false;
        if(frag)rep(i,x.size())cout << x[0];
        else rep(i,x.size())cout << x[0]+1-'0';
        cout << endl;
    }
    else{
        while(true){
