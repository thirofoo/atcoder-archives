/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc385/submissions/60993115
 * Submitted at: 2024-12-21 22:43:44
...
int main() {
  ...
  rep(i, n - 1) {
    ld pre_dx = x[i] - (i == 0 ? 0 : x[i - 1]);
    ld pre_dy = h[i] - (i == 0 ? 0 : h[i - 1]);
    ld dx     = x[i + 1] - x[i];
    ld dy     = h[i + 1] - h[i];
