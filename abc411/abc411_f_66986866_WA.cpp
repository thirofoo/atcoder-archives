/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc411/submissions/66986866
 * Submitted at: 2025-06-22 01:56:50
 * Problem URL: https://atcoder.jp/contests/abc411/tasks/abc411_f
 * Result: WA
 * Execution Time: 846 ms
 */
...
int main() {
  ...
  rep(i, q) {
    ...
    if(g[u_lead].count(v_lead)) {
      ...
      bool flag   = (g[new_lead].size() < g[old_lead].size());
      if(flag) swap(u_lead, v_lead);
      ...
      g[old_lead].clear();
      if(flag) swap(g[u_lead], g[v_lead]);
    }
    cout << ans << '\n';
  }
