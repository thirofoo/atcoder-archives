/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc411/submissions/66987381
 * Submitted at: 2025-06-22 02:52:12
...
int main() {
  ...
  rep(i, q) {
    ...
    if(g[u_lead].count(v_lead)) {
      ...
      if(g[old_lead].count(new_lead)) g[old_lead].erase(new_lead);
      if(g[old_lead].count(old_lead)) g[old_lead].erase(old_lead);

      for(auto to: g[old_lead]) {
        g[to].erase(old_lead);
        if(g[new_lead].count(to)) ans--;
