/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc170/submissions/37268018
 * Submitted at: 2022-12-14 19:09:06
...
int main() {
    ...
    while(q--){
        ll c,d; input(c,d); c--;
        if(!info[school[c]].empty())info[school[c]].erase(info[school[c]].find(rate[c]));
        info[d].insert(rate[c]);

        // segtree更新
        if(!info[school[c]].empty())sg.set(school[c],*info[school[c]].rend());
        else sg.set(school[c],LLONG_MAX);
