/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29493405
 * Submitted at: 2022-02-20 15:11:52
...
void people_action_deciding(){
    rep(i,m){
        ...
        if(i <= 4){
            rep(i,x)tmp += "U";
            if(y >= 4+6*place)rep(i,y-(4+6*place))tmp += "L";
            else rep(i,4+6*place-y)tmp += "R";
            tmp += pdo;
            tmp += "..........";
            ...
            if(!(field[1][1] >= 10 || field[2][1] >= 10 || field[3][1] >= 10 || field[1][30] >= 10 || field[2][30] >= 10 || field[3][30] >= 10 || field[1][2] || field[1][29])){
                ...
                rep(j,m){
                    ...
                    switch(j){
                        ...
                        case 1:player_action[j].insert(cnt+22,go01); break;
                        case 2:player_action[j].insert(cnt+17,go01); break;
                        case 3:player_action[j].insert(cnt+22,go01); break;
                        case 4:player_action[j].insert(cnt+27,go01); break;
                        case 5:player_action[j].insert(cnt+27,go01); break;
                        case 6:player_action[j].insert(cnt+32,go01); break; 
                        case 7:player_action[j].insert(cnt+32,go01); break;
                        case 8:player_action[j].insert(cnt+37,go01); break;
                        case 9:player_action[j].insert(cnt+42,go01); break;
                    }
                }
                F = false;
                letgo = cnt+16;  
