/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29397704
 * Submitted at: 2022-02-18 12:00:16
...
//横一列並べ用の移動文字列
string player1_do = "dRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdRdR";
string player2_do = "dLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdLdL";
 
vector<vector<int>> field(32,vector<int>(32,0));
vector<string> player_action;
vector<vector<P>> pet_count(305,vector<P>(5,P(0,0)));
...
void pet_do(string m,int i){
    ...
    //petが5分割中のどこいるか
    if(x+dx == 30){
        if(y+dy <= 14)pet_count[cnt][4].first++;
        else pet_count[cnt][4].second++;
    }
    else{
        if(y+dy <= 14)pet_count[cnt][(x+dx)/6].first++;
        else pet_count[cnt][(x+dx)/6].second++;
    }
}
    ...
    rep(i,m){
        ...
        if(player_bif1[i] && player_bif2[i] && cnt == 230){
            if(player_bif1[i] == 100)player_bif2[i] = 0;
                ...
                string last = ".";
                if((pet_count[cnt-1][v].first >= pet_count[cnt-1][v].second) && field[x][y-1] == 0){
                    last = "l";
                    player_bif2[i] = 0;
                }
                else if((pet_count[cnt-1][v].first < pet_count[cnt-1][v].second) && field[x][y+1] == 0){
                    last = "r";
                    player_bif2[i] = 0;
                }
                todo += last;
        ...
        if('a' <= action && action <= 'z'){
            ...
            if(cnt >= 90 && action == 'D' && !player_bif1[i]){
                int cnt_up = pet_count[cnt-1][i%4].first +pet_count[cnt-1][i%4].second;
                int cnt_down = pet_count[cnt-1][i%4+1].first +pet_count[cnt-1][i%4+1].second;
                if(cnt_up >= cnt_down){
                    string a = ""; a += "uDD";
                    if(cnt_down >= 2){
                        if(i%2)rep(i,14)a += "R";
