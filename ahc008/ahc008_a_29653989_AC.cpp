/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29653989
 * Submitted at: 2022-02-26 16:36:39
...
string people_do(){
    ...
    rep(i,m){
        ...
        if((cnt >= letgo) && !F && ((pet_cnt[cnt-1][y/3] >= 2) || (pet_cnt[cnt-1][y/3] && cnt >= 260))){
            if(field[x-1][y] == 0){player_action[i].insert(cnt+1,now_action); action = 'u'; already = true;}
        }
        if(cnt >= letgo && !F && ((pet_cnt[cnt-1][11+y/3] >= 2) || (pet_cnt[cnt-1][11+y/3] && cnt >= 260))){
