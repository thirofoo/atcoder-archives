/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29327006
 * Submitted at: 2022-02-14 11:39:17
        ...
        if('a' <= action && action <= 'z'){
            ...
            if(cnt >= 90 && action == 'D' && !player_bif1[i]){
                if(pet_cnt[cnt-1][i%4] >= pet_cnt[cnt-1][i%4+1]){
                    string a = ""; a += "uDD";
                    if(pet_cnt[cnt-1][i%4+1] >= 1){
                        rep(i,15)a += move_left;
                        rep(i,15)a += ".";
                        rep(i,4)a += "uD";
                        if(i%4 == 3)a += "uD";
                        player_bif1[i] = i%4+2;
