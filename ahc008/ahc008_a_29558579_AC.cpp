/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29558579
 * Submitted at: 2022-02-21 11:43:14
...
string people_do(){
    ...
    rep(i,m){
        ...
        if(frag && F){
            if(cnt >= 231 || !(field[1][1] >= 10 || field[2][1] >= 10 || field[3][1] >= 10 || field[1][30] >= 10 || field[2][30] >= 10 || field[3][30] >= 10 || field[1][2] || field[1][29])){
                frag2 = true;
                rep(j,m){
                    ...
                    switch(p_index[j]){
                        ...
                        case 1:player_action[j].insert(cnt+17+(30/m)*1,go01); break;
                        case 2:player_action[j].insert(cnt+17,go23); break;
                        case 3:player_action[j].insert(cnt+17+(30/m)*1,go23); break;
                        case 4:player_action[j].insert(cnt+17+(30/m)*2,go23); break;
                        case 5:player_action[j].insert(cnt+17+(30/m)*2,go01); break;
                        case 6:player_action[j].insert(cnt+17+(30/m)*3,go01); break; 
                        case 7:player_action[j].insert(cnt+17+(30/m)*3,go23); break;
                        case 8:player_action[j].insert(cnt+17+(30/m)*4,go23); break;
                        case 9:player_action[j].insert(cnt+17+(30/m)*5,go23); break;
                    }
                }
                F = false;
                letgo = cnt+16;  
...
int main(){
    ...
    rep(i,m){
        auto [x,y] = people[i];
        people_index.push_back(T(y,x,i));
    }
    sort(people_index.begin(),people_index.end());
    rep(i,m){
        auto [y,x,index] = people_index[i];
        p_index[index] = i;
    }
