/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29298327
 * Submitted at: 2022-02-13 20:11:01
...
// 12*12(左上端っこ専用)
string p1do = "rDrDrDrDrDrDrDrDrDrDrDrdLdLdLdLdLdLdLdLdLdLdRdRdRdRdRdRdRdRdRdRdrUrUrUrUrUrUrUrUrUrUrUrU";
string p2do = "dRdRdRdRdRdRdRdRdRdRdrUrUrUrUrUrUrUrUrUrUrUrUrDrDrDrDrDrDrDrDrDrDrDrdLdLdLdLdLdLdLdLdLdL";
...
void people_action_deciding(){
    rep(i,m){
        ...
        if(i == 0 || i == 1){
            ...
            rep(i,11)tmp += move_right;
            while(tmp.size() <= 300)tmp += p1do;
        }
        else if(i == 2){
            ...
            rep(i,28)tmp += "uR";
            tmp += move_left;
            rep(i,28)if(tmp.size() >= 210)break;else tmp += "uL";
            tmp += move_right;
            rep(i,28)if(tmp.size() >= 210)break;else tmp += "uR";
            rep(i,30)if(tmp.size() >= 210)break;else tmp += move_up;
            while(tmp.size() <= 210)tmp += ".";
            while(tmp.size() <= 240)tmp += move_down;
            while(tmp.size() <= 270)tmp += move_left;
            while(tmp.size() <= 288)tmp += move_up;
            while(tmp.size() <= 300)tmp += place_down;
        }
        else if(i == 3){
            rep(i,30-y)tmp += move_right;
            rep(i,x-1)tmp += move_up;
            tmp += move_down;
            rep(i,28)tmp += "lD";
            tmp += move_up;
            rep(i,28)if(tmp.size() >= 210)break;else tmp += "lU";
            tmp += move_down;
            rep(i,28)if(tmp.size() >= 210)break;else tmp += "lD";
            rep(i,30)if(tmp.size() >= 210)break;else tmp += move_up;
            while(tmp.size() <= 210)tmp += ".";
            while(tmp.size() <= 240)tmp += move_down;
            while(tmp.size() <= 270)tmp += move_left;
