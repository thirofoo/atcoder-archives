/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29654797
 * Submitted at: 2022-02-26 17:07:57
...
string people_do(){
    ...
    rep(i,m){
        ...
        if((cnt >= letgo) && !F && ((pet_cnt[cnt-1][y/3] >= 2) || (pet_cnt[cnt-1][y/3] && cnt >= 255))){
            if(field[x-1][y] == 0){player_action[i].insert(cnt+1,now_action); action = 'u'; already = true;}
        }
        if(cnt >= letgo && !F && ((pet_cnt[cnt-1][11+y/3] >= 2) || (pet_cnt[cnt-1][11+y/3] && cnt >= 255))){
