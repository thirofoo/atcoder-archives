/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc036/submissions/57246811
 * Submitted at: 2024-08-30 17:33:34
 * Problem URL: https://atcoder.jp/contests/ahc036/tasks/ahc036_a
 * Result: AC
 * Execution Time: 1121 ms
 */
...
struct Solver {
  ...
  void createClique() {
    ...
    // 末尾 30 個くらいは A に適切に入れ直してあげる (孤立点である可能性が高いので)
    int replace_num = 30;
    vector<int> back_A, tmp_A;
    for(int i = N - replace_num; i < N; i++) {
      back_A.emplace_back(A[i]);
      A.pop_back();
    }
    used.assign(N, false);
    for(int i = 0; i < N - replace_num; i++) {
      tmp_A.emplace_back(A[i]);
      for(int j = 0; j < replace_num; j++) {
    ...
    for(int i = 0; i < replace_num; i++) if(!used[back_A[i]]) tmp_A.emplace_back(back_A[i]);
    swap(A, tmp_A);
    ...
    iota(priority.begin(), priority.end(), 0);
    sort(priority.begin(), priority.end(), [&](int a, int b) {
      auto &&[ax1, ay1] = place[a];
      auto &&[ax2, ay2] = place[a + 1];
      auto &&[bx1, by1] = place[b];
      auto &&[bx2, by2] = place[b + 1];
