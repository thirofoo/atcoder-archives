/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc011/submissions/32100517
 * Submitted at: 2022-05-30 09:38:08
 * Problem URL: https://atcoder.jp/contests/ahc011/tasks/ahc011_a
 * Result: AC
 * Execution Time: 2954 ms
 */
...
int main() {
    ...
    //初期解作成(?)part
    string ans1 = "",ans2 = "";
    int first = calc_score(ok),second = 0;
    int bx1,by1,bx2,by2;
    vector<vector<vector<bool>>> tmp_ok,first_ok,second_ok;
    int count = 0;
    ...
    while(utility::mytm.elapsed() <= 550){
        ...
        if(score > first){
            second_ok = first_ok; first_ok = tmp_ok;
    ...
    int cnt = t-ans1.size();
    stack<P> handle;
    while(utility::mytm.elapsed() <= 1750 && cnt >= 10){
        count++;
        int now_x = bx1,now_y = by1;
        string tmp = "";
        int pre = -1;
    ...
    //初期解から特定の手数だけ伸ばしてスコア更新part2
    tmp_ok = second_ok;
    cnt = t-ans2.size();
    while(utility::mytm.elapsed() <= 2950 && cnt >= 10){
        count++;
        int now_x = bx2,now_y = by2;
        string tmp = "";
        int pre = -1;
    ...
    cout << (first >= second ? ans1 : ans2) << endl;
