/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc011/submissions/32256922
 * Submitted at: 2022-06-05 09:27:23
...
int n,t,sx,sy,sample_num,turn = 0;
...
int move_check(int d,int pd,int x,int y,vector<vector<vector<bool>>>& tmp_ok){
    ...
    if(d == (pd+2)%4 || nx < 0 || nx >= n || ny < 0 || ny >= n || !tmp_ok[x][y][d] || !tmp_ok[nx][ny][(d+2)%4] || (visited[nx][ny]^(turn%2) && finished[nx][ny]^(turn%2)))return 0;
    
    //閉路が見つかった場合。
    if(visited[nx][ny]^(turn%2) && (!finished[nx][ny])^(turn%2))return -1;
...
void dfs(int* cand,int pd,int x,int y,vector<vector<vector<bool>>>& tmp_ok){
    ...
    rep(i,4){
        ...
        if(now == -1)cycle_flag = true;
            ...
            visited[nx][ny] = !(turn%2);
            dfs(cand,i,nx,ny,tmp_ok);
            finished[nx][ny] = !(turn%2);
        }
    }
...
int calc_score(vector<vector<vector<bool>>>& tmp_ok,bool flag){
    ...
    rep(i,n){
        rep(j,n){
            if(visited[i][j]^(turn%2))continue;
            visited[i][j] = !(turn%2);
...
int main() {
    ...
    while(utility::mytm.elapsed() <= 100){
    ...
    while(utility::mytm.elapsed() <= 2900){
        ...
        if(utility::mytm.elapsed() > 100+2800/sample_num * (cnt+1) && flag){
