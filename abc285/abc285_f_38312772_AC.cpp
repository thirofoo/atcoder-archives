/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc285/submissions/38312772
 * Submitted at: 2023-01-25 13:32:36
 * Problem URL: https://atcoder.jp/contests/abc285/tasks/abc285_f
 * Result: AC
 * Execution Time: 930 ms
 */
...
int main() {
    ...
    while(q--){
        ...
        if(t == 1){
            ...
            rep(i,26){
                if(ar[i]){
                    array<ll,26> tmp = sg.prod(now,now+ar[i]);
                    f &= (ar[i] == tmp[i]);
                    now += ar[i];
                }
                if(!f || now == r)break;
