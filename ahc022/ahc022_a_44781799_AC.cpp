/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc022/submissions/44781799
 * Submitted at: 2023-08-20 14:08:03
 * Problem URL: https://atcoder.jp/contests/ahc022/tasks/ahc022_a
 * Result: AC
 * Execution Time: 3874 ms
 */
    ...
    void arrange() {
        ...
        while( utility::mytm.elapsed() <= TIME_LIMIT ) {
            ...
            for(auto &&[idx,dir]:place_warm_dir[rx][ry]) {
                ...
                // cerr << "Yeaaaaaaah! " << flush;

                // cerr << situation[idx] << " " << total_situ << endl << flush;
                // cerr << dir << endl << flush;

                // out_quantify(situation[idx]);
                // rep(_,DIR_NUM) cerr << ( DIR_NUM-1-dir == _ ? "*" : " " );
                // cerr << endl << flush;
                // cerr << pre_temp << " " << next_temp << endl << flush;
                ...
                // cerr << "Yeaaaaaaah! " << endl << endl << flush;
            }
        ...
        rep(i,l) {
            rep(j,l) {
                ...
                // temp[i][j] = min(1000,max(0,500 + ( temp[i][j] ? 1 : -1 ) * set_cost[sq_s]) );
            }
        }
        ...
        rep(i,n) {
            ...
            while( true ) {
                ...
                rep(_,measure_time) {
                    ...
                    int tmp; cin >> tmp;
                    ave += tmp;
                    // if( tmp < max(0,500 - set_cost[sq_s]) ) ave -= set_cost[sq_s];
                    // if( tmp > min(1000,500 + set_cost[sq_s]) ) ave += set_cost[sq_s];
                    now_measure_time++;
                ...
                // cerr << _ << " " << d << endl << flush;
                // cerr << ave/(width[sq_s]/quantization[sq_s]) << endl << flush;

                warm_bit += (ave/(width[sq_s]/quantization[sq_s])) * rui[d];

                // out_quantify(warm_bit);
                // if( ave >= 500*measure_time ) warm_bit += (1 << d);
        ...
        rep(i,n) {
            ...
            while( true ) {
                ...
                while( abs(one_cnt-zero_cnt) < j_time[sq_s] ) {
                    ...
                    // if( tmp <= 500 - set_cost[sq_s] - judge[sq_s] ) zero_cnt += (1 + (500 - set_cost[sq_s] - judge[sq_s] - tmp) / set_cost[sq_s] );
                    // if( tmp >= 500 + set_cost[sq_s] + judge[sq_s] ) one_cnt += (1 + (tmp - (500 + set_cost[sq_s] + judge[sq_s])) / set_cost[sq_s] );
                    now_measure_time++;
                    if( now_measure_time == MAX_MESURE_TIME ) break;
                }
