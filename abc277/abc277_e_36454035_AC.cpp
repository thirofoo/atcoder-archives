/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc277/submissions/36454035
 * Submitted at: 2022-11-12 23:26:38
...
int main() {
    ...
    todo.push(T(0,0,0));
    while(!todo.empty()){
        auto [now,f,cost] = todo.front(); todo.pop();
        if(dp[f][now] != LLONG_MAX)continue;
        dp[f][now] = cost;

        if(st.count(now))todo.push(T(now,f^1,cost));
        for(auto next:Graph[now]){
            for(auto ok:a[P(now,next)]){
                if(ok^f){
                    todo.push(T(next,f,cost+1));
                }
            }
        }
    }
