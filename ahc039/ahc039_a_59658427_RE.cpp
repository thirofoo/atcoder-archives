/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc039/submissions/59658427
 * Submitted at: 2024-11-10 18:28:25
...
struct Solver {
  ...
  void solve() {
    ...
    vector<int> masu_cnt_list = {5, 10, 20, 30, 40, 50};
    ...
    rep(i, SPACE_LEN_LIST.size()) rep(j, masu_cnt_list.size()) {
      ...
      rep(i, N) {
        auto [x, y] = saba[i];
        // 端の方は面倒くさいので skip
        if(x == GRID_LEN || y == GRID_LEN) continue;
        int nx = x / SPACE_LEN;
        int ny = y / SPACE_LEN;
        saba_cnt[nx][ny]++;
      }
      ...
      while(max_v.size() > masu_cnt || get<0>(max_v.back()) < 0) max_v.pop_back();
