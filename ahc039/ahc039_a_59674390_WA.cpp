/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc039/submissions/59674390
 * Submitted at: 2024-11-11 11:44:21
    ...
    rep(i, SPACE_LEN_LIST.size()) rep(j, masu_cnt_list.size()) {
      ...
      auto dfs = [&](auto &&f, int x, int y, int now_dir) -> void {
        ...
        rep(d, DIR_NUM) {
          ...
          for(auto [tdx, tdy] : {P{0, 0}, P{0, -1}, P{-1, 0}, P{-1, -1}}) {
            if(outField({tx + tdx, ty + tdy}, one_len, one_len)) continue;
            cnt += in_steiner[tx + tdx][ty + tdy];
          }
          if(cnt % 2 == 1) ans.emplace_back(tx * SPACE_LEN, ty * SPACE_LEN);
