/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc039/submissions/59658165
 * Submitted at: 2024-11-10 18:23:22
 * Problem URL: https://atcoder.jp/contests/ahc039/tasks/ahc039_a
 * Result: RE
 * Execution Time: 80 ms
 */

#include <atcoder/all>
#include <bits/stdc++.h>
using namespace std;
using namespace atcoder;
#define rep(i, n) for (int i = 0; i < n; i++)

namespace utility {
struct timer {
  chrono::system_clock::time_point start;
  // 開始時間を記録
  void CodeStart() { start = chrono::system_clock::now(); }
  // 経過時間 (ms) を返す
  double elapsed() const {
    using namespace std::chrono;
    return (double)duration_cast<milliseconds>(system_clock::now() - start)
        .count();
  }
} mytm;
} // namespace utility

inline unsigned int rand_int() {
  static unsigned int tx = 123456789, ty = 362436069, tz = 521288629,
                      tw = 88675123;
  unsigned int tt = (tx ^ (tx << 11));
  tx = ty, ty = tz, tz = tw;
  return (tw = (tw ^ (tw >> 19)) ^ (tt ^ (tt >> 8)));
}

inline double rand_double() { return (double)(rand_int() % (int)1e9) / 1e9; }

// 温度関数
#define TIME_LIMIT 2950
inline double temp(double start) {
  double start_temp = 100, end_temp = 1;
  return start_temp + (end_temp - start_temp) *
                          ((utility::mytm.elapsed() - start) / TIME_LIMIT);
}

// 焼きなましの採用確率
inline double prob(int best, int now, int start) {
  return exp((double)(now - best) / temp(start));
}

//-----------------以下から実装部分-----------------//

using P = pair<int, int>;
const int GRID_LEN = 100000;
const int SPACE_LEN = 5000; // 1 マスの辺の長さ

// right | down | left | up
#define DIR_NUM 4
vector<int> dx = {0, 1, 0, -1};
vector<int> dy = {1, 0, -1, 0};

inline bool outField(pair<int, int> now, int h, int w) {
  auto &&[x, y] = now;
  if (0 <= x && x < h && 0 <= y && y < w)
    return false;
  return true;
}

struct Solver {
  int N, one_len;
  vector<P> saba, iwashi, ans;
  vector<vector<int>> saba_cnt, iwashi_cnt;

  Solver() {
    this->input();
    one_len = GRID_LEN / SPACE_LEN;
    saba_cnt.resize(one_len, vector<int>(one_len, 0));
    iwashi_cnt.resize(one_len, vector<int>(one_len, 0));

    // grid をリサイズする感じ
    rep(i, N) {
      auto [x, y] = saba[i];
      // 端の方は面倒くさいので skip
      if(x == SPACE_LEN || y == SPACE_LEN) continue;
      int nx = x / SPACE_LEN;
      int ny = y / SPACE_LEN;
      saba_cnt[nx][ny]++;
    }
    rep(i, N) {
      auto [x, y] = iwashi[i];
      // 端の方は面倒くさいので skip
      if(x == SPACE_LEN || y == SPACE_LEN) continue;
      int nx = x / SPACE_LEN;
      int ny = y / SPACE_LEN;
      iwashi_cnt[nx][ny]++;
    }
    return;
  }

  void input() {
    cin >> N;
    rep(i, N) {
      int x, y;
      cin >> x >> y;
      saba.push_back({x, y});
    }
    rep(i, N) {
      int x, y;
      cin >> x >> y;
      iwashi.push_back({x, y});
    }
    return;
  }

  void output() {
    cout << ans.size() << endl;
    for (auto &&[x, y] : ans) {
      cout << x << " " << y << endl;
    }
    return;
  }

  void solve() {
    // ==== 貪欲解法 =====
    // saba - iwashi が多い 10 マスを接続するシュタイナー木が答え
    // ※ prim 法ベースでシュタイナー木を作成する
    using T = tuple<int, int, int>;

    // 1. 多い 10 マスを探す & グラフ作成
    vector<T> max_v;
    vector graph(one_len, vector(one_len, vector<T>{}));
    rep(i, one_len) rep(j, one_len) {
      max_v.emplace_back(iwashi_cnt[i][j] - saba_cnt[i][j] + N, i, j);
      rep(d, DIR_NUM) {
        int nx = i + dx[d], ny = j + dy[d];
        if (outField({nx, ny}, one_len, one_len)) continue;
        graph[nx][ny].emplace_back(iwashi_cnt[i][j] - saba_cnt[i][j] + N, i, j);
      }
    }

    int masu_cnt = 10;
    sort(max_v.begin(), max_v.end());
    cerr << max_v.size() << endl;
    while(max_v.size() > masu_cnt) max_v.pop_back();

    // 2. シュタイナー木を作成
    priority_queue<T, vector<T>, greater<T>> pq;
    vector<vector<bool>> in_steiner(one_len, vector<bool>(one_len, false));
    vector<vector<int>> dist;
    vector<vector<P>> prev;

    cerr << "Yeah!" << endl;
    auto [_, x1, y1] = max_v[0];
    in_steiner[x1][y1] = true;

    for (int i = 1; i < masu_cnt; i++) {
      // cerr << i << endl;
      // dijkstra 法で最短経路復元
      auto [_, x, y] = max_v[i];
      pq.push({0, x, y});
      dist.assign(one_len, vector<int>(one_len, 1e9));
      prev.assign(one_len, vector<P>(one_len, {-1, -1}));
      dist[x][y] = 0;

      int hx = -1, hy = -1;
      while (!pq.empty()) {
        auto [cost, x, y] = pq.top();
        pq.pop();
        if(in_steiner[x][y]) {
          hx = x, hy = y;
          break;
        }
        if(dist[x][y] < cost) continue;
        // cerr << x << " " << y << " " << cost << endl;
        for(auto [c, nx, ny] : graph[x][y]) {
          if(dist[nx][ny] <= cost + c) continue;
          dist[nx][ny] = cost + c;
          prev[nx][ny] = {x, y};
          pq.push({cost + c, nx, ny});
        }
      }
      while (!pq.empty()) pq.pop();
      // cerr << "Yeah!" << endl;

      // 経路復元
      while(hx != -1) {
        in_steiner[hx][hy] = true;
        auto [px, py] = prev[hx][hy];
        // cerr << hx << " " << hy << endl;
        hx = px, hy = py;
      }
    }
    cerr << "Yeah!" << endl;

    // 3. シュタイナー木の頂点をうまく出力
    vector<vector<int>> deg(one_len, vector<int>(one_len, 0));
    int sx = -1, sy = -1;
    rep(i, one_len) {
      rep(j, one_len) {
        if(!in_steiner[i][j]) continue;
        rep(d, DIR_NUM) {
          int nx = i + dx[d], ny = j + dy[d];
          if (outField({nx, ny}, one_len, one_len)) continue;
          if(in_steiner[nx][ny]) deg[i][j]++;
        }
      }
    }
    rep(i, one_len) {
      rep(j, one_len) {
        if(!in_steiner[i][j]) continue;
        if(deg[i][j] == 1) {
          sx = i, sy = j;
          break;
        }
      }
      if(sx != -1) break;
    }

    cerr << "Yeaaaaaah!" << endl;
    vector<vector<P>> push_perm = {
      {{0, 0}, {0, 1}, {1, 1}, {1, 0}}, // right
      {{0, 1}, {1, 1}, {1, 0}, {0, 0}}, // down
      {{1, 1}, {1, 0}, {0, 0}, {0, 1}}, // left
      {{1, 0}, {0, 0}, {0, 1}, {1, 1}} // up
    };
    // dfs の行きがけで ans を作成
    vector<vector<bool>> visited(one_len, vector<bool>(one_len, false));
    auto dfs = [&](auto &&f, int x, int y, int now_dir) -> void {
      visited[x][y] = true;
      rep(d, DIR_NUM) {
        int next_dir = (now_dir + 3 + d) % DIR_NUM;
        auto [ndx, ndy] = push_perm[now_dir][d];
        ans.emplace_back((x + ndx) * SPACE_LEN + (ndx == 1 ? -1 : 0), (y + ndy) * SPACE_LEN + (ndy == 1 ? -1 : 0));
        
        int nx = x + dx[next_dir], ny = y + dy[next_dir];
        if(outField({nx, ny}, one_len, one_len)) continue;
        if(visited[nx][ny]) continue;
        if(!in_steiner[nx][ny]) continue;
        f(f, nx, ny, next_dir);
      }
      return;
    };
    dfs(dfs, sx, sy, 0);
    vector<P> final_ans;
    rep(i, ans.size()) {
      if(ans[i] == ans[(i + 1) % ans.size()]) continue;
      final_ans.emplace_back(ans[i]);
    }
    swap(ans, final_ans);

    // 角だけが必要なので必要な頂点だけを抽出
    // final_ans.clear();
    // rep(i, ans.size()) {
    //   auto [x1, y1] = ans[(i - 1 + ans.size()) % ans.size()];
    //   auto [x2, y2] = ans[i];
    //   auto [x3, y3] = ans[(i + 1) % ans.size()];
    //   if(x1 == x2 && x2 == x3) continue;
    //   if(y1 == y2 && y2 == y3) continue;
    //   final_ans.emplace_back(x2, y2);
    // }
    // swap(ans, final_ans);

    rep(i, one_len) {
      rep(j, one_len) {
        if(in_steiner[i][j]) cerr << "o";
        else cerr << ".";
      }
      cerr << endl;
    }

    return;
  }
};

int main() {
  cin.tie(0);
  ios_base::sync_with_stdio(false);

  Solver solver;
  solver.solve();
  solver.output();

  return 0;
}