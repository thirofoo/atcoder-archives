/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc377/submissions/59187783
 * Submitted at: 2024-10-26 22:39:43
...
int main() {
  ...
  rep(i, n) {
    ...
    ll size = cycle.size() - 1;

    ll to = 2, cnt = 1, tmp = ((k & 1) ? 2 : 1);
    while((1LL << cnt) <= k) {
      if(k & (1LL << cnt)) tmp = (tmp * to) % size;
      to = (to * to) % size;
      cnt++;
    }
    ll rest = (tmp - 1 + size) % size;
