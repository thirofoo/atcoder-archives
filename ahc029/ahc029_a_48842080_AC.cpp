/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48842080
 * Submitted at: 2023-12-25 19:09:51
...
struct State{
    ...
    inline void bestWorstUpdate() {
        ...
        sort(sorted_project.begin(), sorted_project.end(), [&](const auto &a, const auto &b) {
            return (a.first.v+pow(2,L+6)) * b.first.h > (b.first.v+pow(2,L+6)) * a.first.h;
        });
        return;
    }
    ...
    inline pair<int,int> selectUseCard() {
        ...
        if( hand[op].t == 0 ) {
            for(int i=0; i<M; i++) {
                if( sorted_project[i].first.v > hand[op].p ) {
