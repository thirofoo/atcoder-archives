/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48846122
 * Submitted at: 2023-12-25 22:47:56
...
struct State{
    ...
    inline pair<int,int> selectUseCard() {
        ...
        for(int j=0; j<N; j++) {
            ...
            if( hand[j].t == 2 && sorted_project.back().first.v < 1.02 * sorted_project.back().first.h ) {
                op = j;
                break;
            }
    ...
    inline int selectBuyCard(int turn, const vector<Card> &cand_card) {
        ...
        // ※ ただし最後の 10 ターンは 0枚目のカードしか買わない
        if( turn >= T - 10 ) return 0;
        ...
        for(int j=0; j<K; j++) {
            ...
            else if( cand_card[j].t == 4 && turn < T - 150 && L < 20 ) {
                // 増資カードが購入可能 ⇒ 最優先で買う
                // ※ ただし最後の 150 ターンは買わない
