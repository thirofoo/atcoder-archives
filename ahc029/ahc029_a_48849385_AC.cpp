/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48849385
 * Submitted at: 2023-12-26 02:48:46
 * Problem URL: https://atcoder.jp/contests/ahc029/tasks/ahc029_a
 * Result: AC
 * Execution Time: 1625 ms
 */
...
struct State{
    ...
    int money = 0, recent_use_hand_id, L = 0, pro_cnt;
    double expectation;
    ...
    int simulate(int turn, int card_id, const vector<Card> &first_card, const vector<vector<Card>> &tester, const vector<Project> &pro_tester) {
        f = false;
        ...
        for(int i=turn+1; i<T; i++) {
            ...
            // if( turn == 0 ) {
            //     cerr << "Cand Card: \n" << flush;
            //     for(int j=0; j<tester[i].size(); j++) cerr << tester[i][j].t << " " << tester[i][j].w * pow(2,L) << " " << tester[i][j].p * pow(2,L) << "\n" << flush;
            //     cerr << "Buy card: " << card_id << '\n' << flush;
            //     cerr << "Last hand: \n" << flush;
            //     for(int j=0; j<hand.size(); j++) cerr << hand[j].t << " " << hand[j].w << " " << hand[j].p << "\n" << flush;
            //     cerr << "Last Project: \n" << flush;
            //     for(int j=0; j<project.size(); j++) cerr << project[j].h << " " << project[j].v << "\n" << flush;
            //     cerr << "Having Money: " << money << "\n" << flush;
            //     cerr << "\n\n" << flush;
            // }
        }
...
struct Solver {
    ...
    void solve(){
        ...
        for(int turn=0; turn<T; turn++) {
            ...
            // Card 購入 part
            int card_id;
            if( turn <= T-150 || turn >= T-50 ) card_id = state.selectBuyCard(turn, cand_card);
            else card_id = monte_carlo_method(turn);
            // auto card_id = state.selectBuyCard(turn, cand_card);// 貪欲ver
            // auto card_id = monte_carlo_method(turn);
            cout << card_id << '\n' << flush;
