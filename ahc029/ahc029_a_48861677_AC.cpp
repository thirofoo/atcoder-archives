/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48861677
 * Submitted at: 2023-12-26 19:22:33
 * Problem URL: https://atcoder.jp/contests/ahc029/tasks/ahc029_a
 * Result: AC
 * Execution Time: 1711 ms
 */
...
struct State{
    ...
    inline pair<int,int> selectUseCard() {
        // ~~~~~~~~~~~~~~~~~~~~~~~ 貪欲で使用カード・宛先選択 ~~~~~~~~~~~~~~~~~~~~~~~ //
        long long op = 0, best_card_expectation = 0;
        for(int j=0; j<N; j++) {
            // 増資カード : 最優先で使用
            if( hand[j].t == 4 ) {
                op = j;
            ...
            // 労働カード : 労働力が高いものを使用
            long long expectation = hand[j].w * (hand[j].t == 1 ? M : 1);
    ...
    inline int selectBuyCard(int turn, const vector<Card> &cand_card) {
        ...
        int best_card = 0;
        long long best_price = 1e16, semi_price = 1e16;
