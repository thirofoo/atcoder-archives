/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48857496
 * Submitted at: 2023-12-26 15:57:01
 * Problem URL: https://atcoder.jp/contests/ahc029/tasks/ahc029_a
 * Result: WA
 * Execution Time: 1530 ms
 */
...
struct Solver {
    ...
    inline int monte_carlo_method(int turn) {
        ...
        while (utility::mytm.elapsed() < end_time && counter) {
            for(int i=0; i<K; i++) {
                if( state.money < 1.5 * cand_card[i].p || (cand_card[i].t == 4 && turn >= T - 50) ) continue;
