/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48846772
 * Submitted at: 2023-12-25 23:17:57
...
struct State{
        ...
        if( hand[card_id].t == 0 ) { // 通常労働カード
            ...
            for(int i=0; i<M; i++) {
                ...
                if( project[i].h <= 0 ) {
                    ...
                    if( is_test ) project[i] = pro_tester[pro_cnt++];
                }
            }
            ...
            for(int i=0; i<M; i++) project[i] = pro_tester[pro_cnt++];
            bestWorstUpdate();
        }
    ...
    inline int selectBuyCard(int turn, const vector<Card> &cand_card) {
        ...
        for(int j=0; j<K; j++) {
            ...
            else if( cand_card[j].t == 2 && cand_card[j].p <= 8*pow(2,L) && card_cnt[0]+card_cnt[1] != 0 && turn < T - 50 ) {
