/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48853917
 * Submitted at: 2023-12-26 12:36:18
 * Problem URL: https://atcoder.jp/contests/ahc029/tasks/ahc029_a
 * Result: AC
 * Execution Time: 1599 ms
 */
...
struct Solver {
    ...
    void genRandomTest() {
        // 乱択 test 生成
        project_tester.assign(T,{}); // Project は使いまわし
    ...
    inline int monte_carlo_method(int turn) {
        ...
        while (utility::mytm.elapsed() < end_time && counter) {
            for(int i=0; i<K; i++) {
                ...
                State branch = state;
                branch.is_test = true;
                int test_score = branch.simulate(turn, i, cand_card, card_tester[counter], project_tester);
                scores[i] += test_score;
                // cerr << test_score << " ";
            }
            counter--;
