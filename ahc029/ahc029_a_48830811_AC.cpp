/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48830811
 * Submitted at: 2023-12-25 00:47:35
...
struct State{
    ...
    inline pair<int,int> selectUseCard() {
        ...
        for(int j=0; j<N; j++) {
            ...
            if( hand[j].t == 2 && worst_expectation < 1.0 ) {
                op = j;
                break;
            }
    ...
    inline int selectBuyCard(int turn, const vector<Card> &cand_card) {
        ...
        for(int j=0; j<K; j++) {
            ...
            else if( cand_card[j].t == 2 && cand_card[j].p <= 5*pow(2,L) && card_cnt[0]+card_cnt[1] != 0 ) {
