/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48834543
 * Submitted at: 2023-12-25 10:47:02
...
struct State{
    ...
    inline void bestWorstUpdate() {
        best_expectation = -1e16, worst_expectation = 1e16;
        for(int i=0; i<M; i++) {
            expectation = (double)project[i].v / project[i].h;
            // Best : v - h が最大
            if( best_expectation < project[i].v - project[i].h ) {
                best_expectation = project[i].v - project[i].h;
                best_project = i;
            }
            // Worst : v / h が最小
