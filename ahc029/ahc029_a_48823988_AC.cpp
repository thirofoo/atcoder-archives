/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48823988
 * Submitted at: 2023-12-24 19:30:08
...
struct Solver {
    ...
    inline void usingCard() {
        // project : v/h が一番高いものを選択
        // card : w-p が一番高いものを選択

        int op1 = 0, best_project = 0, worst_project = 0;
        double best_expectation = 0, worst_expectation = 1e16;

        // project の期待値が最大・最小のものを探索
        for(int j=0; j<M; j++) {
            double expectation = (double)(state.project[j].v+pow(2,state.L+5)) / state.project[j].h;
            // v/h が最大のものを選択
            if( best_expectation < expectation ) {
                best_expectation = expectation;
                best_project = j;
        ...
        for(int j=0; j<N; j++) {
            if( state.hand[j].t <= 1 ) {
                if( best_expectation < state.hand[j].w * (state.hand[j].t == 1 ? M : 1) - (state.hand[j].p+1) ) {
                    op1 = j;
                    best_expectation = state.hand[j].w * (state.hand[j].t == 1 ? M : 1) - (state.hand[j].p+1);
                }
            }
            else if( state.hand[j].t == 4 ) {
                op1 = j;
                break;
            }
        }
        int sub = ( state.hand[op1].t == 0 ? best_project : ( state.hand[op1].t == 2 ? worst_project : 0 ) );
        cout << op1 << " " << sub << "\n" << flush;
        state.useCard(op1, sub);
    }

    inline void buyingCard(int i) {
        // カード購入は貪欲
        ...
        for(int j=0; j<K; j++) {
            ...
            if( cand_card[j].t == 0 || cand_card[j].t == 1 ) {
                // 労働カードは前半は労働力が高いものを選ぶ ( 実は期待値が高いものは効率悪い )
                if( i <= T - 100 ) {
            ...
            else if( cand_card[j].t == 4 && i < T - 200 ) {
                // 増資カードは購入可能なら 100% 買うべき
                // ただし、増資カードは最後の 200 ターンは買わない
