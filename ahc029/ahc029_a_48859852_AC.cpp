/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc029/submissions/48859852
 * Submitted at: 2023-12-26 17:52:52
 * Problem URL: https://atcoder.jp/contests/ahc029/tasks/ahc029_a
 * Result: AC
 * Execution Time: 1610 ms
 */
...
struct State{
    ...
    int recent_use_hand_id, L = 0, pro_cnt, now_turn = 0;
    double expectation;
    long long money = 0;
    ...
    inline void bestWorstUpdate() {
        ...
        sort(sorted_project.begin(), sorted_project.end(), [&](const auto &a, const auto &b) {
            return (a.first.v+5*r2[L+(T-now_turn)/200]) * b.first.h > (b.first.v+5*r2[L+(T-now_turn)/200]) * a.first.h;
        });
        return;
    }
    ...
    inline pair<int,int> selectUseCard() {
        ...
        for(int j=0; j<N; j++) {
            ...
            if( hand[j].t == 2 && (double)sorted_project.back().first.v < (card_cnt[2] == min(N-1,3) ? 1.00 : 0.90) * sorted_project.back().first.h ) {
                op = j;
                break;
            }
