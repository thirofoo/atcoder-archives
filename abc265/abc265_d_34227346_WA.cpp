/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc265/submissions/34227346
 * Submitted at: 2022-08-21 21:41:50
...
int main() {
    ...
    for(int x=1;x<=n;x++){
        ...
        if(i1 != a.end() && *i1 == a[x]+p){
            auto i2 = lower_bound(a.begin()+x,a.end(),a[x]+p+q);
            if(i2 != a.end() && *i2 == a[x]+p+q){
                auto i3 = lower_bound(a.begin()+x,a.end(),a[x]+p+q+r);
