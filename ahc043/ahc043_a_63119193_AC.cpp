/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc043/submissions/63119193
 * Submitted at: 2025-02-24 17:30:31
 * Problem URL: https://atcoder.jp/contests/ahc043/tasks/ahc043_a
 * Result: AC
 * Execution Time: 2802 ms
 */
...
struct Solver {
  ...
  void solve() {   
      ...
      for(auto &delta1 : delta_less2) {
        ...
        for(auto &delta2 : delta_less2) {
          ...
          for(auto &delta : delta_less2) {
            ...
            if(!out_field(next)) visited[next.x * N + next.y] = current_time;
            next = next2 + delta;
            if(!out_field(next)) visited[next.x * N + next.y] = current_time;
          }
          int cand_income = 0;
          for(auto &delta : delta_less2) {
            ...
            if(!out_field(next)) {
              ...
              for(auto &p : another_side[next.x][next.y]) {
                if(visited[p.x * N + p.y] != current_time) continue;
    ...
    while((elapsed_time = utility::mytm.elapsed()) < TIME_LIMIT) {
      ...
      if(!updated) {
        ...
        else swap(station_places[e1], next);
  ...
  inline bool calc_route(Point start) {
    current_time++;
    visited[start.x * N + start.y] = current_time;
    // キューを再初期化
    que = queue<Point>();
    que.push(start);
    ...
    int length = 0;
    for(Point cur = goal;; cur = prev[cur.x][cur.y]) {
      length++;
      if(cur == start) break;
    }
