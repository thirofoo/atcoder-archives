/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/arc157/submissions/39201522
 * Submitted at: 2023-02-25 23:37:46
...
int main() {
    ...
    rep(i,n){
        if(f && s[i] == 'Y'){
            todo.push(T(i-left+ (left == 0 ? 100000000 : 0),left,i));
            f = false;
        }
        if(!f && s[i] == 'X'){
            f = true;
    ...
    if(f)todo.push(T(INT_MAX,left,n));
    while(!todo.empty()){
