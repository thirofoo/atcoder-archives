/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc132/submissions/43731870
 * Submitted at: 2023-07-19 01:06:08
...
int main() {
    ...
    while(!todo.empty()){
        ...
        if( time%3 == 2 && visited[now] != 1e16 ) continue;
        if( time%3 == 2 ) visited[now] = time;
        time++;
        for(auto next: Graph[now]){
            if( time%3 == 2 && visited[next] != 1e16 ) continue;
