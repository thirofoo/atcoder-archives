/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc270/submissions/35149360
 * Submitted at: 2022-09-25 09:43:44
...
int main() {
    ...
    if(uf1.size(0) == n+2)ans = min(tmp,ans);
    tmp = 0;
    while(!todo2.empty()){
    ...
    if(uf2.size(n+1) == n+1)ans = min(tmp,ans);
    tmp = 0;
    while(!todo.empty()){
        auto [c,at,to] = todo.top();
        todo2.push(todo.top()); todo.pop();
    ...
    if(uf3.size(n) == n+1)ans = min(tmp,ans);
    tmp = 0;
    while(!todo2.empty()){
    ...
    if(uf4.size(0) == n)ans = min(tmp,ans);
    print(ans);
