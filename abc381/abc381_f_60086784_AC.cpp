/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc381/submissions/60086784
 * Submitted at: 2024-11-22 22:40:32
...
int main() {
  ...
  rep(i, n) for(auto [now, val]: dp[i]) {
    ...
    rep(j, 20) {
      ...
      auto itr = lower_bound(idx[j].begin(), idx[j].end(), i);
      itr++;
      ll next = now | (1LL << j);
      if(min_idx[next] <= (*itr) + 1) continue;
      min_idx[next] = (*itr) + 1;
  ...
  ll ans = 0;
  rep(i, n + 1) for(auto [now, val]: dp[i]) ans = max(ans, val);
  cout << ans << '\n';
