/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/arc030/submissions/30605023
 * Submitted at: 2022-04-01 17:52:45
...
int main() {
    ...
    rep(i,size){
        string tmp = "";
        int a = min((int)str[i].size(),k);
        rep(j,a){
            tmp += str[i][j];
            if(dp[j+1][i] == "0")dp[j+1][i] = tmp;
            else dp[j+1][i] = min(dp[j+1][i],tmp);
        }
    }
    ...
    for(int i=1;i<k;i++){
        rep(j,size){
            ...
            for(auto l:graph[j]){
                ...
                string tmp = dp[i][j];
                if(dp[tmp.size()][l] == "0")dp[tmp.size()][l] = tmp;
                else dp[tmp.size()][l] = min(dp[tmp.size()][l],tmp);
