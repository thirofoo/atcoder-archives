/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc027/submissions/48356101
 * Submitted at: 2023-12-10 16:18:10
 * Problem URL: https://atcoder.jp/contests/ahc027/tasks/ahc027_a
 * Result: AC
 * Execution Time: 1903 ms
 */
...
struct Solver{
    int n, xl, yl, dr, dc, nx, ny, d_total, rect_cnt, rect_idx, turn, purpose, high_roop;
    ...
    void solve(){
        ...
        while( utility::mytm.elapsed() <= TIME_LIMIT ) {
            int rnd_high = rand_int()%400 + 10;
            int rnd_high_T = rand_int()%400 + 100;
            roop_roop = rand_int()%3 + 1;
    ...
    inline void ansPush(int tx, int ty, int high_t, int high_v) {
        ...
        if( flag && purpose <= ans.size() ) {
            flag = false;
            high.assign({});
            high_idx.assign(n+2,vector<int>(n+2,-1));
            for(int i=1; i<=n; i++) for(int j=1; j<=n; j++) {
                if( d[i][j] >= ( (high_roop%roop_roop + 1) == 1 ? high_v : ((high_roop%roop_roop + 1) == 2 ? sqrt(high_v) : cbrt(high_v)) ) ) {
                    high_idx[i][j] = high.size();
                    high.emplace_back(P(i,j));
                }
            }
            droppingByHigh(tx,ty,high_t,( (high_roop%roop_roop + 1) == 1 ? high_v : ((high_roop%roop_roop + 1) == 2 ? sqrt(high_v) : cbrt(high_v)) ));
