/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc027/submissions/48409010
 * Submitted at: 2023-12-10 18:49:26
 * Problem URL: https://atcoder.jp/contests/ahc027/tasks/ahc027_b
 * Result: WA
 * Execution Time: 1874 ms
 */
...
struct Solver{
    ...
    void solve(){
        while( utility::mytm.elapsed() <= TIME_LIMIT ) {
            ...
            high_t = rand_int()%HIGH_T + HIGH_T/2;
            greedy();
        }
    ...
    void greedy() {
        ...
        if( ans.size() <= 100000 ) cand.emplace_back(pair(best_score / (max((long long)ans.size()-1,1ll)), ans));   
    }
