/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc116/submissions/44183012
 * Submitted at: 2023-08-02 19:02:02
...
int main() {
    ...
    priority_queue<P,vector<P>,greater<P>> multi_used;
    rep(i,n) {
        ll t, d; cin >> t >> d;
        no_used.push(P(d,t));
    }
    ...
    set<ll> st;
    rep(i,k) {
        auto [d, t] = no_used.top();
        if( st.count(t) ) multi_used.push(no_used.top());
        else st.insert(t);
        now += d;
        no_used.pop();
    }
    ans = now + st.size()*st.size();
    while( !no_used.empty() && !multi_used.empty() ) {
        auto [d, t] = no_used.top(); no_used.pop();
        if( st.count(t) ) continue;
        st.insert(t);
        now -= multi_used.top().first;
        now += d;
