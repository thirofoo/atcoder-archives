/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc059/submissions/72330068
 * Submitted at: 2026-01-10 18:36:20
 * Problem URL: https://atcoder.jp/contests/ahc059/tasks/ahc059_a
 * Result: AC
...
// dist をテーブル化して定数倍を削る。
static int gN = 0;
static vector<uint16_t> dist_table;
inline int dist(const Pos &a, const Pos &b) {
    if (!dist_table.empty()) {
        int idx1 = a.r * gN + a.c;
        int idx2 = b.r * gN + b.c;
        return (int)dist_table[(size_t)idx1 * (size_t)(gN * gN) + (size_t)idx2];
    }
...
static void init_dist_table(int N) {
    gN = N;
    int S = N * N;
    dist_table.resize((size_t)S * (size_t)S);
    for (int i = 0; i < S; ++i) {
        int r1 = i / N;
        int c1 = i % N;
        for (int j = 0; j < S; ++j) {
            int r2 = j / N;
            int c2 = j % N;
            dist_table[(size_t)i * (size_t)S + (size_t)j] = (uint16_t)(abs(r1 - r2) + abs(c1 - c2));
        }
    }
}
...
static int recompute_path(const Tree &tr, const vector<array<Pos, 2>> &pos,
                          vector<array<long long, 2>> &cost, int v,
                          vector<CostBackup> *backup, vector<int> *mark, int mark_id) {
    int last = -1;
    while (v != tr.root) {
        last = v;
        if (backup && mark) {
            if ((*mark)[v] != mark_id) {
                (*mark)[v] = mark_id;
                backup->push_back(CostBackup{v, cost[v][0], cost[v][1]});
            }
        }
...
static int recompute_path(const Tree &tr, const vector<array<Pos, 2>> &pos,
                          vector<array<long long, 2>> &cost, int v) {
    return recompute_path(tr, pos, cost, v, nullptr, nullptr, 0);
}
...
int main(int argc, char **argv) {
    ...
    const double T0 = 1.5;
    const double T1 = 0.4;
    const double LOG_RATIO = log(T1 / T0);
    ...
    while (timer.sec() < TIME_LIMIT) {
        ...
        int top1 = -1;
        int top2 = -1;
        if (p1 != -1) top1 = recompute_path(cur, pos, cost, p1, &backup, &mark, mark_id);
        if (p2 != -1 && p2 != p1) top2 = recompute_path(cur, pos, cost, p2, &backup, &mark, mark_id);
        ...
        if (cand_obj <= cur_obj) {
            ...
            double temp = T0 * exp(LOG_RATIO * t);
