/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc010/submissions/31232396
 * Submitted at: 2022-04-24 18:25:59
...
void to_memo(){
    rep(i,30)rep(j,30)update(i,j);
}

ll CalcScore(){
    ll add = 0,length1 = 0,length2 = 0;
    ...
    return length1*length2*100000 + add;
}

ll CalcOne(int i,int j){
    ll add = 0,length = 0;
    rep(k,4){
        int sx = i,sy = j,sd = k,tmp_l = 0,x = i,y = j,d = k;
        bool frag = false;
...
int main() {
    ...
    while(utility::mytm.elapsed() <= 1950){
        ...
        r_time[ni][nj]++;
        r_time[ni][nj] %= 4;
        ll p_score = CalcOne(ni,nj);
        update(ni,nj);
        ll l_score = CalcOne(ni,nj);
        // cout << p_score << " " << l_score << "  ";

        // best_score += CalcDiff();
        if(l_score > p_score){
            best_str[ni*30+nj] = to_string(r_time[ni][nj])[0];
            best_score += l_score - p_score;
        }
    }
    cout << best_str << endl;
