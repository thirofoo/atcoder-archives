/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc397/submissions/65159873
 * Submitted at: 2025-04-24 15:46:24
...
int main() {
  ...
  rep(i, n - 1) {
    st.insert(a[i]);
    if(prev[a[i]] != -1) seg.range_add(prev[a[i]] + 1, i + 1, 1);
    ans        = max(ans, (ll) st.size() + seg.range_max(0, i + 1) + r_variety[i + 1]);
    prev[a[i]] = i;
  }
  cout << ans << endl;
