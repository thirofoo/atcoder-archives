/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc045/submissions/64598321
 * Submitted at: 2025-04-07 01:38:31
...
// Helper function to compute accumulated probability for a fixed p1 from this->existProb
	inline double computeAccumulatedProb(int x1, int y1,
										 const vector<T>& r2Prob,
										 const vector<T>& r3Prob) {
		// Build V2: (distance squared from p1 to p2, prob2)
		vector<pair<long long, double>> V2;
		V2.reserve(r2Prob.size());
		for (const auto& [x2, y2, prob2] : r2Prob) {
			long long d12_sq = (long long)(x1 - x2) * (x1 - x2) + (long long)(y1 - y2) * (y1 - y2);
			V2.emplace_back(d12_sq, prob2);
		}

		// Build V3: (distance squared from p1 to p3, prob3) and compute total probability for r3
		vector<pair<long long, double>> V3;
		V3.reserve(r3Prob.size());
		double totalProb3 = 0.0;
		for (const auto& [x3, y3, prob3] : r3Prob) {
			long long d13_sq = (long long)(x1 - x3) * (x1 - x3) + (long long)(y1 - y3) * (y1 - y3);
			V3.emplace_back(d13_sq, prob3);
			totalProb3 += prob3;
		}

		// Sort V3 by the distance squared
		sort(V3.begin(), V3.end());
		// Build cumulative probability array for V3
		vector<double> cumProb(V3.size(), 0.0);
		if (!V3.empty()) {
			cumProb[0] = V3[0].second;
			for (size_t k = 1; k < V3.size(); ++k)
				cumProb[k] = cumProb[k - 1] + V3[k].second;
		}

		// For each entry in V2, add corresponding weighted probability from V3
		double accumulated = 0.0;
		for (const auto& [d12_sq, prob2] : V2) {
			// Find first element in V3 where d13_sq >= d12_sq
			auto it = lower_bound(V3.begin(), V3.end(), make_pair(d12_sq, -1.0));
			int idx = distance(V3.begin(), it);
			double probCond = totalProb3 - (idx > 0 ? cumProb[idx - 1] : 0.0);
			accumulated += prob2 * probCond;
		}
		return accumulated;
	}

// Helper function to normalize and update a probability distribution vector
	inline void normalizeDistribution(vector<T>& distribution) {
		double tot = 0.0;
		for (const auto& [x, y, prob] : distribution)
			tot += prob;
		if (tot < 1e-12) return;
		vector<T> updated;
		updated.reserve(distribution.size());
		for (const auto& [x, y, prob] : distribution) {
			double normProb = prob / tot;
			if (normProb > 1e-12)
				updated.emplace_back(x, y, normProb);
		}
		distribution = move(updated);
	}

	inline void updateProb(Rect &r2, Rect &r3) { // 引数は非const参照
		// Keep original distributions for reference
		const auto& r1Orig = this->existProb;
		const auto& r2Orig = r2.existProb;
		const auto& r3Orig = r3.existProb;

		// If any distribution is empty, do nothing.
		if (r1Orig.empty() || r2Orig.empty() || r3Orig.empty())
			return;

		// --- Phase 1: Update this->existProb ---
		map<Point, double> newProbMap;
		double totalNewProb_r1 = 0.0;
		for (const auto& [x1, y1, prob1] : r1Orig) {
			double accProb = computeAccumulatedProb(x1, y1, r2Orig, r3Orig);
			double finalProb = prob1 * accProb;
			if (finalProb > 1e-12) {
				newProbMap[Point(x1, y1)] += finalProb;
				totalNewProb_r1 += finalProb;
			}
		}
		this->existProb.clear();
		if (totalNewProb_r1 >= 1e-12) {
			this->existProb.reserve(newProbMap.size());
			for (const auto& [pt, prob] : newProbMap)
				this->existProb.emplace_back(pt.x, pt.y, prob / totalNewProb_r1);
		}

		// --- Phase 2: Update r2.existProb ---
		newProb.clear();
		double totalNewProb = 0.0;
		// Use updated this->existProb and original r3 distribution
		for (const auto& [x2, y2, prob2] : r2.existProb) {
			double condProb = 0.0;
			for (const auto& [x1, y1, prob1] : this->existProb)
				for (const auto& [x3, y3, prob3] : r3Orig) {
			...
			double updatedVal = prob2 * condProb;
			if (updatedVal > 1e-12) {
				newProb.emplace_back(x2, y2, updatedVal);
				totalNewProb += updatedVal;
			}
		}
		if (totalNewProb >= 1e-12) {
			r2.existProb = newProb;
			normalizeDistribution(r2.existProb);
		}

		// --- Phase 3: Update r3.existProb ---
		newProb.clear();
		totalNewProb = 0.0;
		for (const auto& [x3, y3, prob3] : r3.existProb) {
			...
			double updatedVal = prob3 * condProb;
			if (updatedVal > 1e-12) {
				newProb.emplace_back(x3, y3, updatedVal);
				totalNewProb += updatedVal;
			}
		}
		if (totalNewProb >= 1e-12) {
			r3.existProb = newProb;
			normalizeDistribution(r3.existProb);
		}
		return;
	}
};
	...
	void solve() {
		...
		while (utility::timer.elapsed() < TIME_LIMIT - 200 && iterations < 5) {
			...
			if (candCost < bestCost) {
				bestGroup = candGroup;
				bestCost = candCost;
				cerr << "Found better group with cost: " << bestCost << '\n';
			}
			cerr << '\n';
			iterations++;
		}
