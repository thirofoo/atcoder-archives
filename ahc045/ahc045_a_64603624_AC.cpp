/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc045/submissions/64603624
 * Submitted at: 2025-04-07 12:54:34
...
struct Solver {
	...
	vector<T> newProb;
	double prob_threshold = 1e-3; // 高速化のために 1e-3 程度の確率の事象は無視
	inline void updateProb(Rect &r1, Rect &r2, Rect &r3) {
		...
		if (totalNewProb >= prob_threshold) {
			r1.existProb.clear();
