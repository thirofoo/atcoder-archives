/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc045/submissions/64579340
 * Submitted at: 2025-04-06 14:19:11
		...
		for (const auto &[x1, y1, prob1] : existProb) {
			...
			for (const auto &[x2, y2, prob2] : r2.existProb)
				for (const auto &[x3, y3, prob3] : r3.existProb) {
					int d1 = (int)(x1 - x2) * (x1 - x2) + (int)(y1 - y2) * (y1 - y2);
					int d2 = (int)(x1 - x3) * (x1 - x3) + (int)(y1 - y3) * (y1 - y3);
					if (d1 <= d2)
...
struct Solver {
	...
	vector<int> selectGroup() {
		...
		rep(j, N) {
			...
			int dsq = sqrt((expCore.x - expOther.x) * (expCore.x - expOther.x) + (expCore.y - expOther.y) * (expCore.y - expOther.y));
	...
	vector<vector<int>> buildGroups() {
		...
		if (N > 0) {
			...
			for (int j = 1; j < M; ++j) {
				...
				for (int i = 0; i < N; ++i) {
					int dx = expectations[i].x - centroids[j - 1].x;
					int dy = expectations[i].y - centroids[j - 1].y;
		...
		for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {
			...
			for (int i = 0; i < N; ++i) {
				for (int j = 0; j < M; ++j) {
					if (G[j] == 0) continue;
					int dx = expectations[i].x - centroids[j].x;
					int dy = expectations[i].y - centroids[j].y;
			...
			cerr << "Iteration: " << iter << ", Flow: " << result.first << ", Cost: " << result.second << "\n";
			if (result.second == prev_cost) break;
			prev_cost = result.second;
		...
		for (auto& group : groupsAssign) {
			if (group.size() > 1)
				sort(group.begin(), group.end(), [&](int a, int b) {
					...
					return p_a.x != p_b.x ? p_a.x < p_b.x : p_a.y < p_b.y;
	...
	vector<P> buildMSTWithDivination(const vector<int> &group) {
		...
		rep(i, group.size()) rep(j, group.size()) {
			...
			int da = sqrt((ax - bx) * (ax - bx) + (ay - by) * (ay - by));
		...
		sort(totalEdges.begin(), totalEdges.end(), [&](const P &a, const P &b) {
			...
			int da = sqrt((ax - bx) * (ax - bx) + (ay - by) * (ay - by));
			int db = sqrt((cx - dx) * (cx - dx) + (cy - dy) * (cy - dy));
			if(L == 3) {
				da += sqrt(rects[a.second].getVariance());
				db += sqrt(rects[b.second].getVariance());
			}
			return da < db;
		});
		vector<P> mstEdges;
	...
	vector<P> buildMSTWithExpectation(const vector<int>& group) {
		...
		vector<tuple<int, int, int>> potential_edges;
		potential_edges.reserve(n * (n - 1) / 2);
