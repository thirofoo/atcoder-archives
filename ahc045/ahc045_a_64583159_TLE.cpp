/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc045/submissions/64583159
 * Submitted at: 2025-04-06 16:37:48
...
struct Solver {
	...
	vector<int> selectGroup() {
		...
		rep(j, N) {
			...
			dsq += sqrt(rects[j].getVariance()) * (double)(Q - divinationCount) / Q;
		...
		for (int iter = 0; iter < FIXED_ITERATIONS; iter++) {
			...
			for (int i = 0; i < N; i++) 
				for (int j = 0; j < M; j++) {
					...
					// 補正項は分散が大きい頂点は出来るだけサイズが小さいグループに割り当てるようにする目的
					long long cost = dx * dx + dy * dy + alpha * sqrt(rects[i].getVariance()) * G[j];
