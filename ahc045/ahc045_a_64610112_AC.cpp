/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc045/submissions/64610112
 * Submitted at: 2025-04-07 17:48:59
...
struct Solver {
	...
	inline vector<int> selectGroup() {
		...
		rep(j, N) varianceScores.emplace_back(sqrt(rects[j].getVariance()) - 100 * (used_cnt[j] + 1), j);
		...
		rep(iter, FIXED_ITERATIONS) {
			...
			rep(i, N) {
				...
				rep(j, M) {
					...
					double variance_factor = 1.0 + beta * sqrt(max(0.0, variances[i]));
          long long cost = max(0LL, (long long)round((double)dist_sq * variance_factor));
