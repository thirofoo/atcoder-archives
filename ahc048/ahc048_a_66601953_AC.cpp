/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc048/submissions/66601953
 * Submitted at: 2025-06-08 16:59:43
 * Problem URL: https://atcoder.jp/contests/ahc048/tasks/ahc048_a
 * Result: AC
 * Execution Time: 2906 ms
 */
...
MixABCResult findBestABCmix(
  const Color& X, const Color& A,
  int ti, int pr, int pc, int SQ, int W,
  const vector<DIR>& pathDir,
  const vector<Color>& tileCol)
{
  constexpr array<array<int,2>,4> pairs{{{0,1},{1,2},{2,3},{3,0}}};

  MixABCResult best{{-1,-1},0,0,0,1e30,{DIR::NONE,DIR::NONE,DIR::NONE}};

  for(auto p : pairs){
    array<int,2> nb{-1,-1}; bool ok=true;
    for(int k=0;k<2;++k){
      int dir=p[k];
      int nr=pr+delta[dir].first, nc=pc+delta[dir].second;
    ...
    for(int dA=1; dA<=W; ++dA)
      for(int dB=1; dB<=W; ++dB)
        for(int dC=1; dC<=W; ++dC){
          if(dA+dB+dC < W) continue;
          double den = double(dA+dB+dC);
          double wA=dA/den, wB=dB/den, wC=dC/den;

          Color mix{
            A.c*wA + B.c*wB + C.c*wC,
          ...
          if(err < best.err){
            best.err  = err;
            best.nti  = {nb[0],nb[1]};
            best.divA = dA; best.divB = dB; best.divC = dC;
               ...
               const vector<Color>& targets)
{
	...
	iota(palette.begin(), palette.end(), 0);   // 0,1,…,k-1
	vector<Color> initCol(PathNum);

	int ptr = 0;
	shuffle(palette.begin(), palette.end(), rng);   // 1 回目のシャッフル
	for (int p = 0; p < PathNum; ++p) {
	  if (ptr == k) {                              // 使い切ったら新たにシャッフル
	    shuffle(palette.begin(), palette.end(), rng);
	    ptr = 0;
	  }
	  int colIdx = palette[ptr++];
	  cmdAdd(paths[p][0], colIdx, cmds);           // ← 既存の出力ユーティリティ
	...
	for (int turn = 0; turn < maxTurns; turn++) {
		...
		for (int ti = 0; ti < PathNum; ti++) {
			...
			MixABCResult mixABC = findBestABCmix(
			  X, root.tileCol[ti], ti, pr, pc,
			  sqrtPathNum, 4,
			  root.PathDir, root.tileCol);
			int necOpABC = 12 + (mixABC.dirs[0]!=DIR::NONE?2:0) + (mixABC.dirs[1]!=DIR::NONE?2:0) + (mixABC.dirs[2]!=DIR::NONE?2:0);
			if (mixABC.nti[0] >= 0 && mixABC.nti[1] >= 0 &&
			   t - root.op_cnt > 10 * (maxTurns-turn)+necOpABC &&
			   root.score + llround(mixABC.err*1e4) < bestScore) {
			  bestTrace = Trace{'C', {ti, mixABC.nti[0], mixABC.nti[1]}, {mixABC.divA, mixABC.divB, mixABC.divC}, -1, root.id,{mixABC.dirs[0], mixABC.dirs[1], mixABC.dirs[2]}};
		...
		if (bestTrace.op == 'A') {
		...
		} else if(bestTrace.op=='C') {
		  int vA=bestTrace.divIdxs[0];
		  int vB=bestTrace.divIdxs[1];
		  int vC=bestTrace.divIdxs[2];
		  int tA=bestTrace.tileIdxs[0];
		  int tB=bestTrace.tileIdxs[1];
		  int tC=bestTrace.tileIdxs[2];
		  ...
		  root.score = bestScore;
		  root.op_cnt += 6;
		  for(auto d:bestTrace.tileDirIdxs)
		    if(d!=DIR::NONE) root.op_cnt += 2;
		
		  /* ------- コマンド出力（向き変更・仕切り開閉のみ） ------- */
		  // 向き変更
		  for(int idx=0; idx<3; ++idx){
		    if(bestTrace.tileDirIdxs[idx]==DIR::NONE) continue;
		    int tIdx = bestTrace.tileIdxs[idx];
		    cmdToggle(paths[tIdx][0], paths[tIdx].back(), cmds);
		    while(root.PathDir[tIdx]!=bestTrace.tileDirIdxs[idx]){
			...
			reverse(paths[bestTrace.tileIdxs[1]].begin(), paths[bestTrace.tileIdxs[1]].end());
			bool toggle_flag1 = (bestTrace.divIdxs[0] != tilePerPath);
			bool toggle_flag2 = (bestTrace.divIdxs[1] != tilePerPath);
			bool toggle_flag3 = (bestTrace.divIdxs[2] != tilePerPath);
			if (toggle_flag1) root.op_cnt += 2;
			if (toggle_flag2) root.op_cnt += 2;
			...
			auto p3 = paths[bestTrace.tileIdxs[1]][bestTrace.divIdxs[1] - 1];
			auto p4 = (toggle_flag2 ? paths[bestTrace.tileIdxs[1]][bestTrace.divIdxs[1]] : P(0, 0));
			auto p5 = paths[bestTrace.tileIdxs[2]][bestTrace.divIdxs[2] - 1];
			auto p6 = (toggle_flag3 ? paths[bestTrace.tileIdxs[2]][bestTrace.divIdxs[2]] : P(0, 0));
			auto p7 = paths[bestTrace.tileIdxs[0]][0];
			auto p8 = paths[bestTrace.tileIdxs[1]][0];
			auto p9 = paths[bestTrace.tileIdxs[2]][0];
	...
	if (d < Dthreshold) return {root.score + (PathNum + maxTurns - h) * d, cmds};
