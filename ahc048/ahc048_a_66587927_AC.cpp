/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc048/submissions/66587927
 * Submitted at: 2025-06-08 00:18:04
               ...
               const vector<Color>& targets)
{
	...
	State root;
	root.score = 0; root.depth = 0; root.id = 0, root.op_cnt = PathNum * (d < 400 ? 1 : 2);
	...
	for (int turn = 0; turn < maxTurns; turn++) {
		...
		for (int ti = 0; ti < PathNum; ti++) {
			...
			int necOp = 4 + (mix3.dirs[0] != DIR::NONE ? 2 : 0) + (mix3.dirs[1] != DIR::NONE ? 2 : 0);
			if (mix3.nti >= 0
