/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc048/submissions/66599306
 * Submitted at: 2025-06-08 15:04:56
               ...
               const vector<Color>& targets)
{
	...
	for (int turn = 0; turn < maxTurns; turn++) {
		...
		for (int ti = 0; ti < PathNum; ti++) {
			...
			if (mix3.nti >= 0
				&& t - root.op_cnt > 6 * (maxTurns - turn) + necOp
		...
		} else if(bestTrace.op=='C'){          // A+B+C 混色
		  int vA=bestTrace.divIdxs[0];
		  int vB=bestTrace.divIdxs[1];
	...
	if (d < Dthreshold) return {root.score + (maxTurns + PathNum - h) * d, cmds};
	...
	rep(i, rest) final_score += llround(cost[i][tileOfTar[i]] * 10000.0);

	for (int r = 0; r < rest; ++r) {
	  int tIdx = tileOfTar[r];
	  if (tIdx == -1) continue;
	  cmdTake(paths[tIdx][0], cmds);
	}
