/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc048/submissions/66616461
 * Submitted at: 2025-06-09 11:34:59
...
/* === 4-mix :  A + B + C + Tube(W)  ===================== */
inline MixABTResult findBestABTmix(
  const Color& X,const Color& A,
  int ti,int pr,int pc,int SQ,int W,
  const vector<Color>& tubes,
  const vector<DIR>&   pathDir,
  const vector<Color>& tileCol)
{
  /* 方向ペア 6 通り（時計回りで L 字も含む）*/
  constexpr array<array<int,2>,4> PAIR{{
    {0,1},{1,2},{2,3},{3,0}
  }};

  MixABTResult best{{-1,-1},0,0,0,0,1e30,
                    {DIR::NONE,DIR::NONE,DIR::NONE}};
  ...
  for(auto p:PAIR){
    ...
    bool ok=true;
    for(int k=0;k<2;++k){
      int nr=pr+delta[p[k]].first, nc=pc+delta[p[k]].second;
      if(outField({nr,nc},SQ,SQ)){ ok=false; break; }
      nb[k]=nr*SQ+nc;
    }
    if(!ok) continue;
    ...
    for(int a=1;a<=W;++a)
      for(int b=1;b<=W;++b)
        for(int c=1;c<=W;++c){
          /* tube 分割は固定 W */
          double inv=1.0/(a+b+c+W);
          double wA=a*inv, wB=b*inv, wC=c*inv, wT=W*inv;

          double baseC=A.c*wA + B.c*wB + C.c*wC;
          double baseM=A.m*wA + B.m*wB + C.m*wC;
          double baseY=A.y*wA + B.y*wB + C.y*wC;
               ...
               const vector<Color>& targets)
{
	...
	for (int i = 0; i < PathNum; i++) {
		...
		paths[i] = snake(r, r + (n / SQ) - 1, c, c + (n / SQ) - 1, (r + c) % 2);
	}
	...
	for (int turn = 0; turn < maxTurns; turn++) {
		...
		for (int ti = 0; ti < PathNum; ti++) {
			...
			Mix3Result mix3 = findBest3mix(X, root.tileCol[ti], ti, pr, pc, SQ, pathLen, tubes, root.pathDir, root.tileCol);
			...
			MixABCResult mixABC = findBestABCmix(X, root.tileCol[ti], ti, pr, pc, SQ, pathLen, root.pathDir, root.tileCol);
			...
			/* === 4-mix (A+B+C+Tube) === */
			MixABTResult mixABT=findBestABTmix(X, root.tileCol[ti], ti, pr, pc, SQ, pathLen, tubes, root.pathDir, root.tileCol);
			int necOpABT = 12 + 2 * (mixABT.dirs[0]!=DIR::NONE) + 2 * (mixABT.dirs[1]!=DIR::NONE) + 2 * (mixABT.dirs[2]!=DIR::NONE);
			if(mixABT.nti[0] >= 0 && t-root.op_cnt > 10 * (maxTurns-turn)+necOpABT && root.score + llround(mixABT.err * 1e4) < bestScore){
			  bestT = Trace{'D', {ti, mixABT.nti[0], mixABT.nti[1]}, {mixABT.dA, mixABT.dB, mixABT.dC}, mixABT.ci, root.id, {mixABT.dirs[0], mixABT.dirs[1], mixABT.dirs[2]}};
			  bestScore = root.score + llround(mixABT.err * 1e4);
			}
		}
		...
		if (bestT.op == 'A') {
		  ...
		  int vA=bestT.divs[0], vB=bestT.divs[1], vC=bestT.divs[2];
		  int tA=bestT.tiles[0], tB=bestT.tiles[1], tC=bestT.tiles[2];
		  int tubeIdx = bestT.colorIdx;
		  /* ── 新しい色 ── */
		  Color super = (root.tileCol[tA] * vA + root.tileCol[tB] * vB +
		                 root.tileCol[tC] * vC + tubes[tubeIdx] * pathLen) /
		                double(vA + vB + vC + pathLen);
			
		  auto apply=[&](int tid,int v){
		    root.tileCol[tid]=(root.tileCol[tid] * (pathLen-v) + super * v) / double(pathLen);
		  };
		  apply(tA,vA); apply(tB,vB); apply(tC,vC);
		  root.score = bestScore;
		
		  /* op_cnt & 向き変更 (3 本) */
		  root.op_cnt += 6;
		  rep(i,3) if(bestT.dirs[i]!=DIR::NONE){
		    root.op_cnt+=2;
		    cmdToggle(paths[bestT.tiles[i]][0],paths[bestT.tiles[i]].back(),cmds);
		    while(root.pathDir[bestT.tiles[i]]!=bestT.dirs[i]){
		      vector<P> nxt(pathLen);
		      rep(j,pathLen) nxt[j]=paths[bestT.tiles[i]][(j+1)%pathLen];
		      paths[bestT.tiles[i]]=move(nxt);
		      root.pathDir[bestT.tiles[i]] = DIR((int(root.pathDir[bestT.tiles[i]])+1)%DIR_NUM);
		    }
		    cmdToggle(paths[bestT.tiles[i]][0],paths[bestT.tiles[i]].back(),cmds);
		  }
		  ...
		  /* 仕切り開閉 & take/add */
		  reverse(paths[tB].begin(), paths[tB].end());
		  bool tgA = vA != pathLen, tgB = vB != pathLen, tgC = vC != pathLen;
		  root.op_cnt += 2 + 2 * (tgA + tgB + tgC);
		
		  auto pA1 = paths[tA][vA - 1], pA2=tgA ? paths[tA][vA] : P(0,0);
		  auto pB1 = paths[tB][vB - 1], pB2=tgB ? paths[tB][vB] : P(0,0);
		  auto pC1 = paths[tC][vC - 1], pC2=tgC ? paths[tC][vC] : P(0,0);
		  auto s1 = paths[tA][0];
			auto s2 = paths[tB][0];
			auto s3 = paths[tC][0];
		
		  if(tgA) cmdToggle(pA1, pA2, cmds);
		  if(tgB) cmdToggle(pB1, pB2, cmds);
		  if(tgC) cmdToggle(pC1, pC2, cmds);
		  cmdToggle(s1, s2, cmds);
		  cmdToggle(s1, s3, cmds);
		  cmdAdd(s1, tubeIdx, cmds);   // 1 滴
		  cmdTake(s1, cmds);          // 取り出し
			cmdToggle(s1, s2, cmds);
		  cmdToggle(s1, s3, cmds);
		  if(tgA) cmdToggle(pA1, pA2, cmds);
		  if(tgB) cmdToggle(pB1, pB2, cmds);
	...
	for (int trial = 0; trial < 5 && utility::mytm.elapsed() < TIME_LIMIT; ++trial) {
  	root        = baseRoot;
  	paths       = basePaths;
  	cost        = baseCost;
  	tarOfTile   = baseTar;
