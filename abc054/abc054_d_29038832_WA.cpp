/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc054/submissions/29038832
 * Submitted at: 2022-02-04 17:20:11
...
int main() {
            ...
            for(int j=1;j<=400;j++){
                if(i >= a && j >= b && dp[i-a][j-b].second != -1 && dp[i-a][j-b].second != k){
                    if(dp[i][j].first == -1)dp[i][j] = P(dp[i-a][j-b].first+c,k);
                        ...
                        int tmp = min(dp[i-a][j-b].first+c,dp[i][j].first);
                        dp[i][j] =P(tmp,k);
                    }
                }
            }
    ...
    for(int i=1;i<=400;i++){
        ...
        if(dp[i*ma][i*mb].first != -1){
            return cout << dp[i*ma][i*mb].first << endl,0;
        }
    }
    cout << -1 << endl;
    return 0;
