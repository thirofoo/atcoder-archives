/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc025/submissions/46849648
 * Submitted at: 2023-10-22 15:37:07
...
struct Solver {
    ...
    inline void generateRest(vector<int>& rest) {
        ...
        if( right.size() > d && state.q >= 8*n ) generateRest(right);
        else group.emplace_back(right);
        group.emplace_back(middle);
        if( left.size() > d && state.q >= 8*n ) generateRest(left);
