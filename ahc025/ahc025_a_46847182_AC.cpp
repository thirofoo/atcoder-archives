/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc025/submissions/46847182
 * Submitted at: 2023-10-22 13:41:40
...
struct Solver {
    ...
    inline void generateRest(vector<int>& rest) {
        ...
        if( right.size() > 2 && state.q >= 2*n ) generateRest(right);
        else group.emplace_back(right);
        group.emplace_back(middle);
        if( left.size() > 2 && state.q >= 2*n ) generateRest(left);
