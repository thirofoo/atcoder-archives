/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc025/submissions/46850675
 * Submitted at: 2023-10-22 16:08:15
...
struct Solver {
    ...
    inline void generateRest(vector<int>& rest) {
        ...
        if( right.size() > d && state.q >= 2*n ) generateRest(right);
        else group.emplace_back(right);
        group.emplace_back(middle);
        if( left.size() > d && state.q >= 2*n ) generateRest(left);
