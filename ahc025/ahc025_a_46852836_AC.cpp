/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc025/submissions/46852836
 * Submitted at: 2023-10-22 17:23:01
 * Problem URL: https://atcoder.jp/contests/ahc025/tasks/ahc025_a
 * Result: AC
 * Execution Time: 1857 ms
 */
...
struct State {
    ...
    inline void relationExpantion(vector<vector<char>>& r) {
    ...
    inline void groupSort(int ele, vector<int>& rank, vector<vector<char>>& r, const vector<vector<int>>& latest) {
        // ※ rank は降順で並んでいるとする
        if( rank.empty() ) return (void)rank.emplace_back(ele);
        
        store.assign(rank.size(),'?');
        ...
        while( under-top > 1 ) {
            ...
            store[mid] = r[ele][rank[mid]];
        }
        if( top != rank.size() ) {
            if( store[top] == '?' && !relationUpdate(ele, rank[top], r, latest) ) return;
    ...
    inline void swapping(const int g1, const int g2, vector<vector<char>>& r_one, vector<vector<char>>& r_group, vector<int>& res, vector<vector<int>>& latest, vector<int>& rank) {
        ...
        if( r_one[e1][e2] == '?' ) {
            if( !relationUpdate(e1, e2, r_one) ) return;
            relationExpantion(r_one);
        }
        if( r_one[e1][e2] == '=' || r_one[e1][e2] == '<' ) return (void)(cnt++);
        for(int i=0; i<n; i++) for(int j=0; j<n; j++) {
            if( i == j ) continue;
            if( impossible[i][j] && (r_one[e1][i] == '>' || r_one[e1][i] == '=') && (r_one[e2][j] == '<' || r_one[e2][j] == '=') ) return (void)(cnt++);
        }
    ...
    inline void moving(const int g1, int g2, vector<vector<char>>& r_one, vector<vector<char>>& r_group, vector<int>& res, vector<vector<int>>& latest, vector<int>& rank) {
        ...
        if( r_one[latest[g1][i1]][move_lim] == '?' ){
            if( !relationUpdate(latest[g1][i1], move_lim, r_one) ) return;
            relationExpantion(r_one);
            if( r_one[latest[g1][i1]][move_lim] != '<' ) return;
        }
...
struct Solver {
    ...
    inline void generateRest(vector<int>& rest) {
        ...
        if( relation_one[r1][r2] == '?' && q > 0 ) state.relationUpdate(r1, r2, relation_one);
        if( relation_one[r2][r3] == '?' && q > 0 ) state.relationUpdate(r2, r3, relation_one);
        ...
        if( mid == -1 ) {
            if( relation_one[r1][r3] == '?' && q > 0 ) state.relationUpdate(r1, r3, relation_one);
            if( relation_one[r2][r1] == relation_one[r1][r3] || relation_one[r2][r1] == '=' ) mid = r1;
            if( relation_one[r1][r2] == relation_one[r2][r3] || relation_one[r1][r2] == '=' ) mid = r2;
            if( relation_one[r1][r3] == relation_one[r3][r2] || relation_one[r1][r3] == '=' ) mid = r3;
        }
        ...
        for(auto ele:rest) {
            if( mid != ele && relation_one[mid][ele] == '?' && q > 0 ) state.relationUpdate(mid, ele, relation_one);
            if     ( relation_one[mid][ele] == '>' ) left.emplace_back(ele);
            else if( relation_one[mid][ele] == '<' ) right.emplace_back(ele);
            else  middle.emplace_back(ele);
        }
        if( right.size() > 2 ) generateRest(right);
        else group.emplace_back(right);
        group.emplace_back(middle);
        if( left.size() > 2 && q >= 8*n ) generateRest(left);
    ...
    void generateInit() {
        if( q >= 4*n ) {
            ...
            generateRest(rest);
            state.relationExpantion(relation_one);
            
            int idx = 0, delta = 1;
            for(auto v:group) for(auto ele:v) {
                res[ele] = idx;
    ...
    void solve(){
        ...
        while( state.q && utility::mytm.elapsed() <= TIME_LIMIT ) {
            ...
            if( state.cnt >= latest[g1].size()*latest[g2].size() && d > 2 ) {
                if( state.cnt >= 1000 && d > 3 ) {
                    int r1 = rand_int()%(d-2)+1, r2 = rand_int()%(d-2)+1;
                    while( r1 == r2 ) r2 = rand_int()%(d-2)+1;
                    if( r1 > r2 ) swap(r1,r2);
                    g1 = g_rank[r1], g2 = g_rank[r2];
                }
                else if( latest[g1].size() == 1 ) while( latest[g1].size() == 1 ) g1 = g_rank[rand_int()%(d-2)+1];
                else ( rand_int()%2 ? g1 : g2 ) = g_rank[rand_int()%(d-2)+1];
            }
            query = rand_int()%10;
            if( query ) state.swapping(g1, g2, relation_one, relation_group, res, latest, g_rank);
            else state.moving(g1, g2, relation_one, relation_group, res, latest, g_rank);
            // cerr << state.cnt << " " << state.q << endl;
            // if( state.cnt >= 100000 ) break;
        }
