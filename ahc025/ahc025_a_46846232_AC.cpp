/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc025/submissions/46846232
 * Submitted at: 2023-10-22 12:50:35
...
struct Solver {
    ...
    inline void generateRest(vector<int>& rest) {
        ...
        if( relation_one[r1][r2] == '?' && state.q > 30 ) state.relationUpdate(r1, r2, relation_one);
        if( relation_one[r2][r3] == '?' && state.q > 30 ) state.relationUpdate(r2, r3, relation_one);
        if( relation_one[r1][r3] == '?' && state.q > 30 ) state.relationUpdate(r1, r3, relation_one);
        ...
        for(auto ele:rest) {
            if( mid != ele && relation_one[mid][ele] == '?' && state.q > 30 ) state.relationUpdate(mid, ele, relation_one);
