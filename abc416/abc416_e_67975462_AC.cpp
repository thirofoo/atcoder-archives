/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc416/submissions/67975462
 * Submitted at: 2025-07-26 23:46:54
 * Problem URL: https://atcoder.jp/contests/abc416/tasks/abc416_e
 * Result: AC
 * Execution Time: 618 ms
 */

#include <bits/stdc++.h>
#include <atcoder/all>
using namespace std;
using namespace atcoder;
// using mint = modint998244353;
// using mint = modint1000000007;
using ll  = long long;
using ull = unsigned long long;
#define rep(i, n) for(ll i = 0; i < n; i++)
constexpr ll INF = ((1LL << 61) + (1LL << 30) - 1);

int main() {
  cin.tie(nullptr);
  ios_base::sync_with_stdio(false);

  ll n, m;
  cin >> n >> m;
  using P = pair<ll, ll>;
  vector<vector<ll>> dist(n + 1, vector<ll>(n + 1, INF));
  rep(i, n) dist[i][i] = 0;
  rep(i, m) {
    ll a, b, c;
    cin >> a >> b >> c;
    a--, b--;
    dist[a][b] = min(dist[a][b], c);
    dist[b][a] = min(dist[b][a], c);
  }

  //

  ll k, t;
  cin >> k >> t;
  vector<ll> d(k);
  rep(i, k) {
    cin >> d[i];
    d[i]--;
    dist[d[i]][n] = t;
    dist[n][d[i]] = 0;
  }
  rep(l, n + 1) rep(i, n + 1) rep(j, n + 1) {
    if(dist[i][j] > dist[i][l] + dist[l][j]) {
      dist[i][j] = dist[i][l] + dist[l][j];
    }
  }

  ll ans = 0;
  rep(i, n) rep(j, n) {
    if(dist[i][j] == INF) continue;
    ans += dist[i][j];
  }

  ll q;
  cin >> q;
  while(q--) {
    ll type;
    cin >> type;
    if(type == 1) {
      ll x, y, time;
      cin >> x >> y >> time;
      x--, y--;
      if(dist[x][y] != INF) ans -= dist[x][y];
      if(dist[y][x] != INF) ans -= dist[y][x];
      dist[x][y] = min(dist[x][y], time);
      dist[y][x] = min(dist[y][x], time);
      if(dist[x][y] != INF) ans += dist[x][y];
      if(dist[y][x] != INF) ans += dist[y][x];

      for(auto l: {x, y}) {
        rep(i, n + 1) rep(j, n + 1) {
          if(i != n && j != n && dist[i][j] != INF) ans -= dist[i][j];
          dist[i][j] = min({dist[i][j], dist[i][l] + dist[l][j]});
          if(i != n && j != n && dist[i][j] != INF) ans += dist[i][j];
        }
      }
    } else if(type == 2) {
      ll x;
      cin >> x;
      x--;
      dist[x][n] = min(dist[x][n], t);
      dist[n][x] = min(dist[n][x], 0LL);

      for(auto l: {x, n}) {
        rep(i, n + 1) rep(j, n + 1) {
          if(i != n && j != n && dist[i][j] != INF) ans -= dist[i][j];
          dist[i][j] = min({dist[i][j], dist[i][l] + dist[l][j]});
          if(i != n && j != n && dist[i][j] != INF) ans += dist[i][j];
        }
      }
    } else {
      cout << ans << endl;
      // ll tmp = 0;
      // rep(i, n) rep(j, n) {
      //   if(dist[i][j] != INF) tmp += dist[i][j];
      // }
      // cout << tmp << endl;
    }
  }

  return 0;
}