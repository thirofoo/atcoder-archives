/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc360/submissions/55096801
 * Submitted at: 2024-06-30 22:39:44
...
int main() {
    ...
    rep(i, comp.size()) {
        ...
        if(ans < cand) {
            ans = cand;
            left = comp[i];
            right = comp[seg.prod(i, comp.size()).idx+1];
            // cerr << left << ' ' << right << ' ' << ans << '\n';
        }
