/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc176/submissions/36171251
 * Submitted at: 2022-11-03 15:20:32
...
int main() {
    ...
    deque<T> todo;
    todo.push_front(T(0,cx,cy));
    while(!todo.empty()){
        auto [c,x,y] = todo.front(); todo.pop_front();
        ...
        for(int dx=-2;dx<=2;dx++){
            ...
            for(int dy=-2;dy<=2;dy++){
                ...
                if((dx == 0 && abs(dy) == 1) || (dy == 0 && abs(dx) == 1)){
                    todo.push_front(T(c,x+dx,y+dy));
                }
                else todo.push_back(T(c+1,x+dx,y+dy));
            }
        }
    }
