/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/arc186/submissions/59228338
 * Submitted at: 2024-10-28 17:46:25
...
int main() {
  ...
  for(ll i = 1; i <= n; i++) {
    ...
    if(info[i] != -1 && st.find(info[i]) != st.end()) {
      // cerr << "from: " << i << " to: " << info[i] << endl;
      graph[i].emplace_back(info[i]);
      st.erase(info[i]);
    }
    auto it = st.upper_bound(a[i]);
    if(it == st.end() || (*it) >= i) continue;
    // cerr << "from: " << i << " to: " << (*it) << endl;
  ...
  auto f = [&](auto self, ll v, ll p) -> void {
    ...
    size[v] = p_size + 1;
    // cerr << "dp[" << v << "]: " << dp[v].val() << " size[" << v << "]: " << size[v] << endl;
    return;
  };
  mint ans = 1;
  ll rest  = n;
  for(ll i = 1; i <= n; i++) {
    // 訪問済み or 有向木の根でない場合は continue
    if(visited[i] || st.find(i) == st.end()) continue;
    // cerr << "i: " << i << endl;
    f(f, i, -1);
    // トポソの形状 * 数字割り当て
