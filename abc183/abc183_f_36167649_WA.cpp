/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc183/submissions/36167649
 * Submitted at: 2022-11-03 11:35:16
...
int main() {
    ...
    while(q--){
        ...
        if(t == 1){
            ...
            ll sx = uf.size(a),sy = uf.size(b);
            if(sx < sy)swap(a,b);
            // 連結成分x > 連結成分y の状況を作る
            ll x = uf.leader(a),y = uf.leader(b);

            // 大きい成分に小さい成分を足す様に回す
            // => for文がO(N)でなくmaxO(logN)になる
            // (merge sortっぽい状況になる時が最大計算量)
