/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc064/submissions/75300258
 * Submitted at: 2026-04-27 00:12:17
 * Problem URL: https://atcoder.jp/contests/ahc064/tasks/ahc064_a
 * Result: AC
 * Execution Time: 1515 ms
 */
...
static constexpr int BEAM_CAND_LIMIT = 200;
static constexpr int BEAM_ROOT_LIMIT = 50;
static constexpr int TURN_DP_KEEP = 40;
static constexpr int TURN_CELL_KEEP = 4;

static constexpr int PAIR_DP_NOISE_TRIALS = 50;
static constexpr int PAIR_DP_NOISE_SCALE1 = 100000;
static constexpr int PAIR_DP_NOISE_SCALE2 = 150000;
static constexpr int PAIR_DP_NOISE_SCALE3 = 200000;

static constexpr long long TURN_OP_COUNT_W = 200000LL;
static constexpr long long TURN_MOVED_K_W = 1000LL;
...
static constexpr long long BEAM_FIXED_PREFIX_W = 2500000LL;

static constexpr long long MOVE_FIXED_PREFIX_DELTA_W = 5000000LL;
static constexpr long long MOVE_READY_PREFIX_W = 200000LL;

static constexpr long long MERGE_DELTA_GOOD_W = 1000LL;
...
struct Solver {
  ...
  int correct_prefix_len(const Board& b, int r) const {
    int lim = min<int>(b.dsz[r], 10);
    int p = 0;
    while (p < lim && b.dep[r][p] == 10 * r + p) p++;
    return p;
  }

  int fixed_prefix_total(const Board& b) const {
    int res = 0;
    for (int r = 0; r < R; r++) res += correct_prefix_len(b, r);
    return res;
  }

  bool breaks_fixed_prefix(const Board& b, const Move& m) const {
    if (m.type != 0) return false;
    int lock = correct_prefix_len(b, m.i);
    return (int)b.dsz[m.i] - m.k < lock;
  }

  int fixed_prefix_delta(const Board& b, const Move& m) const {
    int before = fixed_prefix_total(b);
    Board nb = b;
    simulate_one(nb, m);
    return fixed_prefix_total(nb) - before;
  }

  int ready_prefix_count(const Board& b) const {
    int res = 0;
    for (int r = 0; r < R; r++) {
      int p = correct_prefix_len(b, r);
      if (p == b.dsz[r] && p < 10) res++;
    }
    return res;
  }
  ...
  bool would_be_seen_single(const Move& m) const {
    Board b = bd;
    if (breaks_fixed_prefix(b, m)) return true;
    simulate_one(b, m);
  ...
  bool simulate_turn_local(
    ...
    for (auto& m : ops) {
      if (breaks_fixed_prefix(b, m)) return false;
      if (m.type == 0) {
  ...
  uint64_t turn_key_allow_empty(Turn t) const {
    if (t.n == 0) return 1;
    return turn_key(t);
  }

  int turn_total_k(const Turn& t) const {
    int res = 0;
    for (int i = 0; i < t.n; i++) res += t.mv[i].k;
    return res;
  }

  void add_top_turn(vector<TurnCand>& v, TurnCand x, int keep) const {
    x.key = turn_key_allow_empty(x.turn);
    if (!x.key) return;

    for (auto& y : v)
      if (y.key == x.key) return;

    v.push_back(x);

    sort(v.begin(), v.end(), [&](const TurnCand& a, const TurnCand& b) {
      if (a.score != b.score) return a.score > b.score;
      if (a.turn.n != b.turn.n) return a.turn.n > b.turn.n;
      return turn_total_k(a.turn) > turn_total_k(b.turn);
    });

    if ((int)v.size() > keep) v.resize(keep);
  }
  ...
  bool try_apply_candidates(vector<Cand> cand) {
    ...
    for (auto& t : turns) {
      vector<Move> ops(t.mv, t.mv + t.n);
      if (apply_turn(ops)) return true;
    }
    return false;
  }

  long long move_final_bonus(const Board& b, const Move& m) const {
    int delta = fixed_prefix_delta(b, m);
    long long score = MOVE_FIXED_PREFIX_DELTA_W * delta;
    Board nb = b;
    simulate_one(nb, m);
    score += MOVE_READY_PREFIX_W * ready_prefix_count(nb);
    return score;
  }

  long long pair_dp_move_score(const Board& b, const Move& m, int before_endpoint) const {
    int d = delta_good(b, m);

    if (d > 0) {
      return MERGE_DELTA_GOOD_W * d
           + MERGE_CUT_SURFACE_W * cut_surface_potential_delta(b, m)
           + dst_balance_score(b, m)
           + move_final_bonus(b, m);
    }

    if (d != 0) return LLONG_MIN / 4;
    if (cuts_good_edge(b, m)) return LLONG_MIN / 4;

    int endpoint_diff = endpoint_match_after(b, m) - before_endpoint;
    int pot = cut_surface_potential_delta(b, m);

    if (endpoint_diff <= 0 && pot <= 0) return LLONG_MIN / 4;

    int inside = m.type == 0 ? seg_good_dep_suffix(b, m.i, m.k) : seg_good_side_prefix(b, m.j, m.k);
    int dst_n = m.type == 0 ? (int)b.ssz[m.j] : (int)b.dsz[m.i];

    long long score = EXPOSE_ENDPOINT_DIFF_W * endpoint_diff
                    + EXPOSE_CUT_SURFACE_W * pot
                    + EXPOSE_INSIDE_W * inside
                    + EXPOSE_K_W * m.k
                    - EXPOSE_DIST_W * abs(m.i - m.j)
                    + dst_balance_score(b, m)
                    + move_final_bonus(b, m);
  ...
  void insert_pair_opt(PairMoveOpt opts[TURN_CELL_KEEP], const Move& m, long long score) const {
    if (score <= LLONG_MIN / 8) return;

    for (int p = 0; p < TURN_CELL_KEEP; p++) {
      if (opts[p].used &&
          opts[p].mv.type == m.type &&
          opts[p].mv.i == m.i &&
          opts[p].mv.j == m.j &&
          opts[p].mv.k == m.k) {
        if (score > opts[p].score) opts[p].score = score;
        return;
      }
    }

    for (int p = 0; p < TURN_CELL_KEEP; p++) {
      if (!opts[p].used || score > opts[p].score) {
        for (int q = TURN_CELL_KEEP - 1; q > p; q--) opts[q] = opts[q - 1];
        opts[p].used = true;
        opts[p].mv = m;
        opts[p].score = score;
        return;
  ...
  void calc_pair_move_options(const Board& b, const Move* last, int last_n, PairMoveOpt opts[R][R][TURN_CELL_KEEP]) const {
    int before_endpoint = endpoint_match_count_of(b);

    for (int i = 0; i < R; i++)
      for (int j = 0; j < R; j++)
        for (int p = 0; p < TURN_CELL_KEEP; p++)
          opts[i][j][p] = PairMoveOpt{};
      ...
      for (int i = 0; i < R; i++) {
        for (int j = 0; j < R; j++) {
          ...
          for (int k = 1; k <= max_k; k++) {
            ...
            if (breaks_fixed_prefix(b, m)) continue;

            long long score = pair_dp_move_score(b, m, before_endpoint);
            insert_pair_opt(opts[i][j], m, score);
          }
        }
      }
    }
  ...
  int deterministic_noise(int i, int j, int opt_id, int salt, int scale) const {
    if (scale <= 0) return 0;
    uint64_t x = 0x9e3779b97f4a7c15ULL;
    x ^= (uint64_t)(i + 1) * 0xbf58476d1ce4e5b9ULL;
    x ^= (uint64_t)(j + 1) * 0x94d049bb133111ebULL;
    x ^= (uint64_t)(opt_id + 1) * 0x632be59bd9b4e019ULL;
    x ^= (uint64_t)(salt + 1) * 0x9ddfea08eb382d69ULL;
    uint64_t h = splitmix64(x);
    return (int)(h % (2 * scale + 1)) - scale;
  }

  void add_pair_dp_action(const PairMoveOpt opts[R][R][TURN_CELL_KEEP], int salt, int noise_scale, vector<Turn>& res, vector<uint64_t>& used) const {
    static constexpr long long NEG = LLONG_MIN / 4;
    long long dp[R + 1][R + 1];
    uint8_t choice[R + 1][R + 1];
    uint8_t opt_choice[R + 1][R + 1];
    ...
    for (int i = 0; i <= R; i++) {
      for (int j = 0; j <= R; j++) {
        ...
        if (i < R && j < R) {
          for (int opt_id = 0; opt_id < TURN_CELL_KEEP; opt_id++) {
            const PairMoveOpt& opt = opts[i][j][opt_id];
            if (!opt.used) break;

            long long val = cur
                          + opt.score
                          + TURN_OP_COUNT_W
                          + TURN_MOVED_K_W * opt.mv.k
                          + deterministic_noise(i, j, opt_id, salt, noise_scale);

            if (val > dp[i + 1][j + 1]) {
              dp[i + 1][j + 1] = val;
              choice[i + 1][j + 1] = 3;
              opt_choice[i + 1][j + 1] = (uint8_t)opt_id;
            }
          }
        }
      }
    ...
    while (i > 0 || j > 0) {
      ...
      if (ch == 1) {
        ...
        const PairMoveOpt& opt = opts[i - 1][j - 1][opt_choice[i][j]];
        if (t.n < R) t.mv[t.n++] = opt.mv;
        i--;
        j--;
      } else {
        break;
    ...
    reverse(t.mv, t.mv + t.n);
    if (!normalize_turn(t)) return;
  ...
  vector<Turn> make_turn_candidates_pair_dp(const Board& b, const Move* last, int last_n) const {
    PairMoveOpt opts[R][R][TURN_CELL_KEEP];
    calc_pair_move_options(b, last, last_n, opts);

    vector<Turn> res;
    res.reserve(BEAM_BRANCH);

    vector<uint64_t> used;
    used.reserve(BEAM_BRANCH * 2 + 10);

    add_pair_dp_action(opts, 0, 0, res, used);

    for (int salt = 1; (int)res.size() < BEAM_BRANCH && salt <= PAIR_DP_NOISE_TRIALS; salt++) {
      int scale;
      if (salt % 3 == 1) scale = PAIR_DP_NOISE_SCALE1;
      else if (salt % 3 == 2) scale = PAIR_DP_NOISE_SCALE2;
      else scale = PAIR_DP_NOISE_SCALE3;

      add_pair_dp_action(opts, salt, scale, res, used);
    }
  ...
  long long evaluate_board(const Board& b, int depth) const {
    ...
    int fixed = fixed_prefix_total(b);
    long long bal = 0;
    for (int i = 0; i < R; i++) {
  ...
  vector<Turn> make_turn_candidates(vector<Cand> cand) const {
    ...
    vector<Cand> cell[R][R];

    for (auto& c : cand) {
      auto& v = cell[c.mv.i][c.mv.j];
      v.push_back(c);

      sort(v.begin(), v.end(), [](const Cand& a, const Cand& b) {
        if (a.score != b.score) return a.score > b.score;
        return a.mv.k > b.mv.k;
      });

      if ((int)v.size() > TURN_CELL_KEEP) v.resize(TURN_CELL_KEEP);
    }

    vector<TurnCand> dp[R + 1][R + 1];

    for (int last_j = 0; last_j <= R; last_j++) {
      TurnCand base;
      base.score = 0;
      base.turn.n = 0;
      base.key = 1;
      dp[R][last_j].push_back(base);
    }

    for (int i = R - 1; i >= 0; i--) {
      for (int last_j = R; last_j >= 0; last_j--) {
        for (auto base : dp[i + 1][last_j]) {
          add_top_turn(dp[i][last_j], base, TURN_DP_KEEP);
        }

        for (int j = last_j; j < R; j++) {
          for (auto& c : cell[i][j]) {
            for (auto base : dp[i + 1][j + 1]) {
              TurnCand nxt = base;

              if (nxt.turn.n >= R) continue;

              nxt.turn.mv[nxt.turn.n++] = c.mv;

              if (!normalize_turn(nxt.turn)) continue;

              nxt.score += c.score;
              nxt.score += TURN_OP_COUNT_W;
              nxt.score += TURN_MOVED_K_W * c.mv.k;
              nxt.key = turn_key_allow_empty(nxt.turn);

              add_top_turn(dp[i][last_j], nxt, TURN_DP_KEEP);
            }
          }
        }
      }
    ...
    vector<TurnCand> top = dp[0][0];

    sort(top.begin(), top.end(), [&](const TurnCand& a, const TurnCand& b) {
      if (a.score != b.score) return a.score > b.score;
      if (a.turn.n != b.turn.n) return a.turn.n > b.turn.n;
      return turn_total_k(a.turn) > turn_total_k(b.turn);
    });
    ...
    for (auto& tc : top) {
      if (tc.turn.n == 0) continue;
      res.push_back(tc.turn);
      if ((int)res.size() >= BEAM_BRANCH) break;
    }
  ...
  void beam_search() {
    ...
    for (int turn = 0; turn < GREEDY_TURN_LIMIT; turn++) {
      ...
      if (pool[best_idx].blocks <= R && fixed_prefix_total(pool[best_idx].b) >= N_CAR) break;
      ...
      for (int idx : beam) {
        ...
        auto turns = make_turn_candidates_pair_dp(nd.b, nd.last, nd.last_n);
  ...
  bool unload_all_dep_to_side() {
    for (int i = 0; i < R; i++) {
      while ((int)bd.dsz[i] > correct_prefix_len(bd, i)) {
        vector<pair<long long, Move>> tries;
        int lock = correct_prefix_len(bd, i);
        for (int j = 0; j < R; j++) {
          int free = SIDE_CAP - (int)bd.ssz[j];
          if (free <= 0) continue;
          int k = min((int)bd.dsz[i] - lock, free);
          if (k <= 0) continue;
          Move m{0, i, j, k};
