/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc294/submissions/40527917
 * Submitted at: 2023-04-11 12:32:28
...
class EulerTour{
    public:
        ...
        explicit EulerTour(const int &_n){
            ...
            finish.assign(n,0);
            discover.assign(n,0);
            vcost1 = segtree<S, Sum_op, Sum_e>(3*n+1);
            ecost1 = segtree<S, Sum_op, Sum_e>(3*n+1);
            vcost2 = segtree<S, Sum_op, Sum_e>(3*n+1);
            ecost2 = segtree<S, Sum_op, Sum_e>(3*n+1);
            depth_visit = segtree<U, Min_op, Min_e>(3*n+1);
        }
        ...
        inline S partV(const int &root){return vcost1.prod(discover[root]  ,finish[root]);}
        inline S partE(const int &root){return ecost1.prod(discover[root]+1,finish[root]);}
