/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc053/submissions/69275428
 * Submitted at: 2025-09-13 20:03:41
...
// ===== パラメータ: 各スケール(桁)あたりのカード枚数 =====
// 既定 30。ビルド時は -DPER_LEVEL_QTY=28 などで変更可。
// 実行時は環境変数 PER_LEVEL でも上書き可能（優先）。
#ifndef PER_LEVEL_QTY
#define PER_LEVEL_QTY 32
#endif
...
int main(){
  ...
  // ★ 各スケールごとのカード枚数（パラメトリック化）
  int per_level = PER_LEVEL_QTY;
  if (const char* env = getenv("PER_LEVEL")) {
    int v = atoi(env);
    if (v > 0) per_level = v;
  }
  per_level = max(1, min(per_level, M)); // 安全に [1, M] に丸め

  // 取れる段数 K を最大化（安全上限 60）
  int K = 0;
  if (W > 0 && free_cards > 0) {
    K = min(60, free_cards / per_level);
  }
  ...
  int coarse_cards_capacity = (K>0 ? K * per_level : 0);  // ★ 置換
  ...
  for(int i=0;i<K;++i){
    ...
    if (wi < 1) wi = 1;
    w[i]=wi;
    // ★ 各スケールの生成枚数を per_level に
    int quota = min(per_level, N - (int)A.size());
  ...
  // SA パラメータ
  const double SA_MS = 1950.0; // 残り時間いっぱい
