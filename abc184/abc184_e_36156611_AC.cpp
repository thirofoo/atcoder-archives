/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc184/submissions/36156611
 * Submitted at: 2022-11-02 19:48:39
...
int main() {
    ...
    while(!todo.empty()){
        ...
        if('a' <= field[x][y] && field[x][y] <= 'z' && ok[field[x][y]-'a']){
            for(auto [nx,ny]:warp[field[x][y]-'a']){
                if(visited[nx][ny])continue;
                todo.push(T(c+1,nx,ny));
            }
            ok[field[x][y]-'a'] = false;
        }
    }
