/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc050/submissions/67384033
 * Submitted at: 2025-07-06 20:12:35
 * Problem URL: https://atcoder.jp/contests/ahc050/tasks/ahc050_a
 * Result: AC
 * Execution Time: 1959 ms
 */
...
struct PrecomputedData {
    ...
    static void init() {
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < N; ++j) {
                a[i][j] = pow(i - center, 2) + pow(j - center, 2); // Squared Euclidean
                b[i][j] = abs(i - (int)center) + abs(j - (int)center); // Manhattan
            }
        }
        initialized = true;
    }
...
private:
    ...
    bool is_rock[N][N];
    double prob1[N][N], prob2[N][N]; // ポインタ入れ替え用のバッファ
...
pair<double, vector<pair<int, int>>> GreedyRunner::run() {
    ...
    double (*current_prob)[N] = prob1;
    double (*next_prob)[N] = prob2;
    ...
    candidates.reserve(num_placements);
    ...
    for (int k = 0; k < num_placements; ++k) {
        ...
        for (int r=0; r<N; ++r) for (int c=0; c<N; ++c) {
            if (current_prob[r][c] > 1e-18) {
                ...
                double p4 = current_prob[r][c] / 4.0;
...
struct Solver {
    ...
    void solve() {
        ...
        const int MAX_RANDOM_POOL = 100000;
