/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc050/submissions/67385144
 * Submitted at: 2025-07-06 20:34:57
 * Problem URL: https://atcoder.jp/contests/ahc050/tasks/ahc050_a
 * Result: AC
 * Execution Time: 1880 ms
 */
...
enum class TieBreakStrategy {
    EUCLID_OUTSIDE_IN, EUCLID_INSIDE_OUT, MANHATTAN_OUTSIDE_IN, MANHATTAN_INSIDE_OUT,
    TOP_TO_BOTTOM, BOTTOM_TO_TOP, LEFT_TO_RIGHT, RIGHT_TO_LEFT,
    FINAL_DETERMINISTIC
};
struct PrecomputedData {
    static double a[N][N]; static int b[N][N]; static bool initialized;
    static void init() {
...
double PrecomputedData::a[N][N]; int PrecomputedData::b[N][N]; bool PrecomputedData::initialized = false;
...
void Solver::solve() {
    ...
    while(utility::mytm.elapsed() < 1000) {
        ...
        if (current_E > max_E) { max_E = current_E; best_P = current_P; }
    ...
    if(!best_P.empty()){
        ...
        while(utility::mytm.elapsed() < 1850) {
            if (num_placements <= WINDOW_SIZE) break;
            vector<pair<int, int>> temp_P = best_P; 
            ...
            if(new_E > max_E){ max_E = new_E; best_P = temp_P; }
            hill_climb_iterations++;
        }
    }
    cerr << "Hill-Climb Iterations: " << hill_climb_iterations << endl;
    cerr << "Best E after phase 2: " << max_E << endl;

    auto [final_E, final_P] = initial_solver.run(TieBreakStrategy::FINAL_DETERMINISTIC, 1);
    cerr << "Final Deterministic Score: " << final_E << endl;
    if(final_E > max_E){ max_E = final_E; best_P = final_P; }
...
pair<double, vector<pair<int, int>>> InitialSolver::run(TieBreakStrategy strategy, int random_pool_size) {
    ...
    for (int k = 0; k < num_placements; ++k) {
        ...
        for (int r=0; r<N; ++r) for (int c=0; c<N; ++c) if (current_prob[r][c] > 1e-18) {
        ...
        } else if (strategy == TieBreakStrategy::FINAL_DETERMINISTIC) {
            double min_impact_score = 1e18;
            best_cell = candidates[0];
            for(const auto& cand_cell : candidates){
                double current_impact_score = 0;
                for(int r = cand_cell.first - 1; r >= 0 && !is_rock[r][cand_cell.second]; --r) current_impact_score += current_prob[r][cand_cell.second];
                for(int r = cand_cell.first + 1; r < N && !is_rock[r][cand_cell.second]; ++r) current_impact_score += current_prob[r][cand_cell.second];
                for(int c = cand_cell.second - 1; c >= 0 && !is_rock[cand_cell.first][c]; --c) current_impact_score += current_prob[cand_cell.first][c];
                for(int c = cand_cell.second + 1; c < N && !is_rock[cand_cell.first][c]; ++c) current_impact_score += current_prob[cand_cell.first][c];
                if(current_impact_score < min_impact_score){ min_impact_score = current_impact_score; best_cell = cand_cell; }
            }
        } else {
            int pool_size = min((int)candidates.size(), random_pool_size);
            if (candidates.size() > pool_size) nth_element(candidates.begin(), candidates.begin() + pool_size - 1, candidates.end(), [&](const auto& a, const auto& b){ return is_better(a, b, strategy); });
            best_cell = candidates[rand_int() % pool_size];
        }

        P.push_back(best_cell); placed[best_cell.first][best_cell.second] = true;
        ...
        if (remaining_prob_sum > 1e-18) { double inv_sum = 1.0 / remaining_prob_sum; for (int i=0; i<N; ++i) for(int j=0; j<N; ++j) current_prob[i][j] *= inv_sum; }
...
double ScoreCalculator::run(const vector<pair<int, int>>& P) {
    ...
    double (*current_prob)[N] = prob1, (*next_prob)[N] = prob2;
    for(int i=0; i<N; ++i) for(int j=0; j<N; ++j) current_prob[i][j] = 0.0;
    if (num_placements > 0) { double p = 1.0 / num_placements; for (const auto& cell : initial_empty_cells) current_prob[cell.first][cell.second] = p; }
    compute_all_stops();
    double expected_prize = 0, prob_alive = 1.0;
    for (int k = 0; k < num_placements; ++k) {
        for(int i=0; i<N; ++i) for(int j=0; j<N; ++j) next_prob[i][j] = 0.0;
        ...
        expected_prize += prob_alive;
        if (remaining_prob_sum > 1e-18) { double inv_sum = 1.0 / remaining_prob_sum; for(int i=0; i<N; ++i) for(int j=0; j<N; ++j) current_prob[i][j] *= inv_sum; }
        is_rock[placed_cell.first][placed_cell.second] = true; update_stops(placed_cell.first, placed_cell.second);
    }
