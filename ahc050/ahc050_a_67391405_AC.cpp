/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc050/submissions/67391405
 * Submitted at: 2025-07-06 22:28:44
...
class Runner {
public:
    Runner(const vector<string>& board, const vector<pair<int, int>>& empty_cells)
...
struct Solver {
    ...
    void solve() {
        ...
        while (util::tm.ms() < 1950) { // 1.95 秒ループ
...
pair<double, vector<pair<int, int>>> Runner::run(int random_pool_size) {
    ...
    for (size_t step = 0; step < empty.size(); ++step) {
        ...
        if (cand.size() == 1) {
            ...
            nth_element(scored.begin(), scored.begin() + pool -1, scored.end(),
        ...
        if (total_before > 1e-12)
            prob_alive *= (remain / total_before);
...
void Runner::update_stops(int r, int c) {
    /* 上方向 */
    int i = r - 1;
    while (i >= 0 && !is_rock[i][c]) {
        down_stop[i][c] = r - 1;
        --i;
    }
    /* 下方向 */
    i = r + 1;
    while (i < N && !is_rock[i][c]) {
        up_stop[i][c] = r + 1;
        ++i;
    }
    /* 左方向 */
    int j = c - 1;
    while (j >= 0 && !is_rock[r][j]) {
        right_stop[r][j] = c - 1;
        --j;
    }
    /* 右方向 */
    j = c + 1;
    while (j < N && !is_rock[r][j]) {
        left_stop[r][j] = c + 1;
        ++j;
    }
}