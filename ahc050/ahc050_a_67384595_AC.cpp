/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc050/submissions/67384595
 * Submitted at: 2025-07-06 20:23:45
 * Problem URL: https://atcoder.jp/contests/ahc050/tasks/ahc050_a
 * Result: AC
 * Execution Time: 1954 ms
 */
...
public:
    SimulatorBase(const vector<string>& s, const vector<pair<int, int>>& ec)
        : S(s), initial_empty_cells(ec) {
...
// スコア計算に特化したクラス
class ScoreCalculator : public SimulatorBase {
private:
    double prob1[N][N], prob2[N][N];
public:
    ScoreCalculator(const vector<string>& s, const vector<pair<int, int>>& ec) : SimulatorBase(s, ec) {}
    double run(const vector<pair<int, int>>& P);
};

// 初期解を探索するクラス
class InitialSolver : public SimulatorBase {
private:
    double prob1[N][N], prob2[N][N];
    bool placed[N][N] = {};
...
public:
    InitialSolver(const vector<string>& s, const vector<pair<int, int>>& ec) : SimulatorBase(s, ec) {
...
pair<double, vector<pair<int, int>>> InitialSolver::run(TieBreakStrategy strategy, int random_pool_size) {
    ...
    for (int k = 0; k < num_placements; ++k) {
        ...
        pair<int, int> best_cell; // ★★★ エラー箇所: ここで宣言する
        if (candidates.empty()) {
...
void SimulatorBase::compute_all_stops() {
    for (int i=0; i<N; ++i) {
...
void SimulatorBase::update_stops(int r, int c) {
