/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc050/submissions/67382321
 * Submitted at: 2025-07-06 19:37:04
            ...
            for(const auto& cell : initial_empty_cells){
                if (!placed[cell.first][cell.second]) {
                    ...
                    if (current_p < min_p - 1e-12) {
                        min_p = current_p;
                        max_dist_at_min_p = distance_from_center(r, c);
                        best_cell = cell;
                    } 
                    // 確率がほぼ同じなら、中心から遠い方を優先
                    else if (std::abs(current_p - min_p) < 1e-12) {
                        double dist = distance_from_center(r, c);
                        if (dist > max_dist_at_min_p) {
                            max_dist_at_min_p = dist;
                            best_cell = cell;
                        }
                    }
                }
