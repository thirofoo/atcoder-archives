/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc367/submissions/56828378
 * Submitted at: 2024-08-17 22:29:58
...
int main() {
  ...
  vector<ll> freqA(200001, 0), freqB(200001, 0);
  Zobrist_hash_set<ll> z;
  z.init();

  auto addL = [&](ll value, vector<ll> &freq) {
    z.flip(value * 200005 + freq[value]);
    freq[value]--;
    z.flip(value * 200005 + freq[value]);
  };
  ...
  vector<ll> q_idx1(queries1.size()), q_idx2(queries2.size());
  long long sq = sqrt(N);
  ...
  long long l = 0, r = 0;
  vector<ll> ret1(queries1.size()), ret2(queries2.size());
