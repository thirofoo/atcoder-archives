/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc400/submissions/64545926
 * Submitted at: 2025-04-05 22:08:57
...
int main() {
  ...
  for(ll i = 6; i * i <= 1e12; i++) {
    ...
    if(tmp == 1) continue;
    ll tmp2 = sqrt(tmp);
    while(tmp % div_cand[tmp2] == 0) {
      // cerr << tmp2 << " " << div_cand[tmp2] << endl;
      tmp /= div_cand[tmp2];
    }
    if(tmp != 1) continue;
