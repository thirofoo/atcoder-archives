/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc426/submissions/69863526
 * Submitted at: 2025-10-04 22:01:32
 * Problem URL: https://atcoder.jp/contests/abc426/tasks/abc426_e
 * Result: WA
 * Execution Time: 909 ms
 */
...
int main() {
  ...
  while(t--) {
    ...
    // 三分探索 (実数値ver)
    ld left = 0, right = max(stop_t, stop_a);
    while(right - left > 0.000000000000001) {
      ...
      ld dist1 = sqrt((pt1.first - pa1.first) * (pt1.first - pa1.first) + (pt1.second - pa1.second) * (pt1.second - pa1.second));
      ...
      ld dist2 = sqrt((pt2.first - pa2.first) * (pt2.first - pa2.first) + (pt2.second - pa2.second) * (pt2.second - pa2.second));
    ...
    ld ans = sqrt((pt.first - pa.first) * (pt.first - pa.first) + (pt.second - pa.second) * (pt.second - pa.second));
    cout << ans << "\n";
  }
