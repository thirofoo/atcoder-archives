/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc066/submissions/76299244
 * Submitted at: 2026-05-31 13:43:00
...
vector<string> proxy_search_topk(const Timer &timer, double search_start, double search_end, int K,
...
string macro_search(const Timer &timer, double search_start, double search_end) {
  return proxy_search_topk(timer, search_start, search_end, 1, MACRO_MIN_LEN, MACRO_MAX_LEN)[0];
}
...
int main() {
  ...
  if (M <= MAX_EXACT_M) {
    ...
    vector<string> cands = proxy_search_topk(timer, 0.0, usable * EXACT_PROXY_FRAC, EXACT_TOPK,
