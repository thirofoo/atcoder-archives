/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/arc146/submissions/34176024
 * Submitted at: 2022-08-20 22:16:42
...
int main() {
    ...
    for(ll i=rest;i>=0;i--){
        ...
        rep(j,k){
            ...
            if(!(b[j]&mask & (1LL << i)))tmp += (1LL << i)-b[j]&mask;
        }
        if(m >= tmp){
            // cout << i << " " << tmp << endl;
            ans += (1LL << i);
            m -= tmp;
            rep(j,k)if(!(b[j]&mask & (1LL << i)))b[j] = 0;
        }
