/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc063/submissions/74930131
 * Submitted at: 2026-04-13 13:02:44
...
// constexpr int DIR_ORDER_COUNT = 8;
// alignas(64) constexpr int8_t DIR_ORDERS[DIR_ORDER_COUNT][4] = {
//     {0, 1, 2, 3}, {0, 1, 3, 2}, {1, 0, 2, 3}, {1, 0, 3, 2}, 
//     {3, 2, 1, 0}, {3, 2, 0, 1}, {2, 3, 1, 0}, {2, 3, 0, 1},
// };
...
constexpr array<int, 17> BEAM_WIDTH_MAX_BY_N = {
    -1, -1, -1, -1, -1, -1, -1, -1,
    160, 160, 160, 160, 160, 160, 120, 80, 80,
};
constexpr array<int, 17> CUT_APPLY_NODE_LIMIT_BY_N = {
    -1, -1, -1, -1, -1, -1, -1, -1,
    40, 40, 40, 40, 40, 20, 20, 20, 20,
};
...
// afterBodyLen in [p - NEAR_P_ALLOW_BY_N[N], p].
// Corrected formula: biting original bodyAt(j) at BFS distance d
//   → afterBodyLen = j + d + 1  (push-front before bite detection)
//   → j = targetL - d - 1
// Soft BFS: body index >= p and food are passable (floor). Index < p is blocked.
int findPrefixBrokenCutPaths(const SnakeState& st, int p, int topK, FixedPath* out) const {
    ...
    topK = max(1, topK);
    int releaseThresh = st.bodyLen - 1 - p;
    const int nearPAllow = NEAR_P_ALLOW_BY_N[N];
    const int L = st.bodyLen;
    beginBfsVisit();
    uint8_t head = st.bodyFront();
    uint8_t neck = (L >= 2) ? st.bodyAt(1) : 255;

    // Goal check: j = targetL - d - 1, with occupied guarantee j < L - d.
    auto isGoal = [&](uint8_t npos, int d) -> bool {
        for (int delta = 0; delta <= nearPAllow; ++delta) {
            ...
            int j = targetL - d - 1;
            if (j < 2 || j >= L) continue;
            if (j >= L - d) continue;  // tail-pop guarantee: cell still exists
    ...
    auto isBlocked = [&](uint8_t npos) -> bool {
        return releaseAt(npos) > releaseThresh;
    };

    // maxDist: j = targetL - d - 1 >= 2  →  d <= targetL - 3  →  d <= p - 3
    int maxDist = p - 3;
    ...
    for (int di = 0; di < 4; ++di) {
        ...
        bfsParent[npos] = head;
        bfsPdir[npos] = (int8_t)di;
        bq[qt] = npos; bqd[qt] = 1; ++qt;
    }

    int qh = 0;
    while (qh < qt) {
        uint8_t pos = bq[qh]; int t = bqd[qh]; ++qh;
        if (t >= maxDist) break;
        if (gn >= topK && t > goalMaxDist) break;
        for (int di = 0; di < 4; ++di) {
            ...
            bfsParent[npos] = pos;
            bfsPdir[npos] = (int8_t)di;
            bq[qt] = npos; bqd[qt] = (int16_t)nt; ++qt;
        }
    }
...
void transitionPrefixBrokenCutRecover(
    ...
    for (int pi = 0; pi < pathCount; ++pi) {
        if (timer.isOver()) break;
        const FixedPath& path = paths[pi];
        if (!gPool.hasSpace(path.len + NCELLS * 2)) continue;
        ...
        for (int si = 0; si < path.len; ++si) {
            ...
            if (ret.bite) {
                int afterLen = nxt.st.bodyLen;
                if (afterLen > p) {
                    // Tail-side bite: just trimmed the wrong suffix.
                    // Treat as walking on empty floor — continue the path.
                    continue;
                }
                // Bite near or below p — do recovery and emit
                int hintAfterBite = min((int)pb, (int)nxt.st.colorLen);
                FixedPath recovery = buildRecoveryPathByOldBody(
                    savedBodyBuf, savedBodyHead, savedBodyLen, nxt.st, pb
                );
        ...
        // If we walked the entire path without any bite (goal cell no longer occupied
        // due to mid-path tail-side bite or tail pops), just discard this candidate.
    }
}
