/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc063/submissions/74932936
 * Submitted at: 2026-04-13 15:34:35
...
void buildReleaseSparse(const SnakeState& st, int suffixStart = -1) const {
    ++releaseCur;
    ...
    for (int i = 1; i < st.bodyLen; ++i) {
        ...
        int rel;
        if (suffixStart >= 0 && i >= suffixStart) {
            rel = 1; // suffix-wrong: passable immediately (bite allowed)
        } else {
            rel = st.bodyLen - 1 - i;
            if (rel < 1) rel = 1;
        }
...
int selectTopBeamIndices(const vector<BeamKey>& keys, int beamWidth, int* order) const {
    static int selectBuf[SELECT_BUF_CAP];
...
int pickWanderStepForState(const SnakeState& st) {
    ...
    for (int di = 0; di < 4; ++di) {
        ...
        uint8_t npos = (uint8_t)next;
        if (npos == neck) continue;
        // Skip if moving here would cause unwanted bite:
        ...
        // However, suffix-wrong body cells (releaseAt <= 1) are allowed — bite cuts bad suffix
        ...
        if (!hasFood) {
            ...
            if (st.bodyOccupiedPos(npos)) {
                ...
                bool safeSuffix = (releaseAt(npos) <= 1);
                if (!safeTail && !safeTailPrev && !safeSuffix) continue;
            }
        } else {
            // Would eat: tail stays. Bite if npos is in body at all
            // Allow suffix body (releaseAt <= 1) — bite cuts the suffix
            if (st.bodyOccupiedPos(npos) && releaseAt(npos) > 1) continue;
        }
...
void transitionGoTargetCandidates(
    ...
    // buildReleaseSparse already called by caller
    ...
    for (int pi = 0; pi < pathCount; ++pi) {
        ...
        if (cur.st.turn + path.len > activeTurnLimit) continue;
...
void transitionPrefixBrokenCutRecover(
    ...
    for (int pi = 0; pi < pathCount; ++pi) {
        ...
        bool ok = true;
        uint8_t savedBodyBuf[SCAP];
        int16_t savedBodyHead, savedBodyLen;
        for (int si = 0; si < path.len; ++si) {
            savedBodyHead = nxt.st.bodyHead;
            savedBodyLen = nxt.st.bodyLen;
            {
                int h = savedBodyHead, len = savedBodyLen;
                if (h + len <= SCAP) {
                    memcpy(savedBodyBuf + h, nxt.st.bodyBuf + h, len);
...
bool runBeamSearch(
    ...
    for (int turn = startTurn; turn <= beamTurnCap; ++turn) {
        ...
        for (int oi = 0; oi < ordN; ++oi) {
            ...
            // Build release schedule once for all transitions on this node
            // suffix-wrong body (index >= prefix) gets release=1 so BFS can route through it
            buildReleaseSparse(node.st, node.prefix);
