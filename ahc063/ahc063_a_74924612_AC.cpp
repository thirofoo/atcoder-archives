/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc063/submissions/74924612
 * Submitted at: 2026-04-13 04:14:45
...
// Avoids all food cells on the path. Returns up to topK paths.
int findPrefixBrokenCutPaths(const SnakeState& st, int p, int topK, FixedPath* out) const {
    ...
    for (int di = 0; di < 4; ++di) {
        ...
        if (st.foodOcc[gBlkByPos[npos]] & gMaskByPos[npos]) continue;
        if (1 < releaseAt(npos)) continue;
        if (bfsSeenStamp[npos] == bfsStamp) continue;
        bfsSeenStamp[npos] = bfsStamp;
        bfsParent[npos] = head;
