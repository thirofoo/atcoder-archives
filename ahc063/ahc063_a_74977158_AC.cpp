/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc063/submissions/74977158
 * Submitted at: 2026-04-15 17:26:11
...
CutCandidateList enumerateCutCandidatesBFS(
    const SnakeState& src, int baseL, int targetColor, int maxDepth,
    int maxAfterBodyLen = -1
) const {
    CutCandidateList out;
    // Track which afterBodyLen values we've already found (one per bite target)
    bool seenAfterLen[NCELLS + 1] = {};
    ...
    static constexpr int BFS_Q_CAP = 2048;
    static BfsNode bfsQ[BFS_Q_CAP];
    int qh = 0, qt = 0;
    ...
    while (qh < qt) {
        BfsNode& cur = bfsQ[qh++];
        if (cur.seqLen >= maxDepth) continue;

        uint8_t head = cur.st.bodyFront();
        uint8_t neck = (cur.st.bodyLen >= 2) ? cur.st.bodyAt(1) : 255;
        uint8_t tail = cur.st.bodyBack();
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            int nposBlk = gBlkByPos[npos];
            uint64_t nposMask = gMaskByPos[npos];
            bool occupied = cur.st.bodyOccupiedPos(npos);
            bool eats = cur.st.hasFoodAtIdx(nposBlk, nposMask);
            bool biteCandidate = occupied && (eats || npos != tail || cur.st.overlapPos == npos);

            if (biteCandidate) {
                SnakeState nxt = cur.st;
                MoveResult ret = nxt.apply(d);
                if (!ret.ok || !ret.bite) continue;
                int lenAfter = nxt.bodyLen;
                if (maxAfterBodyLen >= 0 && lenAfter > maxAfterBodyLen) continue;
                if (lenAfter >= CUT_KEEP_MIN_LEN && (lenAfter % 2 == 1) && !seenAfterLen[lenAfter]) {
                    seenAfterLen[lenAfter] = true;
                    int anchorLen = min(baseL, lenAfter);
                    CutCandidate cand;
                    memcpy(cand.seq, cur.seq, cur.seqLen);
                    cand.seq[cur.seqLen] = (int8_t)d;
                    cand.seqLen = cur.seqLen + 1;
                    cand.afterBodyLen = (int16_t)lenAfter;
                    cand.adjacentToPrefix = isTargetAdjacentToPrefixSegment(nxt, anchorLen, targetColor);
                    out.push_back(cand);
                    if (out.size >= CUT_CANDIDATE_CAP) return out;
                }
            } else {
                if (qt >= BFS_Q_CAP) continue;
                BfsNode& child = bfsQ[qt];
                child.st = cur.st;
                MoveResult ret = child.st.apply(d);
                if (!ret.ok || ret.bite) continue;
                memcpy(child.seq, cur.seq, cur.seqLen);
                child.seq[cur.seqLen] = (int8_t)d;
                child.seqLen = cur.seqLen + 1;
                ++qt;
            }
        }
    }
...
CutCandidateList chooseTopCutCandidatesForBeam(
    ...
    topK = max(1, topK);
    auto cands = enumerateCutCandidatesBFS(src, baseL, targetColor, CUT_ENUM_MAX_DEPTH_BY_N[N], maxAfterBodyLen);
...
// Prefix-broken cut: BFS enumerate bite candidates with afterBodyLen <= p,
// rank with same evaluation as normal Cut, then recover.
template<typename InsertFn>
void transitionPrefixBrokenCutRecover(
    ...
    auto cuts = chooseTopCutCandidatesForBeam(cur.st, p, desired[p], topK, p);
    if (cuts.empty()) return;
