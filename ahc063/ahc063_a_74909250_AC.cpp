/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc063/submissions/74909250
 * Submitted at: 2026-04-12 14:20:48
 * Problem URL: https://atcoder.jp/contests/ahc063/tasks/ahc063_a
 * Result: AC
 * Execution Time: 1993 ms
 */
...
constexpr int SEARCH_TIME_LIMIT_MS = 1970;
...
struct SnakeState {
    ...
    MoveResult apply(int d) {
        ...
        uint16_t nposIdx = gCellIdxByPos[npos];
        int nposBlk = nposIdx >> 6;
        uint64_t nposMask = 1ULL << (nposIdx & 63);
        ...
        if (__builtin_expect(!eats && !biteCandidate, 1)) {
            ...
            if (tail != npos) {
                if (overlapPos == tail) {
                    ...
                    uint16_t tailIdx = gCellIdxByPos[tail];
                    bodyOcc[tailIdx >> 6] &= ~(1ULL << (tailIdx & 63));
                }
            }
...
enum BeamActionKind : uint8_t {
    BEAM_ACTION_ROOT = 0,
    BEAM_ACTION_GO_TARGET = 1,
    BEAM_ACTION_CUT_RECOVER = 2,
    BEAM_ACTION_WANDER = 3,
};
...
struct BeamNode {
    ...
    int optimisticLB = -1;
    uint64_t stateKey = 0;
    uint8_t actionKind = BEAM_ACTION_ROOT;
...
FixedPath findPreferFoodAvoidPathToTarget(const SnakeState& st, int targetColor) const {
    ...
    reconstructPathBackward(result, qd[qh - 1], goalPos, bfsParent, bfsPdir);
    return result;
}
...
inline void initChildNode(const BeamNode& parent, BeamNode& child) const {
    ...
    child.actionKind = BEAM_ACTION_ROOT;
}
...
int selectTopBeamIndices(const vector<BeamNode>& beam, int beamWidth, int* order) const {
    int n = (int)beam.size();
    static int idx[16384];
    static uint8_t used[16384];
    int skN = min(n, 16384);
    for (int i = 0; i < skN; ++i) idx[i] = i;
    sort(idx, idx + skN, [&](int a, int b) {
        return betterBeamNode(beam[a], beam[b]);
    });
    memset(used, 0, (size_t)skN * sizeof(uint8_t));

    int goMax = beamWidth * 90 / 100;
    int cutMin = beamWidth * 0 / 100;
    int wanderMin = beamWidth * 0 / 100;
    if (beamWidth > 0) {
        goMax = max(goMax, 1);
        cutMin = max(cutMin, 1);
        wanderMin = max(wanderMin, 1);
    }
    if (goMax + cutMin + wanderMin > beamWidth) {
        goMax = max(0, beamWidth - cutMin - wanderMin);
    }

    int ordN = 0;
    int goCount = 0, cutCount = 0, wanderCount = 0;

    auto takeOneOfKind = [&](uint8_t kind) -> bool {
        for (int i = 0; i < skN; ++i) {
            if (used[i]) continue;
            int bi = idx[i];
            if (beam[bi].actionKind != kind) continue;
            used[i] = 1;
            order[ordN++] = bi;
            if (kind == BEAM_ACTION_GO_TARGET) ++goCount;
            else if (kind == BEAM_ACTION_CUT_RECOVER) ++cutCount;
            else if (kind == BEAM_ACTION_WANDER) ++wanderCount;
            return true;
        }
        return false;
    };

    while (ordN < beamWidth && cutCount < cutMin) {
        if (!takeOneOfKind(BEAM_ACTION_CUT_RECOVER)) break;
    }
    while (ordN < beamWidth && wanderCount < wanderMin) {
        if (!takeOneOfKind(BEAM_ACTION_WANDER)) break;
    }

    while (ordN < beamWidth) {
        int pickPos = -1;
        for (int i = 0; i < skN; ++i) {
            if (used[i]) continue;
            int bi = idx[i];
            if (beam[bi].actionKind == BEAM_ACTION_GO_TARGET && goCount >= goMax) continue;
            pickPos = i;
            break;
        }
        if (pickPos < 0) {
        ...
        int bi = idx[pickPos];
        order[ordN++] = bi;
        if (beam[bi].actionKind == BEAM_ACTION_GO_TARGET) ++goCount;
        else if (beam[bi].actionKind == BEAM_ACTION_CUT_RECOVER) ++cutCount;
        else if (beam[bi].actionKind == BEAM_ACTION_WANDER) ++wanderCount;
    }
    return ordN;
}
...
void transitionGoTargetCandidates(
    ...
    for (int pi = 0; pi < pathCount; ++pi) {
        ...
        nxt.actionKind = BEAM_ACTION_GO_TARGET;
...
void transitionCutRecoverCandidates(
    ...
    for (int ci = 0; ci < cuts.size; ++ci) {
        ...
        BeamNode nxt;
        initChildNode(cur, nxt);
        nxt.actionKind = BEAM_ACTION_CUT_RECOVER;
...
void transitionWander(const BeamNode& cur, DeadlineChecker& timer, int topK, InsertFn& insertFn) {
    ...
    out.actionKind = BEAM_ACTION_WANDER;
    int moved = 0;
...
bool runBeamSearch(
    ...
    init.actionKind = BEAM_ACTION_ROOT;
    ...
    auto ins = [&](BeamNode&& cand) {
        ...
        int dupIdx = -1;
        {
            const int sz = (int)vec.size();
            const BeamNode* vp = vec.data();
            ...
            for (int i = sz - 1; i >= scanFrom; --i) {
                if (vp[i].stateKey == cand.stateKey) { dupIdx = i; goto found; }
            }
        }
        found:
        if (dupIdx < 0) {
