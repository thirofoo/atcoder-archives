/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc347/submissions/51887900
 * Submitted at: 2024-03-31 00:29:09
...
int main() {
    ...
    reps(i,m,n+1) {
        ...
        reps(j,m,n+1) {
            ...
            rep(k,3) col_max[k] = max(col_max[k], seg[j-m+1][k].prod(0, i+1).val);
        }
    ...
    reps(i,m,n+1) {
        ...
        for(ll j=n; j>=m; j--) {
            ...
            if( j+m-1 <= n ) rep(k,3) col_max[k] = max(col_max[k], seg[j+m-1][k].prod(0, i+1).val);
        }
        rep(k,3) pre_max[k] = dp[i-m+1][k].all_prod().val;
    }
