/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc213/submissions/34119911
 * Submitted at: 2022-08-18 13:09:04
...
inline bool outField(int nx,int ny){
...
int main() {
    ...
    while(!todo.empty()){
        ...
        rep(l,4){
            ...
            if(!outField(nx,ny) && field[nx][ny] == '.' && visited[nx][ny] > c)todo.push(T(c,nx,ny));
        }
        rep(l,20){
            int nx = x+dx2[l],ny = y+dy2[l];
            if(!outField(nx,ny) && visited[nx][ny] > c+1)todo.push(T(c+1,nx,ny));
        }
    }
