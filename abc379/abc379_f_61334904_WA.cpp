/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/abc379/submissions/61334904
 * Submitted at: 2025-01-03 16:15:32
...
int main() {
  ...
  for(ll i = n; i >= 1; i--) {
    for(auto [l, idx]: queries[i]) {
      ans[idx] = sg.prod(0, l);
      // cerr << "idx: " << idx << " prod: " << sg.prod(0, l) << endl;
    }
    sg.set(prev[i], sg.get(prev[i]) + 1);
    st.push(i);
    // rep(j, n + 1) cerr << sg.get(j) << " ";
    // cerr << endl;
  }
