/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc028/submissions/49252867
 * Submitted at: 2024-01-13 18:08:09
...
struct Solver{
    ...
    inline pair<int, vector<P>> calcScore(const string &str) {
            ...
            rep(j,n) rep(k,n) {
                ...
                for(auto [nx, ny]: place[str[i]-'A']) {
                    if( dp[i+1][nx][ny] > dp[i][j][k] + abs(nx-j) + abs(ny-k) ) {
                        dp[i+1][nx][ny] = dp[i][j][k] + abs(nx-j) + abs(ny-k);
                        pre[i+1][nx][ny] = j*n+k;
                    }
                }
            }
        ...
        for(int i=str.size(); i>=1; i--) {
            ...
            cand_score += abs(tsx-prex) + abs(tsy-prey);
            tsx = prex;
            tsy = prey;
        }
        cand.push_back({tsx, tsy});
