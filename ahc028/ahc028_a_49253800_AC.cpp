/*
 * Author: through
 * Submission URL: https://atcoder.jp/contests/ahc028/submissions/49253800
 * Submitted at: 2024-01-13 18:27:47
 * Problem URL: https://atcoder.jp/contests/ahc028/tasks/ahc028_a
 * Result: AC
 * Execution Time: 1806 ms
 */
...
struct Solver{
    ...
    void solve() {
        ...
        // 貪欲で生成した解を時間一杯 swap して山登りする…？
        while( utility::mytm.elapsed() <= TIME_LIMIT ) {
            int r1 = rand_int()%idx.size();
            int r2 = rand_int()%idx.size();
            while( r1 == r2 ) r2 = rand_int()%idx.size();
            swap(idx[r1], idx[r2]);
            string tmp = word[idx[0]];
            rep(i,idx.size()-1) {
                int cnt = score[idx[i]][idx[i+1]];
            ...
            // 移動 part
            // ~~ 3次元DPで最適化 ~~
            auto [cand_score, cand] = calcScore(tmp);
